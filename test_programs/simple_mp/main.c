...
int main(int argc, char **argv)
{
	...
	printf("OPS INDEX: %d", message_pool->ops_index);

	void *buf = NULL;
	if (rte_mempool_get(message_pool, &buf) < 0)
	{
		printf("FAILED TO GET BUF\n");
	}
	else
	{
		Message *data = (Message *)buf;
		memcpy(data->Data, "hello there", 12);
		data->DataLen = 12;
		printf("GOT SOME DATA\n");
		rte_mempool_put(message_pool, buf);
	}
