...
MODULE_PARM_DESC(cplane_server_path,
                 "Sets the path to the onload_cp_server binary.  Defaults to " DEFAULT_CPLANE_SERVER_PATH " if empty.");
                           ...
                           cp_fwd_table_id *fwd_table_id_out)
{
  ...
  if (mib_size > cp->bytes)
  {
    ci_log("%s: Cplane MIB dimensions 0x%zx do not match with mmaped area size"
           " 0x%lx",
           __FUNCTION__, mib_size, cp->bytes);
    kfree(mib->dim);
    mib->dim = NULL;
    return -EFAULT;
  }
...
int oo_cplane_mmap(struct file *file, struct vm_area_struct *vma)
{
  ci_private_t *priv = (ci_private_t *)file->private_data;
  struct oo_cplane_handle *cp;
...
int __cp_announce_hwport(struct oo_cplane_handle *cp, ci_ifid_t ifindex,
...
int cp_announce_hwport(const struct efhw_nic *nic, ci_hwport_id_t hwport)
{
  struct oo_cplane_handle *cp;
  int rc;
...
int oo_cp_get_active_hwport_mask(struct oo_cplane_handle *cp, ci_ifid_t ifindex,
...
{
  return cplane_server_path != NULL && *cplane_server_path != '\0' ? cplane_server_path : DEFAULT_CPLANE_SERVER_PATH;
}
...
#define UINT32_STRLEN 12
  ...
  ci_assert((flags & (CP_SPAWN_SERVER_SWITCH_NS | CP_SPAWN_SERVER_BOOTSTRAP)) != (CP_SPAWN_SERVER_SWITCH_NS | CP_SPAWN_SERVER_BOOTSTRAP));
...
int oo_cp_wait_for_server(struct oo_cplane_handle *cp, enum cp_sync_mode mode)
{
  int rc;
  cp_version_t ver = 0;
  cp_wait_check_fn fn;
...
int oo_cp_check_veth_acceleration(struct oo_cplane_handle *cp, ci_ifid_t ifindex)
{
  int rc = 0;
  struct net_device *dev;
...
int oo_cp_select_instance(ci_private_t *priv, enum oo_op_cp_select_instance inst)
{
  struct net *netns;
  int rc;
               ...
               ci_addr_sh_t *src_out)
{
   ...
   * removed. */
  ci_ifid_t iif = key->flag & CP_FWD_KEY_TRANSPARENT ? CI_IFID_LOOP : key->iif_ifindex;
...
void cicp_kernel_resolve(ci_netif *ni, struct oo_cplane_handle *cp,
