...
#if CI_CFG_EPOLL2
    else if (file->f_op == &oo_epoll_fops &&
  ...
  if (unlikely(!oo_epoll_add_stack(priv, fd_thr)))
  {
    ...
    if (!printed)
      ci_log("Can't add stack %d to epoll set: consider "
             "increasing epoll_max_stacks module option",
             fd_thr->id);
...
static int oo_epoll_has_event(struct file *filp)
{
  ...
  OO_EPOLL_FOR_EACH_STACK(priv, i, thr, ni)
  {
    if (ci_netif_has_event(ni))
      return OO_EPOLL1_EVENT_ON_EVQ | ((thr == priv->p.p1.home_stack) ? OO_EPOLL1_EVENT_ON_HOME : OO_EPOLL1_EVENT_ON_OTHER);
  }
  return 0;
}
...
static int oo_epoll_fop_release(struct inode *inode, struct file *filp)
{
  ...
  switch (priv->type)
  {
  case OO_EPOLL_TYPE_1:
    oo_epoll1_release(priv);
    break;
#if CI_CFG_EPOLL2
  case OO_EPOLL_TYPE_2:
    oo_epoll2_release(priv);
    break;
#endif
  default:
    ci_assert_equal(priv->type, OO_EPOLL_TYPE_UNKNOWN);
  }
