...
static void efab_tcp_helper_rm_schedule_free(tcp_helper_resource_t*);
#endif


#if ! CI_CFG_UL_INTERRUPT_HELPER
static int oo_handle_wakeup_int_driven(
    void*, int is_timeout, struct efhw_nic*, int budget);
static int oo_handle_wakeup_or_timeout(
    void*, int is_timeout, struct efhw_nic*, int budget);
static void tcp_helper_initialize_and_start_periodic_timer(
    tcp_helper_resource_t*);
static void tcp_helper_stop_periodic_work(tcp_helper_resource_t*);

static void tcp_helper_close_pending_endpoints(tcp_helper_resource_t*);
#endif
...
#if CI_CFG_EPOLL3
static void get_os_ready_list(tcp_helper_resource_t* thr, int ready_list);
#endif

static void efab_tcp_helper_drop_os_socket(
    tcp_helper_resource_t* trs, tcp_helper_endpoint_t* ep);
...
ci_inline int oo_trusted_lock_is_locked(tcp_helper_resource_t* trs)
{
  return trs->trusted_lock & OO_TRUSTED_LOCK_LOCKED;
}


static int oo_trusted_lock_try_lock(tcp_helper_resource_t* trs)
{
 ...
 */
static int oo_trusted_lock_set_flags_if_locked(
    tcp_helper_resource_t* trs, unsigned flags)
{
  unsigned l;
 ...
 */
static int oo_trusted_lock_drop(
    tcp_helper_resource_t* trs, int in_dl_context, int has_shared)
{
  unsigned l;
  ci_uint64 sl_flags;
...
again:
  ...
  if( l & OO_TRUSTED_LOCK_CLOSE_ENDPOINT ) {
    ...
    if( has_shared || ef_eplock_lock_or_set_flag(&trs->netif.state->lock,
                          CI_EPLOCK_NETIF_CLOSE_ENDPOINT) ) {
      /* let's reset the shared lock flag to avoid flag ping pong */
      ef_eplock_clear_flags(
          &trs->netif.state->lock, CI_EPLOCK_NETIF_CLOSE_ENDPOINT);

      /* We've got both locks.  If in non-dl context, do the work, else
       * defer work and locks to workitem.
       */
      if( in_dl_context ) {
        /* the flag needs to be reinstated for atomic work to undertake the
         * work */
        oo_trusted_lock_set_flags_if_locked(
            trs, OO_TRUSTED_LOCK_CLOSE_ENDPOINT);
      ...
      OO_DEBUG_TCPH(
          ci_log("%s: [%u] CLOSE_ENDPOINT now", __FUNCTION__, trs->id));
  ...
  if( l & OO_TRUSTED_LOCK_OS_READY ) {
    ...
    if( has_shared || ef_eplock_lock_or_set_flag(&trs->netif.state->lock,
      ...
      OO_DEBUG_TCPH(ci_log("%s: [%u] OS READY now", __FUNCTION__, trs->id));

#if CI_CFG_EPOLL3
      CI_READY_LIST_EACH(trs->netif.state->ready_lists_in_use, tmp, i)
      {
        get_os_ready_list(trs, i);
        if( ! oo_p_dllink_is_empty(
                &trs->netif, oo_p_dllink_ptr(&trs->netif,
  ...
  if( ci_cas32_succeed(&trs->trusted_lock, l, OO_TRUSTED_LOCK_LOCKED) ) {
    ...
    else if( ef_eplock_trylock_and_set_flags(
                 &trs->netif.state->lock, sl_flags) )
      efab_eplock_unlock_and_wake(ni, in_dl_context);
  }
  goto again;
}
 ...
 */
static int oo_trusted_lock_lock_and_set_flags(
    tcp_helper_resource_t* trs, unsigned flags)
{
  unsigned l, new_l;
...
int efab_tcp_helper_netif_try_lock(
    tcp_helper_resource_t* trs, int in_dl_context)
{
...
void efab_tcp_helper_netif_unlock(
    tcp_helper_resource_t* trs, int in_dl_context)
{
  ci_assert_equiv(
      in_dl_context, trs->netif.flags & CI_NETIF_FLAG_IN_DL_CONTEXT);
#if 0
 ...
 */
int efab_tcp_helper_netif_lock_or_set_flags(tcp_helper_resource_t* trs,
    unsigned trusted_flags, ci_uint64 untrusted_flags, int in_dl_context)
{
  do {
    if( efab_tcp_helper_netif_try_lock(trs, in_dl_context) )
      return 1;
    if( ef_eplock_set_flags_if_locked(
            &trs->netif.state->lock, untrusted_flags) )
      return 0;
...
static int efab_thr_table_check_name(const char* name, struct net* netns)
{
...
int efab_thr_get_inaccessible_stack_info(
    unsigned id, uid_t* uid, uid_t* euid, ci_int32* share_with, char* name)
{
  tcp_helpers_table_t* table = &THR_TABLE;
  ci_irqlock_state_t lock_flags;
  tcp_helper_resource_t* thr;
  ...
  CI_DLLIST_FOR_EACH(link, &table->all_stacks)
  {
    ...
    if( match ) {
      if( NI_OPTS(&thr->netif).share_with > 0 ) {
         ...
         * to the kernel space.
         */
        uid_t kshare_with = ci_make_kuid(
            tcp_helper_get_user_ns(thr), NI_OPTS(&thr->netif).share_with);
...
int efab_thr_user_can_access_stack(
    uid_t uid, uid_t euid, tcp_helper_resource_t* thr)
{
  uid_t kshare_with;
 ...
 * stacks.
 */
int efab_thr_table_lookup(const char* name, struct net* netns, unsigned id,
    int flags, enum oo_thr_ref_type ref_type, tcp_helper_resource_t** thr_p)
{
  tcp_helpers_table_t* table = &THR_TABLE;
  ci_irqlock_state_t lock_flags;
  tcp_helper_resource_t* thr;
    ...
    if( match ) {
      if( ! efab_thr_can_access_stack(thr, flags) ) {
        if( ! (flags & EFAB_THR_TABLE_LOOKUP_NO_WARN) ) {
          ...
          ci_log(
              "User %d:%d can't share stack %d(%s) owned by %d:%d "
              "share_with=%d",
              ci_current_from_kuid_munged(ci_getuid()),
              ci_current_from_kuid_munged(ci_geteuid()), thr->id, thr->name,
...
static void tcp_helper_reduce_max_packets(ci_netif* ni, int new_max_packets)
{
  ...
  NI_OPTS(ni).max_rx_packets = rescale(
      NI_OPTS(ni).max_rx_packets, new_max_packets, NI_OPTS(ni).max_packets);
  NI_OPTS(ni).max_tx_packets = rescale(
...
#if ! CI_CFG_UL_INTERRUPT_HELPER
static int __tcp_helper_kill_stack_by_id(unsigned id, unsigned ignore_id)
{
  tcp_helpers_table_t* table = &THR_TABLE;
  ci_irqlock_state_t lock_flags;
  ...
  if( rc == 0 ) {
    ...
    if( ignore_id )
      OO_DEBUG_TCPH(
          ci_log("Orphaned stack %d(%s) owned by %d:%d has been "
                 "released.",
              thr->id, thr->name, ci_current_from_kuid_munged(thr->netif.kuid),
              ci_current_from_kuid_munged(thr->netif.keuid)));
...
void tcp_helper_resource_assert_valid(
    tcp_helper_resource_t* thr, int no_ul, const char* file, int line)
{
...
int tcp_helper_vi_hw_rx_loopback_supported(
    tcp_helper_resource_t* trs, int hwport)
{
  int intf_i;
  ci_assert_lt((unsigned) hwport, CI_CFG_MAX_HWPORTS);
  if( (intf_i = trs->netif.hwport_to_intf_i[hwport]) >= 0 )
...
int tcp_helper_vi_hw_drop_filter_supported(
    tcp_helper_resource_t* trs, int hwport)
{
  int intf_i;
  ci_assert_lt((unsigned) hwport, CI_CFG_MAX_HWPORTS);
...
int tcp_helper_post_filter_add(tcp_helper_resource_t* trs, int hwport,
    const struct efx_filter_spec* spec, int rxq, bool replace)
{
  int intf_i;
  struct efhw_nic* nic;

  int hugepages =
      max(1, DIV_ROUND_UP(NI_OPTS_TRS(trs).rxq_size * EFCT_PKT_STRIDE,
                 CI_HUGEPAGE_SIZE));
  ci_assert_lt((unsigned) hwport, CI_CFG_MAX_HWPORTS);
  if( (intf_i = trs->netif.hwport_to_intf_i[hwport]) < 0 )
    return 0;
  ...
  if( efhw_nic_max_shared_rxqs(nic) ) {
    ...
    rc = efrm_rxq_alloc(vi_rs, rxq, qix, true, hugepages, trs->thc_efct_memfd,
        &trs->thc_efct_memfd_off, &trs->nic[intf_i].thn_efct_rxq[qix]);
    if( rc < 0 ) {
    ...
    if( NI_OPTS(&trs->netif).int_driven ) {
      ...
      if( efab_tcp_helper_netif_lock_or_set_flags(trs,
        ...
        ef_eplock_holder_set_flag(&trs->netif.state->lock,
            CI_EPLOCK_NETIF_NEED_POLL | CI_EPLOCK_NETIF_NEED_PRIME);
...
static int allocate_pio(tcp_helper_resource_t* trs, int intf_i,
    struct efrm_pd* pd, struct efhw_nic* nic, unsigned* pio_buf_offset)
{
  ci_netif* ni = &trs->netif;
  ...
  rc = efrm_pio_map_kernel(
      tcp_helper_vi(trs, intf_i), (void**) &netif_nic->pio.pio_io);
  if( rc < 0 ) {
 ...
 */
static int check_timestamping_support(const char* stack_name, const char* dir,
    int user_val, int device_supports_ts, const char* if_name, int* out_try_ts,
    int* out_retry_without)
{
...
static int allocate_pd(
    ci_netif* ni, struct vi_allocate_info* info, struct efhw_nic* nic)
{
  int rc = 0;
...
#if CI_CFG_CTPIO
static int /*bool*/ should_try_ctpio(
    ci_netif* ni, struct efhw_nic* nic, struct vi_allocate_info* info)
{
  return nic->flags & NIC_FLAG_CTPIO_ONLY ||
         /* Stack configured to use CTPIO. */
         (NI_OPTS(ni).ctpio > 0 &&
             /* NIC claims support for CTPIO. */
             (nic->flags & NIC_FLAG_TX_CTPIO) != 0 &&
             /* CTPIO bypasses the NIC's switch.  When the switch is enabled,
              * don't use CTPIO unless we've been told to do so explicitly. */
             (~nic->flags & NIC_FLAG_MCAST_LOOP_HW ||
                 NI_OPTS(ni).ctpio_switch_bypass));
}
#endif

static int get_vi_settings(
    ci_netif* ni, struct efhw_nic* nic, struct vi_allocate_info* info)
{
  ...
  info->log_resource_warnings =
      NI_OPTS(ni).log_category & (1 << (EF_LOG_RESOURCE_WARNINGS));
  ...
  if( should_try_ctpio(ni, nic, info) ) {
    info->try_ctpio = 1;
    info->retry_without_ctpio =
        NI_OPTS(ni).ctpio < 2 && ! (nic->flags & NIC_FLAG_CTPIO_ONLY);
  ...
  rc = check_timestamping_support(ni->state->pretty_name, "RX",
      ...
      (nic->flags & NIC_FLAG_HW_RX_TIMESTAMPING) != 0, if_name,
      &info->try_rx_ts, &info->retry_without_rx_ts);

  if( rc == 0 )
    rc = check_timestamping_support(ni->state->pretty_name, "TX",
        NI_OPTS(ni).tx_timestamping,
        (nic->flags & NIC_FLAG_HW_TX_TIMESTAMPING) != 0, if_name,
        &info->try_tx_ts, &info->retry_without_tx_ts);
    ...
    *info = info_base;
    ...
    for( i = 0; i < 2; ++i ) {
      rc = efrm_vi_resource_alloc(info->client, evq_virs, info->vi_set, -1,
          info->pd, info->name, info->efhw_flags, info->evq_capacity,
          info->txq_capacity, info->rxq_capacity, q_tag, q_tag,
          info->wakeup_cpu_core, info->wakeup_channel, info->virs,
          &info->vi_io_mmap_bytes, &info->vi_ctpio_mmap_bytes, NULL, NULL,
          info->log_resource_warnings);
    ...
    if( feature_mask == required_feature_mask &&
      ...
      info_base.wakeup_cpu_core = NI_OPTS(ni).irq_core;
      info_base.log_resource_warnings =
          NI_OPTS(ni).log_category & (1 << (EF_LOG_RESOURCE_WARNINGS));
  ...
  if( rc < 0 ) {
    ...
    for( i = 0; i < feature_count; ++i )
      if( features[i].attempt && ! (feature_mask & (1 << i)) ) {
        ...
        get_if_name(ni, info->intf_i, if_name);
        ci_log(
            "[%s]: enabling %s on interface %s failed, "
...
{
  return efrm_rxq_refresh_kernel(
      vi->dh, vi->efct_shm->q[qid].qid, vi->efct_rxq[qid].superbufs);
}


static int initialise_vi(ci_netif* ni, struct ef_vi* vi, struct efrm_vi* vi_rs,
    struct efrm_vi_mappings* vm, void* vi_state, int vi_arch, int vi_variant,
    ...
    ef_vi_stats* vi_stats)
{
  ...
  *vi_out_flags = (vm->out_flags & EFHW_VI_CLOCK_SYNC_STATUS)
  ...
  if( vm->rxq_size > 0 ) {
    ef_vi_init_rxq(
        vi, vm->rxq_size, vm->rxq_descriptors, vi_ids, vm->rxq_prefix_len);
    vi_ids += vm->rxq_size;
  } else {
...
static int allocate_vis(tcp_helper_resource_t* trs,
    ci_resource_onload_alloc_t* alloc, void* vi_state,
    tcp_helper_cluster_t* thc)
{
  ...
  OO_STACK_FOR_EACH_INTF_I(ni, intf_i)
  {
    ...
    snprintf(
        vf_name, sizeof(vf_name), "onload:%s-%d", ns->pretty_name, intf_i);
    ...
    rc = initialise_vi(ni, vi, tcp_helper_vi(trs, intf_i), vm, vi_state,
        nic->devtype.arch, nic->devtype.variant, nic->devtype.revision,
        efhw_vi_nic_flags(nic), &alloc_info, &vi_out_flags,
        &ni->state->vi_stats);
    if( rc < 0 )
      goto error_out;
    ...
    nsn->ctpio_max_frame_len = nsn->ctpio_frame_len_check =
        nsn->oo_vi_flags & OO_VI_FLAGS_CTPIO_EN
            ? NI_OPTS(ni).ctpio_max_frame_len
            : 0;
#endif
    ...
    nsn->vi_instance[0] = (ci_uint16) EFAB_VI_RESOURCE_INSTANCE(vi_rs);
    ...
    vi_state =
        (char*) vi_state + ef_vi_calc_state_bytes(vm->rxq_size, vm->txq_size);
    ...
    if( NI_OPTS(ni).tcp_offload_plugin ) {
      ...
      for( vi_i = 1; vi_i < num_vis; ++vi_i ) {
        struct efrm_vi* vi_rs;
        int qid = ef_vi_add_queue(
            ci_netif_vi(ni, intf_i), &ni->nic_hw[intf_i].vis[vi_i]);
        ...
        rc = initialise_vi(ni, &ni->nic_hw[intf_i].vis[vi_i], vi_rs, vm,
        ...
        nsn->vi_instance[vi_i] = (ci_uint16) EFAB_VI_RESOURCE_INSTANCE(vi_rs);
        nsn->vi_abs_idx[vi_i] =
  ...
  if( ni->flags & CI_NETIF_FLAG_AF_XDP ) {
    ...
    if( NI_OPTS(ni).huge_pages != 0 ) {
      NI_LOG(ni, RESOURCE_WARNINGS,
          ...
          "Ignoring and not disabling hugepage support.",
          ni->state->pretty_name);

      NI_OPTS(ni).huge_pages = 1;
    }
  ...
  OO_STACK_FOR_EACH_INTF_I(ni, intf_i)
  {
    ...
    for( vi_i = 0; vi_i < ci_netif_num_vis(&trs->netif); ++vi_i ) {
      ...
      rc = efrm_vi_resource_deferred(trs_nic->thn_vi_rs[vi_i],
          CI_CFG_PKT_BUF_SIZE, offsetof(ci_ip_pkt_fmt, dma_start),
          &mmap_bytes);
      if( rc < 0 )
        return rc;
...
#endif
    OO_STACK_FOR_EACH_INTF_I(ni, intf_i)
    oo_iobufset_resource_release(
        ni->nic_hw[intf_i].pkt_rs[i], intfs_suspended(trs) & (1 << intf_i));
  }
#ifndef NDEBUG
...
static void release_vi(tcp_helper_resource_t* trs)
{
  ...
  OO_STACK_FOR_EACH_INTF_I(&trs->netif, intf_i)
  {
...
#endif
#if CI_CFG_PIO
    if( NI_OPTS(&trs->netif).pio && (nic->devtype.arch == EFHW_ARCH_EF10) &&
        (trs_nic->thn_pio_io_mmap_bytes != 0) ) {
      efrm_pio_unmap_kernel(
          tcp_helper_vi(trs, intf_i), (void*) netif_nic->pio.pio_io);
    ...
    if( trs_nic->thn_ctpio_io_mmap != NULL )
      efrm_ctpio_unmap_kernel(
          tcp_helper_vi(trs, intf_i), trs_nic->thn_ctpio_io_mmap);
#endif
    for( vi_i = num_vis - 1; vi_i >= 0; --vi_i ) {
...
static void tcp_helper_leak_check(tcp_helper_resource_t* trs)
{
  ...
  for( i = 0; i < CI_TCP_AUX_TYPE_NUM; i++ ) {
    ...
    if( ni->state->n_aux_bufs[i] != 0 ) {
      ci_log("%s[%d]: aux_bufs[%s]: leaked %d out of %d", __func__, NI_ID(ni),
          ci_tcp_aux_type2str(i), ni->state->n_aux_bufs[i],
          ni->state->max_aux_bufs[i]);
    }
  }
...
static int allocate_netif_resources(ci_resource_onload_alloc_t* alloc,
    tcp_helper_resource_t* trs, int cluster_size)
{
  ...
  if( NI_OPTS(ni).max_packets > max_packets_per_stack ) {
    OO_DEBUG_ERR(
        ci_log("WARNING: EF_MAX_PACKETS reduced from %d to %d due to "
  ...
  if( ci_netif_should_allocate_tcp_shared_local_ports(ni) ) {
    no_active_wild_pools =
        is_power_of_2(cluster_size) ? cluster_size : RSS_HASH_SIZE;
    /* Max active wilds is bounded by the number of local IPs and
     * shared local ports. */
    no_active_wild_table_entries = CI_MIN((ci_uint32) CI_CFG_MAX_LOCAL_IPADDRS,
  ...
  filter_table_ext_size =
      sizeof(ci_netif_filter_table_entry_ext) * no_table_entries;
#if CI_CFG_IPV6
  ip6_filter_table_size =
      sizeof(ci_ip6_netif_filter_table) +
  ...
  dma_addrs_bytes = ni->pkt_sets_max * CI_CFG_MAX_INTERFACES * PKTS_PER_SET /
                    (PAGE_SIZE / CI_CFG_PKT_BUF_SIZE) * sizeof(ef_addr);
  sz += dma_addrs_bytes;
  ...
  if( rc < 0 ) {
    OO_DEBUG_ERR(
        ci_log("%s: failed to alloc shmbuf for shared state and "
               "socket buffers (%d)",
            __FUNCTION__, rc));
    goto fail1;
  }
...
#if CI_CFG_UL_INTERRUPT_HELPER
  oo_ringbuffer_state_init(
      &ns->closed_eps, OO_CLOSED_EPS_RING_SIZE, sizeof(oo_sp));
  ...
  OO_DEBUG_MEMSIZE(
      ci_log("added %d (0x%x) bytes for shared netif state and ep buffers, "
             "reached %d (0x%x)",
          ns->netif_mmap_bytes, ns->netif_mmap_bytes, trs->mem_mmap_bytes,
          trs->mem_mmap_bytes));

  if( trs->name[0] == '\0' )
    snprintf(ns->pretty_name, sizeof(ns->pretty_name), "%d", ns->stack_id);
  else
    snprintf(ns->pretty_name, sizeof(ns->pretty_name), "%d,%s", ns->stack_id,
        trs->name);
  ...
  if( rc < 0 ) {
    OO_DEBUG_ERR(
        ci_log("tcp_helper_alloc: failed to allocate EPLOCK (%d)", rc));
    goto fail2;
  }
  ...
  if( NI_OPTS(&trs->netif).int_driven )
    ci_atomic_set(
        &trs->wake_intfs, (1ull << oo_stack_intf_max(&trs->netif)) - 1);
  else
...
static int create_plugin_rx_app(tcp_helper_resource_t* trs)
{
#if CI_CFG_TCP_OFFLOAD_RECYCLER
  ci_netif* ni = &trs->netif;
  bool got_nic = false;
  ...
  OO_STACK_FOR_EACH_INTF_I(ni, intf_i)
  {
    ...
    if( rc ) {
      OO_DEBUG_ERR(ci_log(
          "%s: CEPH_CREATE_APP failed (%d, %d)", __FUNCTION__, intf_i, rc));
      goto fail_plugin_1;
    }
    ...
    if( meta.mapped_csr_size ) {
      size_t bar_off = ef10_tx_dma_page_base(nic->vi_stride,
          trs->nic[intf_i].thn_vi_rs[CI_Q_ID_TCP_APP]->rs.rs_instance);
      bar_off += meta.mapped_csr_offset;
      ni->nic_hw[intf_i].plugin_io = ci_ioremap(
          nic->ctr_ap_addr + (bar_off & PAGE_MASK), meta.mapped_csr_size);
  ...
  if( ! got_nic ) {
    OO_DEBUG_ERR(
        ci_log("%s: no EF100 NICs have the requested plugin", __FUNCTION__));
    return -ENOTSUPP;
  }
#endif
  return 0;
...
static void destroy_plugin_rx_app(tcp_helper_resource_t* trs)
{
#if CI_CFG_TCP_OFFLOAD_RECYCLER
  ci_netif* ni = &trs->netif;
  int intf_i;
...
static ci_uint32 get_plugin_tx_crc_table_region(struct oo_nic* nic)
{
  ci_uint32 region_id;
...
static void release_plugin_tx_crc_table_region(
    struct oo_nic* nic, ci_uint32 region_id)
{
  ci_irqlock_state_t lock_flags;
...
static int create_plugin_tx_app(tcp_helper_resource_t* trs)
{
#if CI_CFG_TX_CRC_OFFLOAD
  ci_netif* ni = &trs->netif;
  int intf_i;
  ...
  OO_STACK_FOR_EACH_INTF_I(ni, intf_i)
  {
    ...
    create = (struct xsn_storage_tx_create_app) { .vi_id = cpu_to_le16(
                                                      ef_vi_instance(tx_vi)) };
    rc = efrm_ext_msg(
        plugin, XSN_STORAGE_TX_CREATE_APP, &create, sizeof(create));
    if( rc ) {
      OO_DEBUG_ERR(ci_log(
          "%s: TX_CREATE_APP failed (%d, %d)", __FUNCTION__, intf_i, rc));
      efrm_ext_release(plugin);
      continue;
    }
...
static void destroy_plugin_tx_app(tcp_helper_resource_t* trs)
{
#if CI_CFG_TX_CRC_OFFLOAD
  ci_netif* ni = &trs->netif;
  int intf_i;
  ...
  OO_STACK_FOR_EACH_INTF_I(ni, intf_i)
  {
    ...
    ni->nic_hw[intf_i].plugin_tx = NULL;
#endif
    release_plugin_tx_crc_table_region(
        trs->nic[intf_i].thn_oo_nic, ni->nic_hw[intf_i].plugin_tx_region_id);
  }
#endif
}


static int allocate_netif_hw_resources(ci_resource_onload_alloc_t* alloc,
    tcp_helper_cluster_t* thc, tcp_helper_resource_t* trs)
{
  ci_netif* ni = &trs->netif;
  ci_netif_state* ns = ni->state;
  int sz, rc;
  ...
  if( NI_OPTS(ni).prealloc_packets && trs->thc_efct_memfd ) {
    ...
    if( n_rxqs ) {
      ...
      if( rc < 0 ) {
        if( rc == -ENOSPC )
          OO_DEBUG_ERR(
              ci_log("tcp_helper_alloc: fallocate hugepage memory "
  ...
  if( (ni->pkt_bufs = vmalloc(sz)) == NULL ) {
    OO_DEBUG_ERR(
        ci_log("tcp_helper_alloc: failed to allocate iobufset table"));
    rc = -ENOMEM;
    goto fail4;
  }
  ...
  OO_STACK_FOR_EACH_INTF_I(ni, intf_i)
  {
    if( (ni->nic_hw[intf_i].pkt_rs = ci_alloc(sz)) == NULL ) {
      OO_DEBUG_ERR(
          ci_log("%s: failed to allocate iobufset tables", __FUNCTION__));
      rc = -ENOMEM;
      goto fail5;
    }
  ...
  OO_DEBUG_MEMSIZE(ci_log("helper=%u map_bytes=%u (0x%x)", trs->id,
  ...
  OO_STACK_FOR_EACH_INTF_I(ni, intf_i)
  {
    ...
    for( i = 0; i < ci_netif_num_vis(ni); ++i )
      LOG_NC(
          ci_log("VI[%d]=%d", i, ef_vi_instance(&ni->nic_hw[intf_i].vis[i])));
  }
...
static void release_ep_tbl(tcp_helper_resource_t* trs)
{
  ci_netif* ni = &trs->netif;
  int i;
...
static void release_netif_resources(tcp_helper_resource_t* trs)
{
  ci_netif* ni = &trs->netif;
  int i;
...
static void release_netif_hw_resources(tcp_helper_resource_t* trs)
{
  OO_DEBUG_SHM(ci_log("%s:", __func__));
...
int oo_version_check(
    const char* version, const char* uk_intf_ver, int debug_lib)
{
  return oo_version_check_impl(
      version, uk_intf_ver, debug_lib, oo_uk_intf_ver);
}
  ...
  while( 1 ) {
    ...
    if( ifindex ) {
      ...
      if( rc == 0 && hwport_mask != 0 ) {
        ...
        ci_log(
            "%s: WARNING interface %s constains %s, which "
            " is not identified as Solarflare interface",
            __FUNCTION__, tag, token);
      }
    } else {
      ci_log(
          "%s: WARNING interface %s contains %s, which "
...
static int oo_get_nics(tcp_helper_resource_t* trs, int ifindices_len)
{
  ...
  if( oo_get_listed_hwports(trs, NI_OPTS(ni).iface_whitelist, &whitelist_mask,
          "whitelist") == 0 ) {
  ...
  if( oo_get_listed_hwports(trs, NI_OPTS(ni).iface_blacklist, &whitelist_mask,
          "blacklist") == 0 ) {
    if( (whitelist_mask & ~hwport_mask) != 0 ) {
  ...
  if( trs->netif.nic_n == 0 && ifindices_len != 0 ) {
    ci_log(
        "%s: ERROR: No Solarflare network interfaces are active/UP,\n"
 ...
 */
void tcp_helper_defer_dl2work(tcp_helper_resource_t* trs, ci_uint32 flag)
{
  OO_DEBUG_TCPH(ci_log(
      "%s: [%u] defer locks with flag=%x", __FUNCTION__, trs->id, flag));
...
static void oo_inject_packets_kernel_force(ci_netif* ni)
{
...
static void tcp_helper_do_non_atomic(struct work_struct* data)
{
  tcp_helper_resource_t* trs =
      container_of(data, tcp_helper_resource_t, non_atomic_work);
  const unsigned handled_aflags =
      (OO_THR_EP_AFLAG_CLEAR_FILTERS | OO_THR_EP_AFLAG_NEED_FREE |
  ...
  again:
    ...
    if( ep_aflags & OO_THR_EP_AFLAG_NEED_FREE ) {
      ...
      tcp_helper_endpoint_clear_filters(ep, 0);
      citp_waitable_obj_free_nnl(
          &trs->netif, SP_TO_WAITABLE(&trs->netif, ep->id));
    }
#if CI_CFG_TCP_OFFLOAD_RECYCLER
  ...
  if( trs->trs_aflags & OO_THR_AFLAG_DEFERRED_TRUSTED ) {
    ...
    OO_DEBUG_TCPH(ci_log("%s: [%u] deferred locks trs_aflags=%d", __FUNCTION__,
        trs->id, trs->trs_aflags));
    ci_assert(ci_netif_is_locked(&trs->netif));
    ...
    if( trs_aflags & OO_THR_AFLAG_POLL_AND_PRIME ) {
      ...
      OO_DEBUG_TCPH(
          ci_log("%s: [%u] deferred POLL_AND_PRIME", __FUNCTION__, trs->id));
...
void tcp_helper_endpoint_queue_non_atomic(
    tcp_helper_endpoint_t* ep, unsigned why_aflag)
{
  ci_irqlock_state_t lock_flags;
...
static void tcp_helper_destroy_work(struct work_struct* data)
{
  tcp_helper_resource_t* trs =
      container_of(data, tcp_helper_resource_t, work_item_dtor);
...
static int tcp_helper_rm_alloc_proxy(ci_resource_onload_alloc_t* alloc,
    const ci_netif_config_opts* opts, int ifindices_len,
    tcp_helper_resource_t** rs_out)
{
  ...
  if( alloc->in_flags & CI_NETIF_FLAG_DO_ALLOCATE_SCALABLE_FILTERS_RSS ) {
    ...
    rc = tcp_helper_cluster_alloc_thr(alloc->in_name, alloc->in_cluster_size,
        alloc->in_cluster_restart, in_flags, opts, &rs);
    if( rc != 0 )
      return rc;
    ni = &rs->netif;
...
void tcp_helper_free_ephemeral_ports(
    struct efab_ephemeral_port_head* table, ci_uint32 entries)
{
  ci_uint32 i;
  for( i = 0; i < entries; ++i ) {
...
static void donate_ephemeral_ports(struct efab_ephemeral_port_head* list_head,
...
void tcp_helper_donate_ephemeral_ports(
    struct efab_ephemeral_port_head* list_head,
    struct efab_ephemeral_port_keeper* new_head,
    struct efab_ephemeral_port_keeper* new_tail, int count)
{
 ...
 * an IP-specific list). */
static void tcp_helper_donate_global_ephemeral_ports(
    struct efab_ephemeral_port_head* list_head,
    struct efab_ephemeral_port_keeper* new_head,
...
int tcp_helper_alloc_ephemeral_ports(
    struct efab_ephemeral_port_head* list_head,
    struct efab_ephemeral_port_head* global_head, ci_addr_t laddr, int count)
{
  ...
  for( i = 0; i < count; ++i ) {
    ...
    for( rc = -ENODATA; rc != 0 && existing != NULL;
      ...
      if( rc != 0 && rc != -EADDRINUSE )
        OO_DEBUG_ERR(CI_RLLOG(10,
            "%s: unexpected failure reusing " CI_IP_PRINTF_FORMAT
            ":%u for %d-th ephemeral "
            "port: rc=%d",
            __FUNCTION__, CI_IP_PRINTF_ARGS(&existing->laddr),
    ...
    if( keeper == NULL ) {
      if( (rc = efab_alloc_ephemeral_port(laddr, 0, &keeper)) != 0 ) {
        OO_DEBUG_ERR(
            ci_log("%s: failed to allocate %d-th ephemeral port: "
                   "rc=%d",
                __FUNCTION__, i, rc));
        break;
      }
  ...
  if( new_global_head != NULL ) {
    ...
    tcp_helper_donate_global_ephemeral_ports(
        global_head, new_global_head, new_global_tail);
  }

  return i;
}

static int __efab_create_os_socket(tcp_helper_resource_t* trs,
    tcp_helper_endpoint_t* ep, struct file* os_file, ci_int32 domain);

#if CI_CFG_TCP_SHARED_LOCAL_PORTS
static ci_active_wild* tcp_helper_alloc_active_wild(
    tcp_helper_resource_t* rs, struct efab_ephemeral_port_keeper* keeper)
{
  ci_active_wild* aw;
...
static int tcp_helper_alloc_to_aw_pool(tcp_helper_resource_t* rs,
    ci_addr_t laddr, struct efab_ephemeral_port_keeper* port)
{
  ci_netif* ni = &rs->netif;
  int idx;
  ...
  idx = ni->state->active_wild_pools_n > 1
            ? ci_netif_active_wild_nic_hash(
                  ni, addr_any, port->port_be16, addr_any, 0)
            : 0;
  idx &= ni->state->active_wild_pools_n - 1;
...
static int tcp_helper_alloc_list_to_aw_pool(tcp_helper_resource_t* rs,
    ci_addr_t laddr, struct efab_ephemeral_port_head* ports)
{
  ci_netif* ni = &rs->netif;
  int i = 0;
  int rc;
   ...
   * bigger problems. */
  rc = tcp_helper_get_ephemeral_port_list(rs->trs_ephem_table_consumed, laddr,
      rs->trs_ephem_table_entries, &consumed);
  ...
  for( ; port != NULL; port = port->next ) {
    ...
    if( rc < 0 ) {
       ...
       * without it.
       */
      NI_LOG(&rs->netif, RESOURCE_WARNINGS,
          "%s: Only alloced %d of active"
          " shared",
          __FUNCTION__, CI_MAX(0, i - 1));
      return -ENOBUFS;
    }
    consumed->head = port;
    ++i;
 ...
 * thread-safety.
 */
int tcp_helper_get_ephemeral_port_list(struct efab_ephemeral_port_head* table,
    ...
    struct efab_ephemeral_port_head** list_out)
{
  ...
  do {
    *list_out = &table[bucket];
    ...
    if( (*list_out)->port_count == EPHEMERAL_PORT_LIST_NO_PORT &&
        ci_cas32_succeed(
            &(*list_out)->port_count, EPHEMERAL_PORT_LIST_NO_PORT, 0) ) {
  ...
  CI_RLLOG(1,
      "%s: No space in ephemeral port table for local address " IPX_FMT,
      __FUNCTION__, IPX_ARG(AF_IP(laddr)));
...
static int tcp_helper_alloc_aw_for_ephem_ports(tcp_helper_resource_t* rs,
    struct efab_ephemeral_port_head* list_head, ci_addr_t laddr)
{
  int rc;
  int count = 0;
  struct efab_ephemeral_port_head* consumed;
...
int tcp_helper_increase_active_wild_pool(
    tcp_helper_resource_t* rs, ci_addr_t laddr)
{
  struct efab_ephemeral_port_head* list_head;
  ...
  rc = tcp_helper_get_ephemeral_port_list(
      rs->trs_ephem_table, laddr, rs->trs_ephem_table_entries, &list_head);
  ...
  if( ! CI_IPX_ADDR_IS_ANY(laddr) ) {
    rc = tcp_helper_get_ephemeral_port_list(rs->trs_ephem_table, addr_any,
        rs->trs_ephem_table_entries, &global_head);
  ...
  while( to_alloc > 0 ) {
    ...
    rc = tcp_helper_alloc_ephemeral_ports(
        list_head, global_head, laddr, CI_MIN(to_alloc, BATCH_SIZE));
    if( rc < 0 )
      goto out;
...
static int thr_install_tproxy(tcp_helper_resource_t* thr)
{
  ...
  return tcp_helper_install_tproxy(
      1, thr, NULL, ni_opts, thr->tproxy_ifindex, thr->tproxy_ifindex_count);
}


static void thr_uninstall_tproxy(tcp_helper_resource_t* thr)
{
  if( thr->tproxy_ifindex != NULL ) {
    tcp_helper_install_tproxy(
        0, thr, NULL, NULL, thr->tproxy_ifindex, thr->tproxy_ifindex_count);
...
static inline void netns_get_identifiers(
    ci_netif_state* state, const struct net* ns)
{
...
void tcp_helper_put_ns_components(
    struct oo_cplane_handle* cplane, struct oo_filter_ns* filter_ns)
{
...
int tcp_helper_get_ns_components(
    struct oo_cplane_handle** cplane, struct oo_filter_ns** filter_ns)
{
  int oof_preexisted;
  int rc;
  ...
  *filter_ns = oo_filter_ns_get(
      &efab_tcp_driver, current->nsproxy->net_ns, &oof_preexisted);
  if( *filter_ns == NULL ) {
...
struct efab_ephemeral_port_head* tcp_helper_alloc_ephem_table(
    ci_uint32 min_entries, ci_uint32* entries_out)
{
...
#if CI_CFG_UL_INTERRUPT_HELPER
static bool stack_has_events(ci_netif* ni, ci_uint32 intfs)
{
...
static bool stack_has_ul_job(
    tcp_helper_resource_t* trs, ci_uint32* intfs, ci_uint32* flags)
{
...
static void request_pending_wakeups(tcp_helper_resource_t* trs)
{
...
int oo_wait_for_interrupt(ci_private_t* priv, void* argp)
{
  ...
  while( 1 ) {
    ...
    rc = wait_event_interruptible_timeout(
        trs->ulh_waitq, stack_has_ul_job(trs, &intfs, &arg->flags), timeout);
    ...
    if( intfs == 0 && ci_frc64_get() - trs->netif.state->evq_last_prime >
...
static int oo_handle_wakeup_in_ul(
    void* context, int is_timeout, struct efhw_nic* nic, int budget)
{
  ...
  trs =
      CI_CONTAINER(tcp_helper_resource_t, nic[tcph_nic->thn_intf_i], tcph_nic);
  ...
  do {
    ...
    if( l & CI_EPLOCK_NETIF_NEED_POLL )
      break;
  } while( ci_cas64u_fail(
      &trs->netif.state->lock.lock, l, l | CI_EPLOCK_NETIF_NEED_POLL) );
...
int tcp_helper_rm_alloc(ci_resource_onload_alloc_t* alloc,
    const ci_netif_config_opts* opts, int ifindices_len,
    tcp_helper_cluster_t* thc, tcp_helper_resource_t** rs_out)
{
  tcp_helper_resource_t* rs;
  ci_irqlock_state_t lock_flags;
  struct efhw_nic* nic;
  ...
  if( (opts->packet_buffer_mode & CITP_PKTBUF_MODE_PHYS) &&
    ...
    OO_DEBUG_ERR(
        ci_log("%s: ERROR: EF_PACKET_BUFFER_MODE=%d not permitted "
               "(phys_mode_gid=%d egid=%d pid=%d)",
            __FUNCTION__, opts->packet_buffer_mode, phys_mode_gid,
            ci_getegid(), current->tgid);
  ...
  if( opts->packet_buffer_mode & ~CITP_PKTBUF_MODE_PHYS ) {
    OO_DEBUG_ERR(
        ci_log("%s: ERROR: EF_PACKET_BUFFER_MODE=%d not supported. It "
               "was used on 6000 series NICs only.",
            __FUNCTION__, opts->packet_buffer_mode);
        ci_log(
            "%s: HINT: Use EF_PACKET_BUFFER_MODE=0/2 instead.", __FUNCTION__));
    rc = -ENOTSUPP;
    goto fail1;
  }
  ...
  if( opts->xdp_mode == EF_XDP_MODE_COMPATIBLE && ! cplane_track_xdp ) {
    OO_DEBUG_ERR(
        ci_log("%s: ERROR: EF_XDP_MODE=compatible but "
  ...
  if( oo_accelerate_veth ) {
    ...
    if( ni->cplane_init_net == NULL ) {
      ...
      OO_DEBUG_ERR(ci_log(
          "%s: failed to get init_net control plane handle", __FUNCTION__));
      OO_DEBUG_ERR(
          ci_log("%s: support for containers will be limited", __FUNCTION__));
    }
  } else {
   ...
   */
  rs->wq = alloc_workqueue(
      rs->wq_name, WQ_UNBOUND | WQ_CPU_INTENSIVE | WQ_HIGHPRI | WQ_SYSFS, 0);
  if( rs->wq == NULL ) {
    OO_DEBUG_ERR(
        ci_log("%s: [%d] Failed to allocate stack due to workqueue "
               "allocation failure",
            __func__, NI_ID(ni)));
    rc = -ENOMEM;
    goto fail5;
   ...
   */
  rs->reset_wq = alloc_workqueue(
      rs->reset_wq_name, WQ_UNBOUND | WQ_HIGHPRI | WQ_SYSFS, 0);
  if( rs->reset_wq == NULL ) {
    OO_DEBUG_ERR(
        ci_log("%s: [%d] Failed to allocate stack due to reset workqueue "
               "allocation failure",
            __func__, NI_ID(ni)));
    rc = -ENOMEM;
    goto fail5a;
  ...
  if( alloc->in_memfd >= 0 ) {
    ...
    if( ! rs->thc_efct_memfd ) {
      rc = -EBADF;
      OO_DEBUG_ERR(ci_log("%s: [%d] Bad fd for efct (%d).", __func__,
          NI_ID(ni), alloc->in_memfd));
      goto fail_memfd;
    }
  }
  ...
  ** buffers. */
  ...
  if( thc != NULL ) {
    ...
    rs->trs_ephem_table_entries = thc->thc_ephem_table_entries;
  ...
  if( rs->trs_ephem_table != NULL ) {
    ...
    if( rs->trs_ephem_table_consumed == NULL ) {
      OO_DEBUG_ERR(
          ci_log("%s: [%d] Failed to allocate table of consumed "
                 "ephemeral ports.",
              __func__, NI_ID(ni)));
      goto fail12;
    }
  ...
  if( ci_netif_should_allocate_tcp_shared_local_ports(ni) &&
    ...
    if( thc == NULL ) {
      tcp_helper_get_ephemeral_port_list(rs->trs_ephem_table, addr_any,
          rs->trs_ephem_table_entries, &ephemeral_ports);
      tcp_helper_alloc_ephemeral_ports(
          ephemeral_ports, NULL, addr_any, NI_OPTS(ni).tcp_shared_local_ports);
      ...
      tcp_helper_get_ephemeral_port_list(thc->thc_ephem_table, addr_any,
          thc->thc_ephem_table_entries, &ephemeral_ports);
    }
  ...
  {
#if CI_CFG_UL_INTERRUPT_HELPER
    efrm_eventq_register_callback(
        tcp_helper_vi(rs, intf_i), &oo_handle_wakeup_in_ul, &rs->nic[intf_i]);
#else
    if( NI_OPTS(ni).int_driven )
      efrm_eventq_register_callback(tcp_helper_vi(rs, intf_i),
          &oo_handle_wakeup_int_driven, &rs->nic[intf_i]);
    else
      efrm_eventq_register_callback(tcp_helper_vi(rs, intf_i),
          &oo_handle_wakeup_or_timeout, &rs->nic[intf_i]);
#endif
  }
#if ! CI_CFG_UL_INTERRUPT_HELPER
...
fail2:
  ...
  if( rc != -EEXIST && alloc->in_name[0] ) {
    int rc1 =
        efab_thr_table_check_name(alloc->in_name, rs->netif.cplane->cp_netns);
    if( rc1 == -EEXIST )
      rc = rc1;
  }
  ci_irqlock_unlock(&THR_TABLE.lock, &lock_flags);
...
int tcp_helper_alloc_ul(ci_resource_onload_alloc_t* alloc, int ifindices_len,
    tcp_helper_resource_t** rs_out)
{
...
static void thr_reset_stack_tx_cb_state_init(
    struct thr_reset_stack_tx_cb_state* cb_state, tcp_helper_resource_t* thr,
    int intf_i)
{
...
{
  return queue_delayed_work_on(thr->periodic_timer_cpu, thr->wq, dwork, delay);
}
#endif
...
static void set_pkt_bufset_hwaddrs(
    ci_netif* ni, int bufset_id, int intf_i, const ci_uint64* hw_addrs)
{
  int i;
  int page_order = ni->packets->set[bufset_id].page_order;
  ef_addr* addr = ni->dma_addrs + ni->packets->set[bufset_id].dma_addr_base +
...
static void tcp_helper_reset_stack_locked(tcp_helper_resource_t* thr)
{
  ...
  if( thr->thc != NULL ) {
    ...
    ci_log(
        "Stack %s:%d in cluster %s can't restore filters post-NIC-reset.\n"
  ...
  for( intf_i = 0; intf_i < CI_CFG_MAX_INTERFACES; ++intf_i ) {
    if( intfs_to_reset & (1 << intf_i) ) {
      ...
      EFRM_WARN_LIMITED("%s: reset stack %d intf %d (0x%x)", __FUNCTION__,
          thr->id, intf_i, intfs_to_reset);
      ...
      if( NI_OPTS(ni).pio && (nic->devtype.arch == EFHW_ARCH_EF10) &&
        ...
        rc = efrm_pio_realloc(
            pd, thr->nic[intf_i].thn_pio_rs, tcp_helper_vi(thr, intf_i));
        if( rc < 0 ) {
          OO_DEBUG_TCPH(
              ci_log("%s: [%d:%d] pio_realloc failed %d, "
      ...
      for( i = 0; i < pkt_sets_n; ++i ) {
        ...
        rc = oo_iobufset_resource_remap_bt(
            ni->nic_hw[intf_i].pkt_rs[i], hw_addrs);
        if( rc == -ENOSYS ) {
...
static void tcp_helper_purge_txq_work(struct work_struct* data)
{
   ...
   * lock-holder. */
  if( efab_tcp_helper_netif_lock_or_set_flags(
          trs, OO_TRUSTED_LOCK_PURGE_TXQS, CI_EPLOCK_NETIF_PURGE_TXQS, 0) ) {
...
{
  tcp_helper_resource_t* trs =
      container_of(data, tcp_helper_resource_t, reset_work);
  ...
  if( ! ci_netif_trylock(&trs->netif) ) {
    ...
    ci_log(
        "[%d]: unable to process NIC reset before destroying stack", trs->id);
    return;
  }
locked:
  ci_assert(ci_netif_is_locked(&trs->netif));
...
static bool current_is_proper_ul_context(void)
{
  return ! in_interrupt() &&
...
static void tcp_helper_rm_free(tcp_helper_resource_t* trs)
{
#if ! CI_CFG_UL_INTERRUPT_HELPER
  unsigned l, new_l;
#endif
...
static void tcp_helper_dtor_schedule(tcp_helper_resource_t* trs)
{
...
static void efab_tcp_helper_k_ref_count_is_zero(tcp_helper_resource_t* trs)
{
  ...
  OO_DEBUG_TCPH(ci_log("%s: [%u] ref " OO_THR_REF_FMT, __FUNCTION__, trs->id,
      OO_THR_REF_ARG(trs->ref)));
...
static void tcp_helper_close_cleanup_ep(
    tcp_helper_resource_t* trs, tcp_helper_endpoint_t* ep)
{
 ...
 * may not be held.
 */
static void tcp_helper_close_pending_endpoints(tcp_helper_resource_t* trs)
{
  ci_irqlock_state_t lock_flags;
  tcp_helper_endpoint_t* ep;
  ci_sllink* link;
  ...
  */
  ...
  while( ci_sllist_not_empty(&trs->ep_tobe_closed) ) {
    ...
    OO_DEBUG_TCPH(ci_log(
        "%s: [%u:%d] closing", __FUNCTION__, trs->id, OO_SP_FMT(ep->id)));
...
#if ! CI_CFG_UL_INTERRUPT_HELPER
static void efab_tcp_helper_rm_reset_untrusted(tcp_helper_resource_t* trs)
{
...
static void efab_tcp_helper_rm_schedule_free(tcp_helper_resource_t* trs)
{
...
#if CI_CFG_NIC_RESET_SUPPORT
static void efab_tcp_helper_flush_reset_wq(tcp_helper_resource_t* trs)
{
...
static void efab_tcp_helper_rm_free_locked(tcp_helper_resource_t* trs)
{
  ci_netif* netif;
#if ! CI_CFG_UL_INTERRUPT_HELPER
  ...
  ci_assert(NULL != trs);
#if ! CI_CFG_UL_INTERRUPT_HELPER
  ci_assert(trs->trusted_lock ==
            (OO_TRUSTED_LOCK_LOCKED | OO_TRUSTED_LOCK_AWAITING_FREE));
#endif
  ...
  if( ci_cas32u_succeed(
          &netif->n_ep_orphaned, OO_N_EP_ORPHANED_INIT, n_orphaned) &&
      n_orphaned == 0 )
...
ci_inline void tcp_helper_stop(tcp_helper_resource_t* trs)
{
  int intf_i;
...
void tcp_helper_dtor(tcp_helper_resource_t* trs)
{
  ...
  OO_DEBUG_TCPH(ci_log("%s [%u]: starting %s", __FUNCTION__, trs->id,
      trs->netif.flags & CI_NETIF_FLAG_WEDGED ? "wedged" : "gracious"));
  ...
  if(
    ...
    tcp_helper_free_ephemeral_ports(
        trs->trs_ephem_table, trs->trs_ephem_table_entries);
  ...
  OO_DEBUG_ERR(if( rc ) ci_log(
      "%s [%u]: failed to free instance number", __FUNCTION__, trs->id));
...
int efab_tcp_driver_ctor()
{
  ...
  efab_tcp_driver.timesync =
      vmap(&efab_tcp_driver.timesync_page, 1, VM_USERMAP, PAGE_KERNEL);
...
void efab_tcp_driver_stop(void)
{
...
void efab_tcp_driver_dtor(void)
{
  ...
  if( ci_atomic_read(&efab_tcp_driver.sendpage_pinpages_n) != 0 ) {
    ci_log("%s: ERROR: sendpage_pinpages_n is %d at destruction", __FUNCTION__,
        ci_atomic_read(&efab_tcp_driver.sendpage_pinpages_n));
  }
#endif
...
static int add_ep(
    tcp_helper_resource_t* trs, unsigned id, tcp_helper_endpoint_t* ep)
{
  ci_netif* ni = &trs->netif;
...
static int install_socks(tcp_helper_resource_t* trs, unsigned id, int num)
{
...
int efab_tcp_helper_more_socks(tcp_helper_resource_t* trs)
{
  ...
  if( ni->ep_tbl_n >= ni->ep_tbl_max )
    return -ENOSPC;

  if( ni->flags & CI_NETIF_FLAG_IN_DL_CONTEXT ) {
    ef_eplock_holder_set_flag(
        &ni->state->lock, CI_EPLOCK_NETIF_NEED_SOCK_BUFS);
    return -EBUSY;
  }
  ...
  return install_socks(
      trs, ni->ep_tbl_n, EP_BUF_PER_PAGE << OO_SHARED_BUFFER_CHUNK_ORDER);
}
...
static void efab_tcp_helper_no_more_bufs(tcp_helper_resource_t* trs)
{
  ...
  if( ++ni->state->stats.bufset_alloc_nospace == 1 )
    OO_DEBUG_ERR(
        ci_log(FN_FMT "Failed to allocate packet buffers: "
                      "no more buffer table entries. ",
            FN_PRI_ARGS(&trs->netif));
        ci_log(FN_FMT "New limits: max_packets=%d rx=%d tx=%d "
                      "rxq_limit=%d",
            FN_PRI_ARGS(ni), NI_OPTS(ni).max_packets,
            NI_OPTS(ni).max_rx_packets, NI_OPTS(ni).max_tx_packets,
            NI_OPTS(ni).rxq_limit));
}
 ...
 */
static int efab_tcp_helper_iobufset_map(tcp_helper_resource_t* trs,
    struct oo_buffer_pages* pages, int n_hw_pages,
    struct oo_iobufset** all_out, uint64_t* hw_addrs, int* page_order)
{
  ci_netif* ni = &trs->netif;
  int rc, intf_i;
  ...
  OO_STACK_FOR_EACH_INTF_I(ni, intf_i)
  {
    ...
    rc = oo_iobufset_resource_alloc(pages, pd, &iobuf,
        &hw_addrs[intf_i * n_hw_pages], intfs_suspended(trs) & (1 << intf_i),
        &cur_map_order);
    if( rc < 0 ) {
      while( --intf_i >= 0 )
...
static int efab_tcp_helper_min_nics_order(tcp_helper_resource_t* trs)
{
  int i, intf_i;
  int min_nic_order;
  ...
  OO_STACK_FOR_EACH_INTF_I(&trs->netif, intf_i)
  {
    ...
    if( NI_OPTS(&trs->netif).packet_buffer_mode == 0 ) {
      for( i = 0; i < efhw_nic_buffer_table_orders_num(nic); i++ )
        min_nic_order =
            min(min_nic_order, efhw_nic_buffer_table_orders(nic)[i]);
    } else {
      min_nic_order = 0;
    }
...
static int efab_tcp_helper_iobufset_alloc(tcp_helper_resource_t* trs,
    struct oo_iobufset** all_out, struct oo_buffer_pages** pages_out,
    uint64_t* hw_addrs, int* page_order)
{
  ...
  *pages_out = NULL;
...
#endif
  rc = oo_iobufset_pages_alloc(
      HW_PAGES_PER_SET_S, min_nics_order, &flags, &pages);
  if( rc != 0 )
    return rc;
#if CI_CFG_PKTS_AS_HUGE_PAGES
  ...
  rc = efab_tcp_helper_iobufset_map(
      trs, pages, 1 << HW_PAGES_PER_SET_S, all_out, hw_addrs, page_order);
  if( rc < 0 ) {
    ...
    struct page** pages, unsigned gup_flags)
{
  ...
  for( n = 0; n < max_pages; n += rc ) {
    rc = pin_user_pages(
        base + n * PAGE_SIZE, max_pages - n, gup_flags, pages + n, NULL);
    if( rc <= 0 ) {
 ...
 *     this fixup code. */
static long efab_get_unstraddled_user_pages(
    unsigned long base, long max_pages, struct page** pages)
{
  long rc;
#ifdef FOLL_SPLIT
  long count;
   ...
   * It is definitely needed for linux-4.18 (RHEL 8) and linux-4.19 (Debain
   * 10).
   *
...
int efab_tcp_helper_map_usermem(tcp_helper_resource_t* trs,
    struct oo_iobufs_usermem* ioum, unsigned long user_base, int n_pages,
    uint64_t** hw_addrs_out)
{
  ...
  ioum->groups =
      kmalloc_array(ioum->n_groups, sizeof(*ioum->groups), GFP_KERNEL);
  hw_addrs = kmalloc(
      sizeof(hw_addrs[0]) * n_pages * oo_stack_intf_max(ni), GFP_KERNEL);
  if( ioum->n_groups > 1 )
    tmp_hw_addrs = kmalloc(
        sizeof(hw_addrs[0]) * n_pages * oo_stack_intf_max(ni), GFP_KERNEL);
  else
  ...
  for( i = 0, group_i = 0; group_i < ioum->n_groups; ++group_i ) {
    ...
    rc = efab_tcp_helper_iobufset_map(
        trs, g->pages, group_pages, g->all, tmp_hw_addrs, NULL);
    if( rc < 0 ) {
  ...
  for( i = 0, group_i = 0; group_i < ioum->n_groups; ++group_i ) {
    ...
    if( order ) {
      ...
      while( i < i_max ) {
        efab_put_pages(pages + i + 1, CI_MIN(1 << order, i_max - i) - 1);
        i += 1 << order;
      }
    } else {
      i += group_pages;
...
void efab_tcp_helper_unmap_usermem(
    tcp_helper_resource_t* trs, struct oo_iobufs_usermem* ioum)
{
  ci_netif* ni = &trs->netif;
  int group_i;
  int intf_i;
...
int efab_tcp_helper_more_bufs(tcp_helper_resource_t* trs)
{
  ...
  hw_addrs = ci_vmalloc(
      sizeof(uint64_t) * (1 << HW_PAGES_PER_SET_S) * CI_CFG_MAX_INTERFACES);
  if( hw_addrs == NULL ) {
    ci_log("%s: [%d] out of memory", __func__, trs->id);
    return -ENOMEM;
  }

  rc = efab_tcp_helper_iobufset_alloc(
      trs, iobrs, &pages, hw_addrs, &page_order);
  if( CI_UNLIKELY(rc < 0) ) {
  ...
  OO_DEBUG_SHM({
    ...
    ci_log("[%d] allocated new bufset id %d, current=%d n_freepkts=%d",
        NI_ID(ni), bufset_id, ni->packets->id, ni->packets->n_free);
  ...
  if( page_order > CI_CFG_PKTS_PER_SET_S ) {
    /* page_order=INT_MAX means that there are no hardware interfaces
  ...
  OO_STACK_FOR_EACH_INTF_I(ni, intf_i)
  {
    set_pkt_bufset_hwaddrs(
        ni, bufset_id, intf_i, hw_addrs + intf_i * (1 << HW_PAGES_PER_SET_S));
  }
  ci_vfree(hw_addrs);
...
#if ! CI_CFG_UL_INTERRUPT_HELPER
void tcp_helper_rm_dump(oo_fd_flags fd_flags, oo_sp sock_id,
      ...
      for( i = 0; i < trs->netif.ep_tbl_n; ++i )
        if( trs->netif.ep_tbl[i] ) {
          ...
          if( s->b.state == CI_TCP_STATE_FREE || s->b.state == CI_TCP_CLOSED )
            continue;
          tcp_helper_rm_dump(s->b.state == CI_TCP_STATE_UDP ? OO_FDFLAG_EP_UDP
                                                            : OO_FDFLAG_EP_TCP,
...
static int tcp_helper_wakeup(
    tcp_helper_resource_t* trs, int intf_i, int budget)
{
  ci_netif* ni = &trs->netif;
  int n = 0, prime_async;
...
static int oo_handle_wakeup_int_driven(
    void* context, int is_timeout, struct efhw_nic* nic_, int budget)
{
  struct tcp_helper_nic* tcph_nic = context;
  tcp_helper_resource_t* trs;
  ci_netif* ni;
  ...
  if( trs->trs_aflags & OO_THR_AFLAG_POLL_AND_PRIME ) {
    ...
    if( trs->trs_aflags & OO_THR_AFLAG_POLL_AND_PRIME ||
        ! ci_bit_test_and_clear(
            &ni->state->evq_prime_deferred, tcph_nic->thn_intf_i) )
      return 0;
  ...
  while( 1 ) {
    if( ci_netif_intf_has_event(ni, tcph_nic->thn_intf_i) ) {
       ...
       */
      ci_bit_set(&ni->state->evq_prime_deferred, tcph_nic->thn_intf_i);
      if( ef_eplock_set_flag_if_locked(
              &ni->state->lock, CI_EPLOCK_NETIF_NEED_PRIME) ) {
        break;
      } else if( oo_trusted_lock_set_flags_if_locked(
                     trs, OO_TRUSTED_LOCK_NEED_PRIME) ) {
        break;
      }
      if( ! ci_bit_test_and_clear(
              &ni->state->evq_prime_deferred, tcph_nic->thn_intf_i) )
        break;
      if( tcp_helper_request_wakeup_nic_from_wakeup(
              trs, tcph_nic->thn_intf_i) == 0 )
        break;
    }
...
#if ! CI_CFG_UL_INTERRUPT_HELPER
static void linux_set_periodic_timer_restart(
    tcp_helper_resource_t* rs, unsigned long timeout)
{
...
static unsigned long tcp_helper_next_ip_timer(ci_netif* ni)
{
...
static void ci_netif_collect_periodic_metrics(ci_netif* ni)
{
...
static void linux_tcp_timer_do(
    tcp_helper_resource_t* rs, unsigned long* next_timer)
{
  ci_netif* ni = &rs->netif;
  ci_uint64 now_frc;
  int rc;
...
static void linux_tcp_helper_periodic_timer(struct work_struct* work)
{
...
static void tcp_helper_stop_periodic_work(tcp_helper_resource_t* rs)
{
  ci_irqlock_state_t lock_flags;
 ...
 */
void efab_tcp_helper_close_endpoint(
    tcp_helper_resource_t* trs, oo_sp ep_id, int already_locked)
{
  ci_netif* ni;
   ...
   */
  if( ! (current->flags & PF_EXITING) && (w->state & CI_TCP_STATE_TCP) &&
  ...
  if( ! ci_sllink_busy(&tep_p->tobe_closed) )
    ...
    ci_irqlock_unlock(&trs->lock, &lock_flags);
    ci_log("%s: [%d:%d] is already closing", __FUNCTION__, trs->id,
        OO_SP_FMT(ep_id));
    return;
  }
#else
  ...
  if( (already_locked && (~trs->netif.flags & CI_NETIF_FLAG_IN_DL_CONTEXT)) ||
      efab_tcp_helper_netif_lock_or_set_flags(trs,
          OO_TRUSTED_LOCK_CLOSE_ENDPOINT, CI_EPLOCK_NETIF_CLOSE_ENDPOINT,
          0) ) {
    OO_DEBUG_TCPH(ci_log(
        "%s: [%d:%d] closing now", __FUNCTION__, trs->id, OO_SP_FMT(ep_id)));
    tcp_helper_close_pending_endpoints(trs);
    if( ! already_locked )
      efab_tcp_helper_netif_unlock(trs, 0);
  } else {
...
int efab_attach_os_socket(tcp_helper_endpoint_t* ep, struct file* os_file)
{
  ...
  if( ! S_ISSOCK(os_file->f_path.dentry->d_inode->i_mode) ||
    ...
    fput(new_os_socket);
    OO_DEBUG_ERR(ci_log("%s: %d:%d os_file=%p is not a socket", __FUNCTION__,
        ep->thr->id, OO_SP_FMT(ep->id), os_file));
    return -EBUSY;
  }
...
int __efab_create_os_socket(tcp_helper_resource_t* trs,
    tcp_helper_endpoint_t* ep, struct file* os_file, ci_int32 domain)
{
  int rc;
  citp_waitable_obj* wo;
  ...
  if( rc < 0 ) {
    LOG_E(ci_log(
        "%s: ERROR: efab_attach_os_socket failed (%d)", __FUNCTION__, rc));
...
int efab_create_os_socket(tcp_helper_resource_t* trs,
    tcp_helper_endpoint_t* ep, ci_int32 domain, ci_int32 type, int flags)
{
  int rc;
  struct socket* sock;
  struct file* os_file;
  ...
  if( IS_ERR(os_file) ) {
    LOG_E(ci_log("%s: ERROR: sock_alloc_file failed (%ld)", __FUNCTION__,
        PTR_ERR(os_file)));
...
void tcp_helper_endpoint_wakeup(
    tcp_helper_resource_t* thr, tcp_helper_endpoint_t* ep)
{
...
static void get_os_ready_list(tcp_helper_resource_t* thr, int ready_list)
{
  ...
  while( ci_dllist_not_empty(&thr->os_ready_lists[ready_list]) ) {
    ...
    ep = CI_CONTAINER(
        tcp_helper_endpoint_t, epoll[ready_list].os_ready_link, lnk);
    ...
    oo_p_dllink_add_tail(ni,
        oo_p_dllink_ptr(ni, &ni->state->ready_lists[ready_list]), ready_link);
  }
...
#if ! CI_CFG_UL_INTERRUPT_HELPER
static void wakeup_post_poll_list(tcp_helper_resource_t* thr)
{
  ci_netif* ni = &thr->netif;
  tcp_helper_endpoint_t* ep;
  int n = ni->ep_tbl_n;
  ...
  */
  ...
  CI_READY_LIST_EACH(ni->state->ready_lists_in_use, tmp, n)
  {
    ...
    if( ! oo_p_dllink_is_empty(
            ni, oo_p_dllink_ptr(ni, &ni->state->ready_lists[n])) )
...
static inline void tcp_helper_unlock_prime(tcp_helper_resource_t* thr)
{
  ci_netif* ni = &thr->netif;
  int intf_i;
...
ci_uint64 efab_tcp_helper_netif_lock_callback(
    eplock_helper_t* epl, ci_uint64 lock_val, int in_dl_context)
{
  tcp_helper_resource_t* thr =
      CI_CONTAINER(tcp_helper_resource_t, netif.eplock_helper, epl);
  const ci_uint64 all_after_unlock_flags =
      (CI_EPLOCK_NETIF_NEED_PRIME | CI_EPLOCK_NETIF_PKT_WAKE);
  ...
  /* from dl context we can only run a subset of work, for other work we need
   * to defer. defer_flags contains all work items that will cause us to defer
   * to non-atomic */
  if( in_dl_context ) {
  ...
  again:
    ...
    lock_val = ci_netif_unlock_slow_common(
        ni, lock_val, all_handled_flags & ~defer_flags);
    ...
    */
    ...
    if( flags_set & CI_EPLOCK_NETIF_CLOSE_ENDPOINT ) {
      ...
      if( oo_trusted_lock_lock_and_set_flags(
              thr, OO_TRUSTED_LOCK_CLOSE_ENDPOINT) ) {
        /* We've got both locks.  If in non-atomic context, do the work,
         * else defer work and locks to workitem.
         */
        OO_DEBUG_TCPH(
            ci_log("%s: [%u] CLOSE_ENDPOINT now", __FUNCTION__, thr->id));
    ...
    if( flags_set & CI_EPLOCK_NETIF_NEED_WAKE ) {
      OO_DEBUG_TCPH(
          ci_log("%s: [%u] wake up endpoints", __FUNCTION__, thr->id));
    ...
    */
    if( (flags_set & CI_EPLOCK_NETIF_NEED_PKT_SET) ||
      ...
      OO_DEBUG_TCPH(
          ci_log("%s: [%u] NEED_PKT_SET now", __FUNCTION__, thr->id));
    ...
    if( (flags_set & CI_EPLOCK_NETIF_NEED_SOCK_BUFS) ||
      ...
      OO_DEBUG_TCPH(
          ci_log("%s: [%u] NEED_SOCK_BUFS now", __FUNCTION__, thr->id));
    ...
    if( flags_set & CI_EPLOCK_NETIF_KERNEL_PACKETS ) {
      OO_DEBUG_TCPH(ci_log("%s: [%u] forward %u packets to kernel",
          __FUNCTION__, thr->id, kernel_packets_pending(ni->state)));
...
int efab_eplock_wake_and_do(ci_netif* ni, ci_uint64 l)
{
...
extern int iterate_netifs_unlocked(ci_netif** p_ni,
    enum oo_thr_ref_type ref_type, enum oo_thr_ref_type ref_zero)
{
...
int efab_tcp_helper_vi_stats_query(tcp_helper_resource_t* trs,
    unsigned int intf_i, void* data, size_t data_len, int do_reset)
{
  struct efrm_vi* virs;
...
static int oo_inject_packet_kernel(ci_netif* ni, ci_ip_pkt_fmt* pkt)
{
  ...
  do {
    ...
    memcpy(skb->data + len, (void*) ((uintptr_t) (&frag->buf) + offbuf_off),
...
static void oo_inject_packets_work(struct work_struct* work)
{
  ...
  netif_is_locked = 0;
  for( pkt = PKT_CHK(ni, data->pkt_head);;
       pkt = PKT_CHK(ni, data->pkt_head) ) {
...
ci_inline int oo_xdp_rx_pkt_locked(
    ci_netif* ni, struct net_device* dev, ci_ip_pkt_fmt* pkt)
{
  ...
  struct bpf_prog* xdp_prog =
      rcu_dereference(oo_nics[ni->intf_i_to_hwport[pkt->intf_i]].prog);
  ...
  xdp->data_hard_start =
      xdp->data; /* no headroom, should be 256 bytes at least */
...
/* bool */ int efab_tcp_helper_xdp_rx_pkt(
    tcp_helper_resource_t* trs, ci_ip_pkt_fmt* pkt)
{
  int ret = XDP_PASS;
...
int efab_tcp_helper_tcp_offload_set_isn(
    tcp_helper_resource_t* trs, oo_sp ep_id, ci_uint32 isn)
{
#if CI_CFG_TCP_OFFLOAD_RECYCLER
  ci_netif* ni = &trs->netif;
  int intf_i;
  ...
  OO_STACK_FOR_EACH_INTF_I(ni, intf_i)
  {
    if( ni->nic_hw[intf_i].plugin_rx &&
      ...
      int rc = efrm_ext_msg(ni->nic_hw[intf_i].plugin_rx, XSN_CEPH_SYNC_STREAM,
          &sync, sizeof(sync));
      if( rc < 0 )
        return rc;
    }
...
int efab_tcp_helper_tcp_offload_get_stream_id(tcp_helper_resource_t* trs,
    oo_sp ep_id, ci_int32 intf_i, ci_uint32* stream_id, ci_uint64* ddr_base,
    ci_uint64* ddr_size)
{
#if CI_CFG_TCP_OFFLOAD_RECYCLER
  ci_netif* ni = &trs->netif;
...
int efab_tcp_helper_efct_superbuf_config_refresh(
    tcp_helper_resource_t* trs, oo_efct_superbuf_config_refresh_t* op)
{
  ...
  return efrm_rxq_refresh(nic->thn_efct_rxq[op->qid],
      ...
      CI_USER_PTR_GET(op->current_mappings), op->max_superbufs);
}

static tcp_helper_resource_t* thr_ref2thr(oo_thr_ref_t ref)
{
...
static void thr_release_base(oo_thr_ref_t ref)
{
  ...
  OO_DEBUG_TCPH(ci_log(
      "%s [%d] " OO_THR_REF_FMT, __func__, thr->id, OO_THR_REF_ARG(ref)));
...
oo_thr_ref_release_fn oo_thr_ref_release[OO_THR_REF_INFTY] = {
  thr_release_base, thr_release_file, thr_release_app
};
