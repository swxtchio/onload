...
{
  return oo_resource_op(
      ci_netif_get_driver_handle(ni), OO_IOC_TCP_MORE_BUFS, NULL);
}

int ci_tcp_helper_more_socks(ci_netif* ni)
{
  return oo_resource_op(
      ci_netif_get_driver_handle(ni), OO_IOC_TCP_MORE_SOCKS, NULL);
}
...
{
  return oo_resource_op(
      ci_netif_get_driver_handle(ni), OO_IOC_TCP_CLEAR_EPCACHE, NULL);
}
#endif
 ...
 *
 *--------------------------------------------------------------------*/
int ci_tcp_helper_ep_set_filters(
    ci_fd_t fd, oo_sp ep, ci_ifid_t bindto_ifindex, oo_sp from_tcp_id)
{
  ...
  if( rc < 0 )
    LOG_SV(
        ci_log("%s: failed for %d (rc=%d)", __FUNCTION__, OO_SP_FMT(ep), rc));
  return rc;
}
 ...
 *
 *--------------------------------------------------------------------*/
int ci_tcp_helper_ep_reuseport_bind(ci_fd_t fd, const char* cluster_name,
    ...
    ci_uint32 cluster_hot_restart_opt, ci_addr_t addr, ci_uint16 port_be16)
{
  ...
  op.cluster_hot_restart_opt = cluster_hot_restart_opt, op.addr = addr;
 ...
 *
 *--------------------------------------------------------------------*/
int ci_tcp_helper_ep_mcast_add_del(
    ci_fd_t fd, oo_sp ep, ci_uint32 mcast_addr, ci_ifid_t ifindex, int add)
{
  ...
  VERB(ci_log(
      "%s: id=%d %s", __FUNCTION__, OO_SP_FMT(ep), add ? "add" : "del"));
  rc = oo_resource_op(
      fd, add ? OO_IOC_EP_FILTER_MCAST_ADD : OO_IOC_EP_FILTER_MCAST_DEL, &op);

  if( rc < 0 )
    LOG_SV(ci_log("%s: %s failed for %d (rc=%d)", __FUNCTION__,
        add ? "add" : "del", OO_SP_FMT(ep), rc));
  return rc;
}


int __ci_tcp_helper_stack_attach(ci_fd_t from_fd,
    efrm_nic_set_t* out_ptr_nic_set, ci_uint32* out_map_size, bool is_service)
{
  int rc;
...
int ci_tcp_helper_stack_attach(
    ci_fd_t from_fd, efrm_nic_set_t* out_ptr_nic_set, ci_uint32* out_map_size)
{
  return __ci_tcp_helper_stack_attach(
      from_fd, out_ptr_nic_set, out_map_size, false);
}

int ci_tcp_helper_sock_attach(
    ci_fd_t stack_fd, oo_sp ep_id, int domain, int type)
{
  int rc;
  oo_sock_attach_t op;
...
int ci_tcp_helper_tcp_accept_sock_attach(
    ci_fd_t stack_fd, oo_sp ep_id, int type)
{
  int rc;
...
int ci_tcp_helper_pipe_attach(
    ci_fd_t stack_fd, oo_sp ep_id, int flags, int fds[2])
{
  int rc;
  oo_pipe_attach_t op;
    ...
    size_t addrlen, ci_uint16* out_port)
{
  ...
  LOG_U(ci_log("Bound OS sock: %d RC: %d", ntohs(*out_port), rc));
  return rc;
}
...
int ci_tcp_helper_endpoint_shutdown(
    ci_netif* ni, oo_sp sock_id, int how, ci_uint32 old_state)
{
  ...
  rc = oo_resource_op(
      ci_netif_get_driver_handle(ni), OO_IOC_TCP_ENDPOINT_SHUTDOWN, &op);
  if( rc < 0 ) {
    errno = -rc;
    return -1;
  }
...
{
  return oo_resource_op(
      ci_netif_get_driver_handle(ni), OO_IOC_TCP_CLOSE_OS_SOCK, &sock_id);
}
...
int ci_tcp_helper_os_sock_create_and_set(ci_netif* ni, ci_fd_t fd,
    ci_sock_cmn* s, int level, int optname, const void* optval, int optlen)
{
  int rc;
...
int ci_tcp_helper_alloc_active_wild(ci_netif* ni, ci_addr_t laddr)
{
  ...
  return oo_resource_op(
      ci_netif_get_driver_handle(ni), OO_IOC_ALLOC_ACTIVE_WILD, &aaw);
}
#endif
...
int ci_netif_evq_poll_k(ci_netif* ni, int _n)
{
  ...
  return oo_resource_op(
      ci_netif_get_driver_handle(ni), OO_IOC_EVQ_POLL, &intf_i);
}
#endif


int ci_tcp_helper_zc_register_buffers(
    ci_netif* ni, void* base, int num_pages, uint64_t* hw_addrs, uint64_t* id)
{
  ...
  int rc = oo_resource_op(
      ci_netif_get_driver_handle(ni), OO_IOC_ZC_REGISTER_BUFFERS, &arg);
  if( rc < 0 )
    return rc;
  *id = arg.id;
  return 0;
...
{
  return oo_resource_op(
      ci_netif_get_driver_handle(ni), OO_IOC_ZC_UNREGISTER_BUFFERS, &id);
}
