...
void ci_tcp_tx_change_mss(ci_netif *ni, ci_tcp_state *ts)
{
  ...
  LOG_U(ci_log(LNTS_FMT "%s: before=%d after=%d", LNTS_PRI_ARGS(ni, ts),
                            ...
                            ci_ip_pkt_fmt *pkt)
{
  ...
  pkt->buf_len = (oo_tx_ether_hdr_size(pkt) + CI_IPX_HDR_SIZE(af) + sizeof(ci_tcp_hdr) + optlen);
  ...
  LOG_U(log(LNTS_FMT "enqueue [" CI_TCP_FLAGS_FMT "] seq=%x",
...
int ci_tcp_retrans_one(ci_tcp_state *ts, ci_netif *netif, ci_ip_pkt_fmt *pkt)
{
  ...
  */
  if (af == AF_INET && !(tcp->tcp_flags & (CI_TCP_FLAG_SYN | CI_TCP_FLAG_FIN)))
    ci_assert_equal(CI_IPX_HDR_SIZE(af) + sizeof(*tcp) + CI_TCP_HDR_OPT_LEN(tcp) + SEQ_SUB(pkt->pf.tcp_tx.end_seq, pkt->pf.tcp_tx.start_seq),
                    oo_tx_l3_len(pkt));
  ...
  *seq_used = 0;
  ...
  while (1)
  {
    ...
    if (pkt->flags & CI_PKT_FLAG_RTQ_SACKED)
    {
      ...
      if (OO_PP_IS_NULL(ts->retrans_ptr))
        break;
      pkt = PKT_CHK(ni, ts->retrans_ptr);
    ...
    if (at_start_of_block)
      ci_tcp_retrans_coalesce_block(ni, ts, pkt);
    ...
    */
    ...
    if (SEQ_LE(ts->congrecover, pkt->pf.tcp_tx.start_seq))
      return 1;
...
void ci_tcp_tx_advance(ci_tcp_state *ts, ci_netif *ni)
{
  ...
  LOG_U(ci_log("%s: " NTS_FMT "sendq.num=%d inflight=%d", __FUNCTION__,
  ...
  {
    cwnd_right_edge = ts->snd_nxt + ts->cwnd + ts->cwnd_extra - ci_tcp_inflight(ts);
                          ...
                          unsigned right_edge, ci_uint32 *p_stop_cntr)
{
  ...
  while (1)
  {
    ...
    if (ef_vi_transmit_fill_level(&ni->ep) > NI_OPTS(ni).cong_notify_thresh && ts->congstate == CI_TCP_CONG_OPEN)
    {
    ...
    LOG_U(log(LNTS_FMT RCV_WND_FMT " snd=%08x-%08x-%08x enq=%08x",
  ...
  if (sent_num != 0)
  {
    LOG_U(log(LNT_FMT "%d packets sent in tx_advance: from %d to %d",
...
void ci_tcp_reply_with_rst(ci_netif *netif, const struct oo_sock_cplane *sock_cp,
                           ciip_tcp_rx_pkt *rxp)
{
  ...
  */
...
#if CI_CFG_IPV6
  {
    ...
    {
      TX_PKT_SET_FLOWLABEL(af, pkt, ci_make_flowlabel(netif, ipx_hdr_saddr(af, ip), tcp->tcp_source_be16, ipx_hdr_daddr(af, ip), tcp->tcp_dest_be16, IPPROTO_TCP));
    }
  }
#endif
...
int ci_tcp_send_wnd_update(ci_netif *ni, ci_tcp_state *ts, int sock_locked)
{
  ...
  if (SEQ_SUB(ts->rcv_delivered + ci_tcp_max_rcv_window(ts),
              tcp_rcv_wnd_right_edge_sent(ts)) >= ci_tcp_ack_trigger_delta(ts) &&
                      ...
                      unsigned tclass)
{
  ...
  ip6->hop_limit = (hop_limit == -1) ? CI_IPV6_DFLT_HOPLIMIT : (ci_uint8)hop_limit;
