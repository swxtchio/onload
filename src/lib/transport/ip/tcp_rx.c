                                                 ...
                                                 unsigned end_seq)
{
  ...
  if (SEQ_EQ(end_seq, tcp_seq) && rcv_nxt != rcv_rhs && SEQ_EQ(tcp_seq, rcv_rhs))
    return 0;
  return 1;
}
                                   ...
                                   ci_ip_pkt_fmt *pkt)
{
  ...
  ci_assert(ts->congstate != CI_TCP_CONG_OPEN && ts->congstate != CI_TCP_CONG_NOTIFIED);
                             ...
                             unsigned end)
{
  ...
  */
  ...
  while (1)
  {
    ...
    if (SEQ_LE(start, start_block_end->pf.tcp_tx.start_seq))
      break;
...
got_start_pkt:
  ...
  while (1)
  {
    ...
    if (SEQ_LT(end, pkt->pf.tcp_tx.end_seq))
      break;
    end_block = pkt;
    if (OO_PP_IS_NULL(end_block->pf.tcp_tx.block_end))
      break;
  ...
  */
  ...
  while (1)
  {
    ...
    if (SEQ_EQ(end, pkt->pf.tcp_tx.end_seq))
      break;
    if (OO_PP_IS_NULL(pkt->next))
      break;
  ...
  if (!(start_block->flags & CI_PKT_FLAG_RTQ_SACKED) && start_pkt_prev)
  {
    ...
    */
    ...
    while (1)
    {
      ...
      if (OO_PP_EQ(start_block->next, OO_PKT_P(start_pkt)))
        break;
                                      ...
                                      ciip_tcp_rx_pkt *rxp)
{
  ...
  while (1)
  {
    ...
    LOG_TV(log(LNT_FMT "ACKED id=%d seq=%08x-%08x [" CI_TCP_FLAGS_FMT "]"
                       " (%08x) %d",
               LNT_PRI_ARGS(netif, ts), OO_PKT_FMT(p),
...
static int ci_tcp_check_ooo_stripe(ci_netif *netif, ci_tcp_state *ts)
{
  ...
  while (1)
  {
    ...
    if (gap_found[0] && gap_found[1])
      return 1;
    if (PKT_TCP_RX_ROB(block_pkt)->next_block < 0)
      break;
                                 ...
                                 ciip_tcp_rx_pkt *rxp)
{
      ...
      block_id = PKT_TCP_RX_ROB(block_pkt)->next_block)
  {
    ...
    LOG_TV(log(LNT_FMT "OOO check: from %08x-%08x to %08x-%08x",
               LNT_PRI_ARGS(netif, ts),
               OO_PP_NOT_NULL(prev_id) ? CI_BSWAP_BE32(PKT_IPX_TCP_HDR(af, prev_pkt)->tcp_seq_be32) : 0,
               OO_PP_NOT_NULL(prev_id) ? PKT_TCP_RX_ROB(prev_pkt)->end_block_seq : 0,
               OO_PP_NOT_NULL(block_id) ? CI_BSWAP_BE32(PKT_IPX_TCP_HDR(af, block_pkt)->tcp_seq_be32) : 0,
               OO_PP_NOT_NULL(block_id) ? PKT_TCP_RX_ROB(block_pkt)->end_block_seq : 0));
  }
                                 ...
                                 ciip_tcp_rx_pkt *rxp)
{
  ...
  */
  ...
  if ((tcp->tcp_flags & ~(CI_TCP_FLAG_RST | CI_TCP_FLAG_ACK) & CI_TCP_FLAG_MASK))
  ...
  else
  {
        ...
        (sock_cp.so_bindtodevice = ci_rx_pkt_ifindex(netif, pkt)) == 0)
    {
      LOG_U(ci_log("%s: no interface matching link-local address " IPX_FMT
                   " found",
                   __FUNCTION__, IPX_ARG(AF_IP_L3(sock_cp.laddr))));
      goto freepkt_out;
    }
#endif
  ...
  */
  ...
  LOG_TC(if (tsr->amss == 0) tsr->amss = netif->state->max_mss;
         log(LNT_FMT "SYN-RECV rcv=%08x-%08x snd=%08x-%08x",
             LNT_PRI_ARGS(netif, tls),
             tsr->rcv_nxt, tsr->rcv_nxt + ci_tcp_rcvbuf2window(tls->s.so.rcvbuf, tsr->amss, tsr->rcv_wscl),
                               ...
                               ciip_tcp_rx_pkt *rxp)
{
  ...
  if (pkt->intf_i == OO_INTF_I_LOOPBACK)
  {
    ...
    if (handle_syn_sent_opts(netif, ts, rxp) < 0)
      return;
                                 ...
                                 ciip_tcp_rx_pkt *rxp)
{
  ...
  */
  ...
  {
    LOG_U(log(LPF "%d CLOSE-WAIT duplicate data " TCP_RX_FMT,
              S_FMT(ts), TCP_RX_ARGS(rxp, ts)));
    send_challenge_ack = 1;
  }
...
{
  ci_log("HANDLE FIN WAIT 1");
  if (ci_tcp_sendq_is_empty(ts) && ci_tcp_retransq_is_empty(ts))
  {
    LOG_U(log(LPF "%d FIN-WAIT1->FIN-WAIT2", S_FMT(ts)));
                                         ...
                                         ciip_tcp_rx_pkt *rxp)
{
  ...
  if (ts->tcpflags & rxp->flags & CI_TCPT_FLAG_TSO)
  {
                          ...
                          ts->tspaws, ts->tsrecent))
    {
       ...
       */
      if (SEQ_EQ(rxp->seq, ts->tslastack) && SEQ_EQ(ts->tslastack, ts->tslastseq) && SEQ_EQ(rxp->seq, pkt->pf.tcp_rx.end_seq))
        log("\tPAWS reordered tsval=0x%x tsrecent=0x%x "
            "tslastack=0x%x tslastseq=0x%x",
            rxp->timestamp,
                                     ...
                                     ciip_tcp_rx_pkt *rxp)
{
  ...
  if (ts->fast_path_check == ~CI_TCP_FAST_PATH_MASK)
    log("\tfast_disabled");
  else if ((CI_TCP_FAST_PATH_WORD(tcp) & CI_TCP_FAST_PATH_MASK) != ts->fast_path_check)
    log("\tfast_check(%x,%x)",
                                    ...
                                    ciip_tcp_rx_pkt *rxp)
{
   ...
   */
  if ((ts->tcpflags & CI_TCPT_FLAG_STRIPE) && SEQ_EQ(pkt->pf.tcp_rx.end_seq, rxp->seq) && SEQ_LE(rxp->seq, tcp_rcv_nxt(ts)))
  {
    ...
    */
        ...
        if (ci_tcp_rx_enqueue_ooo(netif, ts, rxp))
        {
          ...
          if (ts->acks_pending)
          {
            ...
            */
            ...
            if (ackpkt)
              ci_tcp_send_ack(netif, ts, ackpkt, CI_FALSE);
          }
          TCP_FORCE_ACK(ts);
        }
      }
    ...
    if (TCP_ACK_FORCED(ts))
    {
      ...
      */
      ...
      if (pkt)
        ci_tcp_send_ack_rx(netif, ts, pkt, CI_FALSE, do_update_wnd);
    }
...
unacceptable_seq:
  ...
  if (ts->s.b.state == CI_TCP_TIME_WAIT &&
      (tcp->tcp_flags & (CI_TCP_FLAG_SYN | CI_TCP_FLAG_ACK)) == CI_TCP_FLAG_SYN)
...
int ci_tcp_rx_deliver_to_conn(ci_sock_cmn *s, void *opaque_arg)
{
  ...
  not_fast = (/* flags and header length as expected, and in a state in
              ...
              ((CI_TCP_FAST_PATH_WORD(tcp) & CI_TCP_FAST_PATH_MASK) - ts->fast_path_check) |
              /* seq no. in-order? */
