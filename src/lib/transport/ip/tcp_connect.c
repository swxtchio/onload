 ...
 */
static int ci_tcp_validate_sa(
    sa_family_t domain, const struct sockaddr* sa, socklen_t sa_len)
{
  /*
  ...
  if( ! sa ) {
    LOG_U(ci_log(
        LPF "invalid sockaddr : sa = %lx, sa_len = %d", (long) sa, sa_len));
    if( sa_len == 0 )
  ...
  if( CI_SIN(sa)->sin_family != domain &&
    ...
    LOG_U(
        ci_log(LPF "address family %d does not match "
                   "with socket domain %d",
            CI_SIN(sa)->sin_family, domain));
...
static void ci_tcp_bind_check_laddr(
    ci_netif* ni, ci_sock_cmn* s, ci_addr_t addr)
{
 ...
 */
static int __ci_tcp_bind(ci_netif* ni, ci_sock_cmn* s, ci_fd_t fd,
    ci_addr_t addr, ci_uint16* port_be16, int may_defer)
{
  ...
  ci_assert(port_be16);
  ci_assert(
      s->b.state & CI_TCP_STATE_TCP || s->b.state == CI_TCP_STATE_ACTIVE_WILD);
...
#endif
    {
      ...
      if( s->domain == AF_INET )
        ...
        ci_make_sockaddr_in6_from_ip6(
            &sa_u.sin6, user_port, (ci_uint32*) addr.ip6);
      ...
      rc = efab_tcp_helper_bind_os_sock_kernel(netif2tcp_helper_resource(ni),
          SC_SP(s), &sa_u.sa, sizeof(sa_u), port_be16);
#else
      rc = ci_tcp_helper_bind_os_sock(fd, &sa_u.sa, sizeof(sa_u), port_be16);
#endif
      if( rc == 0 )
        s->s_flags &=
            ~(CI_SOCK_FLAG_CONNECT_MUST_BIND | CI_SOCK_FLAG_DEFERRED_BIND);
    }
#if CI_CFG_TCP_SHARED_LOCAL_PORTS
...
static int /*bool*/
ci_tcp_connect_check_local_dst_addr(
    ci_tcp_socket_listen* tls, ci_addr_t dst_addr)
{
...
oo_sp ci_tcp_connect_find_local_peer(
    ci_netif* ni, int locked, ci_addr_t dst_addr, int dport_be16)
{
  ...
  if( locked ) {
    ...
    sock = ci_netif_listener_lookup(ni,
        CI_IS_ADDR_IP6(dst_addr) ? AF_SPACE_FLAG_IP6 : AF_SPACE_FLAG_IP4,
        dst_addr, dport_be16);
  }
  ...
  for( i = 0; i < (int) ni->state->n_ep_bufs; ++i ) {
    ...
    if( wo->waitable.state != CI_TCP_LISTEN )
      continue;
    if( wo->waitable.sb_aflags & CI_SB_AFLAG_ORPHAN )
      continue;
    tls = SOCK_TO_TCP_LISTEN(&wo->sock);
    if( tls->s.cp.lport_be16 != dport_be16 )
      continue;
    if( ! ci_tcp_connect_check_local_dst_addr(tls, dst_addr) )
      continue;
    if( tls->s.cp.so_bindtodevice != CI_IFID_BAD )
      continue;
    goto found;
  }
  return OO_SP_NULL;
...
static void ci_tcp_init_ipcache_ip4_hdr(ci_tcp_state* ts)
{
  ...
  ts->outgoing_hdrs_len -=
      CI_IPX_HDR_SIZE(AF_INET6) - CI_IPX_HDR_SIZE(AF_INET);
...
static int ci_tcp_connect_check_dest(
    citp_socket* ep, ci_addr_t dst, int dport_be16)
{
...
#if CI_CFG_IPV6
  /*
   * Socket was bound to IPv4 and connecting to IPv6 or vice versa - hand it
   * over.
   */
  ...
  if( CI_LIKELY(ipcache->status == retrrc_success ||
    ...
    if( NI_OPTS(ep->netif).tcp_server_loopback != CITP_TCP_LOOPBACK_OFF )
      ts->local_peer = ci_tcp_connect_find_local_peer(
          ep->netif, 1 /* locked */, dst, dport_be16);
    else
...
static int /*bool*/
cicp_check_ipif_ifindex(
    struct oo_cplane_handle* cp, ci_ifid_t ifindex, void* data)
{
  return ifindex == *(ci_ifid_t*) data;
}


int ci_tcp_use_mac_filter_listen(
    ci_netif* ni, ci_sock_cmn* s, ci_ifid_t ifindex)
{
  ...
  if( ((NI_OPTS(ni).cluster_ignore == 1) ||
    ...
    if( ifindex <= 0 ) {
      ...
      return cicp_find_ifindex_by_ip(
          ni->cplane, sock_laddr(s), cicp_check_ipif_ifindex, &ifindex);
    }
...
{
  /* swxtch architectures ONLY work in UL */
  if( ni->nic_hw->vis[0].nic_type.arch == EF_VI_ARCH_SWXTCH ) {
    return 1;
  }
...
int ci_tcp_sock_set_stack_filter(ci_netif* ni, ci_sock_cmn* s)
{
  ...
  rc = ci_netif_filter_insert(ni, SC_ID(s), sock_af_space(s),
      sock_ipx_laddr(s), sock_lport_be16(s), sock_ipx_raddr(s),
...
ci_inline ci_uint32 ci_tcp_prev_seq_initial_seqno(
    ci_netif* ni, const ci_tcp_prev_seq_t* prev_seq)
{
  return ci_tcp_initial_seqno(
      ni, prev_seq->laddr, prev_seq->lport, prev_seq->raddr, prev_seq->rport);
}


ci_inline ci_uint32 ci_tcp_prev_seq_future_isn(
    ci_netif* ni, const ci_tcp_prev_seq_t* prev_seq, ci_iptime_t ticks)
{
...
ci_inline ci_uint32 ci_tcp_prev_seq_hash1(
    ci_netif* ni, const ci_tcp_prev_seq_t* prev_seq)
{
  return onload_hash1(ni->state->seq_table_entries_n - 1, prev_seq->laddr,
      prev_seq->lport, prev_seq->raddr, prev_seq->rport, IPPROTO_TCP);
}


ci_inline ci_uint32 ci_tcp_prev_seq_hash2(
    ci_netif* ni, const ci_tcp_prev_seq_t* prev_seq)
{
  return onload_hash2(prev_seq->laddr, prev_seq->lport, prev_seq->raddr,
      prev_seq->rport, IPPROTO_TCP);
}
...
ci_inline void ci_tcp_prev_seq_remember_at(ci_netif* ni,
    const ci_tcp_prev_seq_t* prev_seq_from, ci_tcp_prev_seq_t* prev_seq)
{
  ci_uint32 isn_now;
  ...
  prev_seq->expiry = ci_tcp_time_now(ni) + NI_CONF(ni).tconst_peer2msl_time;
  ...
  */
  isn_now = ci_tcp_initial_seqno(
      ni, prev_seq->laddr, prev_seq->lport, prev_seq->raddr, prev_seq->rport);
  if( SEQ_GT(prev_seq->seq_no, isn_now) ) {
    ci_uint32 isn_after_2msl = ci_tcp_future_isn(ni, prev_seq->laddr,
        prev_seq->lport, prev_seq->raddr, prev_seq->rport,
    ...
    if( SEQ_GT(isn_after_2msl, prev_seq->seq_no) ) {
      ...
      ci_iptime_t expiry_reduce =
          ci_tcp_isn2tick(ni, isn_after_2msl - prev_seq->seq_no);
...
static void __ci_tcp_prev_seq_free(ci_netif* ni,
    const ci_tcp_prev_seq_t* prev_seq_val,
...
static void ci_tcp_prev_seq_free(ci_netif* ni, ci_tcp_prev_seq_t* prev_seq)
{
...
static ci_tcp_prev_seq_t* __ci_tcp_prev_seq_lookup(
    ci_netif* ni, const ci_tcp_prev_seq_t* ts_prev_seq)
{
...
static int ci_tcp_connect_ul_start(ci_netif* ni, ci_tcp_state* ts, ci_fd_t fd,
    ci_addr_t dst, unsigned dport_be16, int* fail_rc)
{
  ci_ip_pkt_fmt* pkt;
  int rc = 0;
  ...
  if( ts->s.s_flags & CI_SOCK_FLAG_CONNECT_MUST_BIND ) {
...
#ifndef __KERNEL__
    /* In the normal case, we only install filters for IP addresses configured
    ...
    if( ! (ts->s.s_flags & CI_SOCK_FLAG_FILTER) )
      active_wild = ci_netif_active_wild_get(ni, sock_ipx_laddr(&ts->s),
          sock_ipx_raddr(&ts->s), dport_be16, &source_be16, &prev_seq);
#endif
    ...
    if( active_wild == OO_SP_NULL ) {
      ...
      if( CI_UNLIKELY(rc != 0) ) {
        LOG_U(ci_log(
            "__ci_tcp_bind returned %d at %s:%d", rc, __FILE__, __LINE__));
        *fail_rc = rc;
    ...
    /* Commit source port now.  In case of failure down the lane, an implicit
     * port might be overwritten by following attempt */
  ...
  if( CI_UNLIKELY(! pkt) ) {
    ...
    rc = ci_netif_pkt_wait(
        ni, &ts->s, CI_SLEEP_NETIF_LOCKED | CI_SLEEP_NETIF_RQ);
  ...
  rc = ci_tcp_ep_set_filters(
      ni, S_SP(ts), ts->s.cp.so_bindtodevice, active_wild);
  if( rc < 0 ) {
    ...
    if( rc != -EBUSY || ! ci_netif_timewait_try_to_free_filter(ni) ||
        (rc = ci_tcp_ep_set_filters(
             ni, S_SP(ts), ts->s.cp.so_bindtodevice, active_wild)) < 0 ) {
      ci_assert_nequal(rc, -EFILTERSSOME);
      ...
      if( added_scalable )
        ts->s.s_flags &=
            ~CI_SOCK_FLAG_SCALACTIVE; /* rollback scalactive flag */
  ...
  if( active_wild != OO_SP_NULL ) {
    ...
    ts->s.s_flags &=
        ~(CI_SOCK_FLAG_DEFERRED_BIND | CI_SOCK_FLAG_CONNECT_MUST_BIND);
  }

  LOG_TC(log(LNT_FMT "CONNECT " IPX_PORT_FMT "->" IPX_PORT_FMT,
      LNT_PRI_ARGS(ni, ts), IPX_ARG(AF_IP(ipcache_laddr(&ts->s.pkt))),
      (unsigned) CI_BSWAP_BE16(TS_IPX_TCP(ts)->tcp_source_be16),
  ...
  if( prev_seq )
    ...
    tcp_snd_nxt(ts) = ci_tcp_initial_seqno(ni, tcp_ipx_laddr(ts),
        TS_IPX_TCP(ts)->tcp_source_be16, tcp_ipx_laddr(ts),
  ...
  if( (ts->tcpflags & CI_TCPT_FLAG_WSCL) ) {
    if( NI_OPTS(ni).tcp_rcvbuf_mode == 1 )
      ts->rcv_wscl = ci_tcp_wscl_by_buff(ni, ci_tcp_max_rcvbuf(ni, ts->amss));
    else
      ts->rcv_wscl =
  ...
  TS_IPX_TCP(ts)->tcp_window_be16 =
      ci_tcp_calc_rcv_wnd_syn(ts->s.so.rcvbuf, ts->amss, ts->rcv_wscl);
  tcp_rcv_wnd_right_edge_sent(ts) =
      tcp_rcv_nxt(ts) + TS_IPX_TCP(ts)->tcp_window_be16;
  ...
  if( ! (ts->tcpflags & CI_TCPT_FLAG_TSO) )
    ts->outgoing_hdrs_len =
        CI_IPX_HDR_SIZE(ipcache_af(&ts->s.pkt)) + sizeof(ci_tcp_hdr);
...
static int ci_tcp_connect_ul_syn_sent(ci_netif* ni, ci_tcp_state* ts)
{
  ...
  if( ts->s.b.state == CI_TCP_SYN_SENT ) {
    ...
    CI_TCP_SLEEP_WHILE(ni, ts, CI_SB_FLAG_WAKE_RX, timeout,
...
#endif
  if( rc == -EAGAIN ) {
    LOG_TC(log(LNT_FMT "timeout on sleep: %d", LNT_PRI_ARGS(ni, ts), -rc));
  ...
  if( ts->s.b.state == CI_TCP_CLOSED ) {
    ...
    if( ts->s.b.sb_aflags & CI_SB_AFLAG_OS_BACKED ) {
      ...
      CI_TRY(efab_tcp_helper_set_tcp_close_os_sock(
          netif2tcp_helper_resource(ni), S_SP(ts)));
#else
...
#ifndef __KERNEL__
static int complete_deferred_bind(ci_netif* netif, ci_sock_cmn* s, ci_fd_t fd)
{
  ci_uint16 source_be16 = 0;
  int rc;
  ...
  if( CI_LIKELY(rc == 0) ) {
    ...
    LOG_TC(log(NSS_FMT "Deferred bind returned " IPX_FMT " :%u",
        NSS_PRI_ARGS(netif, s), IPX_ARG(AF_IP(addr_any)),
        ntohs(sock_lport_be16(s))));
  } else {
...
static int ci_tcp_retrieve_addr(ci_netif* netif,
    const struct sockaddr* serv_addr, ci_addr_t* dst_addr, ci_uint16* dst_port)
{
...
start_again:
    ...
    ** determining status  (e.g. non-blocking connect and connect poll)*/
    if( s->b.state & CI_TCP_STATE_SYNCHRONISED ) {
      ...
      if( serv_addr->sa_family == AF_UNSPEC )
        LOG_E(
            ci_log("Onload does not support TCP disconnect via "
  ...
  if(
      ...
      /* Check for NAT. */
      || (dnat = ci_tcp_retrieve_addr(
              ep->netif, serv_addr, &dst_addr, &dst_port)) < 0
  ...
  crc = ci_tcp_connect_ul_start(ep->netif, ts, fd, dst_addr, dst_port, &rc);
...
int ci_tcp_listen_init(ci_netif* ni, ci_tcp_socket_listen* tls)
{
   ...
   */
  LOG_EP(log("Initialise cache and pending list for id %d", S_FMT(tls)));
...
#ifdef __KERNEL__
int ci_tcp_connect_lo_samestack(
    ci_netif* ni, ci_tcp_state* ts, oo_sp tls_id, int* stack_locked)
{
  int crc, rc = 0;
  ...
  *stack_locked = 1;
  ...
  crc = ci_tcp_connect_ul_start(
      ni, ts, CI_FD_BAD, sock_ipx_raddr(&ts->s), ts->s.pkt.dport_be16, &rc);
 ...
 * exit. */
int ci_tcp_connect_lo_toconn(
    ci_netif* c_ni, oo_sp c_id, ci_addr_t dst, ci_netif* l_ni, oo_sp l_id)
{
  ci_tcp_state* ts;
  ...
  LOG_TC(log("%s: connect %d:%d to %d:%d", __FUNCTION__, c_ni->state->stack_id,
      OO_SP_TO_INT(c_id), l_ni->state->stack_id, OO_SP_TO_INT(l_id)));
  ...
  if( ! stack_locked ) {
    ...
    if( rc1 != 0 ) {
      ...
      ci_log(
          "%s([%d:%d] to [%d:%d]): leaking the shadow listener "
          "[%d:%d] rc=%d",
          __func__, c_ni->state->stack_id, OO_SP_TO_INT(c_id),
          l_ni->state->stack_id, OO_SP_TO_INT(l_id), c_ni->state->stack_id,
          tls->s.b.bufid, rc);
  ...
  ci_assert(w);
  LOG_TV(ci_log("%s: %d:%d to %d:%d shadow %d:%d accepted %d:%d", __FUNCTION__,
      c_ni->state->stack_id, OO_SP_TO_INT(c_id), l_ni->state->stack_id,
      OO_SP_TO_INT(l_id), c_ni->state->stack_id, tls->s.b.bufid,
...
int ci_tcp_reuseport_bind(ci_sock_cmn* sock, ci_fd_t fd)
{
  ...
  if( (rc = ci_tcp_ep_reuseport_bind(fd, CITP_OPTS.cluster_name,
           ...
           CITP_OPTS.cluster_hot_restart_opt, sock_ipx_laddr(sock),
    ...
    socklen_t addrlen, ci_fd_t fd)
{
  ...
  if( ((s->s_flags & CI_SOCK_FLAG_TPROXY) != 0) && (new_port == 0) ) {
    NI_LOG(ep->netif, USAGE_WARNINGS,
   ...
   *  * without SO_REUSEPORT they would end up in non-rss stack degrading
   * performance with lock contention, epoll3 and accelerated loopback */
  if( CI_IPX_IS_LOOPBACK(addr) &&
      NI_OPTS(ep->netif).scalable_filter_enable !=
  ...
  if( ((s->s_flags & CI_SOCK_FLAG_TPROXY) != 0) && CI_IPX_ADDR_IS_ANY(addr) ) {
    NI_LOG(ep->netif, USAGE_WARNINGS,
        "Sockets with IP_TRANSPARENT set must "
  ...
  CI_LOGLEVEL_TRY_RET(
      LOG_TV, __ci_tcp_bind(ep->netif, ep->s, fd, addr, &new_port, 1));
...
handover:
  if( ! (ep->s->b.sb_aflags & CI_SB_AFLAG_OS_BACKED) ) {
    rc =
        ci_tcp_helper_os_sock_create_and_set(ep->netif, fd, s, -1, 0, NULL, 0);
    if( rc < 0 )
...
int ci_tcp_listen(citp_socket* ep, ci_fd_t fd, int backlog)
{
  ...
  */
  ...
  if( s->s_flags & CI_SOCK_FLAG_BOUND_ALIEN ) {
    ...
    will_accelerate =
        (netif->state->flags & CI_NETIF_FLAG_USE_ALIEN_LADDRS || scalable) &&
        ! CI_IPX_IS_LOOPBACK(s->laddr);
  }
  ...
  if( will_accelerate ) {
    ...
    rc = ci_tcp_ep_set_filters(
        netif, S_SP(tls), tls->s.cp.so_bindtodevice, OO_SP_NULL);
    if( rc == -EFILTERSSOME ) {
  ...
  if( (s->s_flags & CI_SOCK_FLAG_SCALPASSIVE) == 0 ||
      NI_OPTS(netif).scalable_listen !=
          CITP_SCALABLE_LISTEN_ACCELERATED_ONLY ) {
#ifdef __ci_driver__
    rc = efab_tcp_helper_listen_os_sock(
        netif2tcp_helper_resource(netif), S_SP(tls), backlog);
#else
...
static int ci_tcp_shutdown_listen(citp_socket* ep, int how, ci_fd_t fd)
{
  ...
  citp_waitable_wake_not_in_poll(
      ep->netif, &tls->s.b, CI_SB_FLAG_WAKE_RX | CI_SB_FLAG_WAKE_TX);
  ci_netif_unlock(ep->netif);
...
int ci_tcp_getsockname(
    citp_socket* ep, ci_fd_t fd, struct sockaddr* sa, socklen_t* p_sa_len)
{
  ci_sock_cmn* s = ep->s;
  int rc = 0;
