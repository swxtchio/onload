                                     ...
                                     ci_addr_t dst_addr, int dport_be16)
{
  ...
  if (locked)
  {
    ...
    sock = ci_netif_listener_lookup(ni, CI_IS_ADDR_IP6(dst_addr) ? AF_SPACE_FLAG_IP6 : AF_SPACE_FLAG_IP4,
                                    dst_addr, dport_be16);
  }
...
int ci_tcp_use_mac_filter_listen(ci_netif *ni, ci_sock_cmn *s, ci_ifid_t ifindex)
{
  int mode;
...
int ci_tcp_can_set_filter_in_ul(ci_netif *ni, ci_sock_cmn *s)
{
  ...
  {
    LOG_E(log("NOT SCALABLE"));
    return 0;
  }
  if (s->b.state == CI_TCP_LISTEN)
  {
    LOG_E(log("TCP LISTEN"));
    return 0;
  }
  ...
  {
    LOG_E(log("SCALPASSIVE and ACCELERATED ONLY"));
    return 0;
  }
...
ci_tcp_clock_isn_safe(ci_netif *ni, const ci_tcp_prev_seq_t *prev_seq)
{
  ...
  ci_uint32 isn_after_2msl = ci_tcp_prev_seq_future_isn(ni, prev_seq, NI_CONF(ni).tconst_peer2msl_time);
                                           ...
                                           ci_tcp_prev_seq_t *prev_seq)
{
  ...
  */
  ...
  {
    ci_uint32 isn_after_2msl = ci_tcp_future_isn(ni, prev_seq->laddr, prev_seq->lport,
                                                 prev_seq->raddr, prev_seq->rport,
                                                 NI_CONF(ni).tconst_peer2msl_time);
    if (SEQ_GT(isn_after_2msl, prev_seq->seq_no))
    {
      ...
      ci_iptime_t expiry_reduce = ci_tcp_isn2tick(ni, isn_after_2msl - prev_seq->seq_no);
...
int ci_tcp_listen_init(ci_netif *ni, ci_tcp_socket_listen *tls)
{
  ...
  tls->epcache.avail_stack = oo_ptr_to_statep(ni, &ni->state->passive_cache_avail_stack);
                          ...
                          socklen_t *namelen)
{
  ...
  ci_uint16 port = dnat ? ts->pre_nat.dport_be16 : TS_IPX_TCP(ts)->tcp_dest_be16;
