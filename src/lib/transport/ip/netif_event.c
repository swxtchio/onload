...
{
  return (pkt->rx_flags & CI_PKT_RX_FLAG_RX_SHARED) ? CI_TP_LOG_NR : CI_TP_LOG_U;
}
...
int ci_netif_evq_poll(ci_netif *ni, int intf_i)
{
  ef_vi *evq = ci_netif_vi(ni, intf_i);
  int n_evs;
  size_t evs_per_poll = ef_vi_flags(evq) & EF_VI_RX_EVENT_MERGE ? NI_OPTS(ni).evs_per_poll / 4 : NI_OPTS(ni).evs_per_poll;
#if CI_CFG_WANT_BPF_NATIVE && CI_HAVE_BPF_NATIVE
  ...
  if (CI_LIKELY(status & FUTURE_IP4))
  {
    ...
    */
    ...
    else
    {
      ...
      LOG_E(log(LPF "HANDLE RX POST FUTURE PASSING TO KERNEL"));
                                        ...
                                        ci_ip_pkt_fmt *pkt)
{
  ...
  pkt->n_buffers = 1;
  while (1)
  { /* reverse the chain of fragments */
    ...
    for (i = 0; i < n_evs; ++i)
    {
      ...
      else if (EF_EVENT_TYPE(ev[i]) == EF_EVENT_TYPE_RX_MULTI)
      {
        ...
        for (j = 0; j < n_ids; ++j)
        {
          ...
          ci_assert_equal(pkt->intf_i, intf_i);
          __handle_rx_pkt(ni, ps, &s.rx_pkt);
          if ((ev[i].rx_multi.flags & (EF_EVENT_FLAG_SOP | EF_EVENT_FLAG_CONT)) == EF_EVENT_FLAG_SOP)
          {
            /* Whole packet in a single buffer. */
            uint16_t len;
...
int ci_netif_poll_intf_future(ci_netif *ni, int intf_i, ci_uint64 start_frc)
{
  ...
  if (EF_EVENT_TYPE(ev[0]) == EF_EVENT_TYPE_RX)
  {
    ...
    if ((ev[0].rx.flags & (EF_EVENT_FLAG_SOP | EF_EVENT_FLAG_CONT)) == EF_EVENT_FLAG_SOP)
    {
