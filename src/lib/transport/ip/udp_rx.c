...
int ci_udp_rx_deliver(ci_sock_cmn *s, void *opaque_arg)
{
  ...
  LOG_E(log("%s: " NS_FMT "pay_len=%d " CI_IP_PRINTF_FORMAT " -> " CI_IP_PRINTF_FORMAT, __FUNCTION__,
                      ...
                      int ip_paylen)
{
  ...
  LOG_E(log(LPF "handle_rx: UDP:%p IP:%p", udp, oo_ipx_hdr(pkt)));
  ...
  {
    int oo_vi_flags =
        (0 <= pkt->intf_i && pkt->intf_i < oo_stack_intf_max(ni)) ? ni->state->nic[pkt->intf_i].oo_vi_flags : 0;
    ...
    LOG_E(log(LPF "handle_rx passing to kernel"));
