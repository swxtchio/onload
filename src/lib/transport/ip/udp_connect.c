...
static int ci_udp_sys_getsockname(ci_fd_t sock, citp_socket *ep)
{
  ...
  )
  {
    LOG_UV(log("%s: OS sock domain %d != expected domain %d or "
               ...
               salen,
               (int)(ep->s->domain == AF_INET ? sizeof(struct sockaddr_in) : sizeof(struct sockaddr_in6))));
    return -1;
  }
...
static int ci_udp_set_filters(citp_socket *ep, ci_udp_state *us)
{
  ...
  if (rc < 0)
  {
    LOG_E(log(FNS_FMT "ci_tcp_ep_set_filters failed (%d)",
                                  ...
                                  ci_uint16 lport)
{
  ...
  {
    LOG_E(log(FNS_FMT "HANDOVER (%d <= %d <= %d)",
                         ...
                         socklen_t addrlen, ci_uint16 lport)
{
  ...
  LOG_E(log("%s: CONCLUDING", __FUNCTION__));
  ...
  {
    LOG_E(log("%s: CI AF INET6", __FUNCTION__));
    goto handover;
  }
#endif
  ...
  {
    LOG_E(log("%s: CI UDP SHOULD HANDOVER", __FUNCTION__));
    goto handover;
  }
  ...
  {
    LOG_E(log("%s SOCKET ERROR", __FUNCTION__));
      ...
      !CI_IPX_IS_MULTICAST(udp_ipx_laddr(us)))
  {
    ...
    LOG_E(log("%s: NOT SURE TBH", __FUNCTION__));
    goto handover;
  }
...
handover:
  LOG_E(log("%s: " SK_FMT " HANDOVER", __FUNCTION__, SK_PRI_ARGS(ep)));
      ...
      ((struct sockaddr_in *)sa)->sin_port != 0)
  {
                        ...
                        (ci_dllist *)(ci_uintptr_t)CITP_OPTS.udp_reuseports)
    {
      if (force_reuseport->port == ((struct sockaddr_in *)sa)->sin_port)
      {
        ...
        LOG_E(log("%s " SF_FMT ", applied legacy SO_REUSEPORT flag for port %u",
...
{
  CHECK_UEP(ep);
  LOG_E(log("%s(" SF_FMT ", addrlen=%d)", __FUNCTION__,
                            ...
                            socklen_t addrlen, ci_fd_t os_sock)
{
  ...
  {
    LOG_E(log(FNT_FMT "HANDOVER (udp_connect_handover == 2)" IPX_PORT_FMT,
  ...
  {
    LOG_E(log(FNT_FMT "HANDOVER not IPv4", FNT_PRI_ARGS(ep->netif, us)));
    goto handover;
  }
#endif
  ...
  default:
    ...
    {
      LOG_E(log(FNT_FMT "HANDOVER " IPX_PORT_FMT, FNT_PRI_ARGS(ep->netif, us),
  ...
  {
    LOG_E(log(FNT_FMT IPX_PORT_FMT " - route via OS socket",
              FNT_PRI_ARGS(ep->netif, us), IPX_ARG(AF_IP(dst)),
              CI_BSWAP_BE16(serv_sin->sin_port)));
    ci_udp_clr_filters(ep);
    return 0;
  ...
  LOG_E(log(LPF "connect: " SF_FMT " %sCONNECTED L:" IPX_PORT_FMT
...
void ci_udp_all_fds_gone(ci_netif *netif, oo_sp sock_id, int do_free)
{
  ...
  LOG_E(ci_log("ci_udp_all_fds_gone: " NTS_FMT,
...
#ifdef __KERNEL__
  ci_assert_equal(ci_netif_get_valid_ep(netif, sock_id)->oofilter.sf_local_port, NULL);
#endif
