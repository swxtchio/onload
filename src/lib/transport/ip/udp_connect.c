...
static int ci_udp_set_filters(citp_socket* ep, ci_udp_state* us)
{
  ...
  LOG_UC(log(FNS_FMT "Setting UDP filters", FNS_PRI_ARGS(ep->netif, ep->s)));
    ...
    citp_socket* ep, ci_udp_state* us, ci_fd_t os_sock)
{
  ...
  if( (rc = ci_udp_set_filters(ep, us)) != 0 )
    ...
    LOG_UC(log(FNS_FMT "ERROR: ci_udp_set_filters failed (%d)",
        FNS_PRI_ARGS(ep->netif, ep->s), errno));
    ...
    const struct sockaddr* serv_addr, socklen_t addrlen, ci_fd_t os_sock)
{
  ...
  LOG_UC(log(FNT_FMT, FNT_PRI_ARGS(ep->netif, us)));
