                                     ...
                                     struct msghdr *msg, int *bytes)
{
  ...
  do
  {
    ...
    zc_msg.iov[i].iov_len = msg->msg_iov[i].iov_len > bytes_remaining ? bytes_remaining : msg->msg_iov[i].iov_len;
  ...
  rc = (*(onload_zc_recv_filter_callback)((ci_uintptr_t)us->recv_q_filter))(&zc_msg, (void *)((ci_uintptr_t)us->recv_q_filter_arg), cb_flags);
  ...
  if (CI_LIKELY(rc >= 0))
  {
    ...
    if (!(rinf->flags & MSG_PEEK))
    {
      ...
      if (us->recv_q_filter)
      {
        ...
        cb_flags = CI_IP_IS_MULTICAST(oo_ip_hdr(pkt)->ip_daddr_be32) ? ONLOAD_ZC_MSG_SHARED : 0;
        filterrc =
            (*(onload_zc_recv_filter_callback)((ci_uintptr_t)us->recv_q_filter))(&zc_msg, (void *)((ci_uintptr_t)us->recv_q_filter_arg), cb_flags);
