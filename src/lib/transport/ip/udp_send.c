...
#if CI_CFG_IPV6
#define msg_namelen_ok(af, namelen) ((af) == AF_INET6 ? (namelen) >= sizeof(struct sockaddr_in6) : (namelen) >= sizeof(struct sockaddr_in))
#else
...
#define MAX_IP_FRAGMENTS 50
  ...
  while (OO_PP_NOT_NULL(p->next))
  {
    ...
    if (first_frag)
    {
      ...
      frag_len = (WITH_CI_CFG_IPV6(IS_AF_INET6(af) ? CI_BSWAP_BE16(first_hdr->ip6.payload_len) - sizeof(ci_ip6_frag_hdr) :)
    ...
    else
    {
      ...
      frag_len = WITH_CI_CFG_IPV6(IS_AF_INET6(af) ? CI_BSWAP_BE16(p_ipx->ip6.payload_len) - sizeof(ci_ip6_frag_hdr) :)
  ...
  udp->udp_check_be16 = WITH_CI_CFG_IPV6(IS_AF_INET6(af) ? ci_ip6_udp_checksum(&first_hdr->ip6, udp, iov, n + 1) :)
                                         ...
                                         const struct msghdr *msg, int flags)
{
  ...
  if (~ni->state->flags & CI_NETIF_FLAG_USE_ALIEN_LADDRS &&
      lport != 0 && (CI_IPX_ADDR_IS_ANY(laddr) || cicp_user_addr_is_local_efab(ni, laddr)))
  {
                   ...
                   const ci_msghdr *msg, int flags)
{
  ...
  {
    LOG_U(ci_log("%s: MSG_MORE not yet supported", __FUNCTION__));
...
no_error:
...
#endif
  {
    ...
    else
    {
          ...
          us->s.pkt.status != retrrc_nomac)
      {
        ...
        LOG_U(ci_log("%s RETRC_NOMAC %d", __FUNCTION__, us->s.pkt.status));
        goto send_via_os;
      }
    }
  ...
  else
  {
    ...
    {
      LOG_U(ci_log("%s UDP NAME NOK", __FUNCTION__));
      goto send_via_os;
    }
    ...
    {
      LOG_U(ci_log("%s IPX ADDR IS ANY", __FUNCTION__));
      goto send_via_os;
    }
        ...
        oo_cp_ipcache_is_valid(ni, &us->ephemeral_pkt))
    {
          ...
          us->ephemeral_pkt.status != retrrc_nomac)
      {
        ...
        LOG_U(ci_log("%s EPHERMERAL FAILURE", __FUNCTION__));
        goto send_via_os;
      }
    ...
    else if (si_trylock_and_inc(ni, &sinf, us->stats.n_tx_lock_cp))
    {
      ...
      if (us->ephemeral_pkt.status != retrrc_success &&
          us->ephemeral_pkt.status != retrrc_nomac)
      {
        LOG_U(ci_log("%s EPHERMERAL FAILURE 2", __FUNCTION__));
        goto send_via_os;
      }
      sinf.ipcache.mtu = us->ephemeral_pkt.mtu;
    }
...
so_error:
  LOG_U(ci_log("%s SEND ERROR", __FUNCTION__));
...
send_via_os:
  LOG_U(ci_log("%s SEND VIA OS", __FUNCTION__));
  if (sinf.stack_locked)
    ci_netif_unlock(ni);
