...
int cicp_user_build_fwd_key(ci_netif *ni, const ci_ip_cached_hdrs *ipcache,
...
void cicp_user_retrieve(ci_netif *ni,
                        ...
                        const struct oo_sock_cplane *sock_cp)
{
  ...
  ci_ipcache_set_saddr(ipcache,
                       nat_applied ? CI_ADDR_FROM_ADDR_SH(pre_nat_laddr) : CI_ADDR_FROM_ADDR_SH(data.base.src));
  ...
  ipcache->status = (data.flags & CICP_FWD_DATA_FLAG_ARP_VALID) ? retrrc_success : retrrc_nomac;
  memcpy(ci_ip_cache_ether_shost(ipcache), &data.src_mac, ETH_ALEN);
  if (data.flags & CICP_FWD_DATA_FLAG_ARP_VALID)
  {
    ci_log("setting dst mac");
    memcpy(ci_ip_cache_ether_dhost(ipcache), &data.dst_mac, ETH_ALEN);
  }

  if (CI_IPX_IS_MULTICAST(daddr))
  {
    ipcache_ttl(ipcache) = IS_AF_INET6(af) ? CI_IPV6_DFLT_MCASTHOPS : sock_cp->ip_mcast_ttl;
  }
  else
  {
...
void cicp_ip_cache_update_from(ci_netif *ni, ci_ip_cached_hdrs *ipcache,
...
int cicp_ipif_check_ok(struct oo_cplane_handle *cp,
                       ci_ifid_t ifindex, void *data)
{
  return 1;
}

int cicp_llap_check_onloaded(struct oo_cplane_handle *cp,
...
int oo_deferred_send_one(ci_netif *ni, struct oo_deferred_pkt *dpkt)
{
  ...
  if (!CICP_MAC_ROWID_IS_VALID(dpkt->ver.id))
  {
    ...
    ci_log("mac row is is not valid");
  ...
  if (data.flags & CICP_FWD_DATA_FLAG_ARP_FAILED)
  {
    if (dpkt->flag & OO_DEFERRED_FLAG_FIRST)
    {
       ...
       * timeout. */
      ci_log("something about arp failing");
      dpkt->flag &= ~OO_DEFERRED_FLAG_FIRST;
      return 0;
    }
    oo_deferred_arp_failed(ni,
                           dpkt->flag & OO_DEFERRED_FLAG_IS_IPV6 ? AF_INET6 : AF_INET,
                           pkt);
    return 1;
  }
  if (!(data.flags & CICP_FWD_DATA_FLAG_ARP_VALID))
  {
    ci_log("arp not valid");
    dpkt->flag &= ~OO_DEFERRED_FLAG_FIRST;
    return 0;
  }
...
int oo_deferred_send(ci_netif *ni)
{
  ...
  oo_p_dllink_for_each_safe(ni, l, tmp, deferred_list)
  {
                ...
                dpkt->ts + NI_CONF(ni).tconst_defer_arp))
    {
      ...
      oo_p_dllink_add(ni, oo_p_dllink_ptr(ni, &ni->state->deferred_list_free), l);
    }
    else
    {
      ret = 0;
