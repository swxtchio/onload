...
ci_inline void set_entry_state(
    ci_netif_filter_table_entry_fast* entry, ci_uint32 state)
{
  entry->__id_and_state = __ID(entry) | state;
}

#if OO_DO_STACK_POLL
ci_inline void set_entry_id(
    ci_netif_filter_table_entry_fast* entry, ci_uint32 id)
{
...
static int ci_ip4_netif_filter_lookup(ci_netif* netif, unsigned laddr,
    unsigned lport, unsigned raddr, unsigned rport, unsigned protocol)
{
  ...
  hash1 = __onload_hash1(
      tbl->table_size_mask, laddr, lport, raddr, rport, protocol);
  first = hash1;

  LOG_NT(log("tbl_lookup: %s %s:%u->%s:%u hash=%u:%u at=%u",
      CI_IP_PROTOCOL_STR(protocol), ip_addr_str(laddr),
      (unsigned) CI_BSWAP_BE16(lport), ip_addr_str(raddr),
      (unsigned) CI_BSWAP_BE16(rport), first,
      __onload_hash2(laddr, lport, raddr, rport, protocol), hash1));
  ...
  while( 1 ) {
    ...
    if( CI_LIKELY(OCCUPIED(entry)) ) {
      ...
      if( ((laddr - entry->laddr) | (lport - entry_ext->lport) |
              (raddr - sock_raddr_be32(s)) | (rport - sock_rport_be16(s)) |
    ...
    if( hash1 == first ) {
      LOG_E(ci_log(FN_FMT "ERROR: LOOP %s:%u->%s:%u hash=%u:%u",
          FN_PRI_ARGS(netif), ip_addr_str(laddr), lport, ip_addr_str(raddr),
          rport, hash1, hash2));
      return -ELOOP;
    }
  }
...
oo_sp ci_netif_filter_lookup(ci_netif* netif, int af_space, ci_addr_t laddr,
    unsigned lport, ci_addr_t raddr, unsigned rport, unsigned protocol)
{
  int rc = -ENOENT;

#if CI_CFG_IPV6
  if( IS_AF_SPACE_IP6(af_space) ) {
    rc = ci_ip6_netif_filter_lookup(
        netif, laddr, lport, raddr, rport, protocol);
    if( rc >= 0 )
  ...
  if( IS_AF_SPACE_IP4(af_space) )
    ...
    rc = ci_ip4_netif_filter_lookup(
        netif, laddr.ip4, lport, raddr.ip4, rport, protocol);
  if( rc >= 0 )
    return CI_NETIF_FILTER_ID_TO_SOCK_ID(netif, rc);
  return OO_SP_NULL;
}

int ci_netif_listener_lookup(
    ci_netif* netif, int af_space, ci_addr_t laddr, unsigned lport)
{
  oo_sp sock = ci_netif_filter_lookup(
      netif, af_space, laddr, lport, addr_any, 0, IPPROTO_TCP);
  if( OO_SP_IS_NULL(sock) )
    sock = ci_netif_filter_lookup(
        netif, af_space, addr_any, lport, addr_any, 0, IPPROTO_TCP);
  return sock;
}


ci_uint32 ci_netif_filter_hash(ci_netif* ni, ci_addr_t laddr, unsigned lport,
...
handle_entry(ci_netif* ni, ci_netif_filter_table_entry_fast* entry,
    ci_netif_filter_table_entry_ext* entry_ext, unsigned laddr, unsigned lport,
    unsigned raddr, unsigned rport, unsigned protocol, int intf_i, int vlan,
    ...
    int /*bool*/ check_lport)
{
  ...
  LOG_NV(ci_log("%s match=%d: %s %s:%u->%s:%u hash=%u:%u at=%u check_lport=%d",
      __FUNCTION__, is_match, CI_IP_PROTOCOL_STR(protocol), ip_addr_str(laddr),
      (unsigned) CI_BSWAP_BE16(lport), ip_addr_str(raddr),
      (unsigned) CI_BSWAP_BE16(rport),
      __onload_hash1(ni->filter_table->table_size_mask, laddr, lport, raddr,
          rport, protocol),
    ...
    void* callback_arg, ci_uint32* hash_out)
{
  ...
  hash1 =
      __onload_hash1(table_size_mask, laddr, lport, raddr, rport, protocol);
  first = hash1;

  LOG_NV(log("%s: %s %s:%u->%s:%u hash=%u:%u at=%u", __FUNCTION__,
      CI_IP_PROTOCOL_STR(protocol), ip_addr_str(laddr),
      (unsigned) CI_BSWAP_BE16(lport), ip_addr_str(raddr),
      (unsigned) CI_BSWAP_BE16(rport), first,
      __onload_hash2(laddr, lport, raddr, rport, protocol), hash1));
  ...
  if( STATE(entry) == OCCUPIED_PREFERRED ) {
    ...
    if( handle_entry(ni, entry, &ni->filter_table_ext[hash1], laddr, lport,
            raddr, rport, protocol, intf_i, vlan, callback, callback_arg,
            0 /*check_lport*/) )
      return 1;
  }
...
static int ci_ip4_netif_filter_insert(ci_netif_filter_table* tbl,
    ci_netif* netif, oo_sp tcp_id, unsigned laddr, unsigned lport,
    unsigned raddr, unsigned rport, unsigned protocol)
{
    ...
    if( hash1 == first ) {
      ...
      if( ! (s->s_flags & CI_SOCK_FLAG_SW_FILTER_FULL) ) {
        LOG_E(ci_log(FN_FMT "%d FULL %s %s:%u->%s:%u hops=%u",
            FN_PRI_ARGS(netif), OO_SP_FMT(tcp_id),
            CI_IP_PROTOCOL_STR(protocol), ip_addr_str(laddr),
            (unsigned) CI_BSWAP_BE16(lport), ip_addr_str(raddr),
            (unsigned) CI_BSWAP_BE16(rport), hops));
  ...
  LOG_TC(
      ci_log(FN_FMT "%d INSERT %s %s:%u->%s:%u hash=%u:%u at=%u "
                    "over=%u:%u hops=%u",
          FN_PRI_ARGS(netif), OO_SP_FMT(tcp_id), CI_IP_PROTOCOL_STR(protocol),
          ip_addr_str(laddr), (unsigned) CI_BSWAP_BE16(lport),
          ip_addr_str(raddr), (unsigned) CI_BSWAP_BE16(rport), first, hash2,
          hash1, STATE(entry), __ID(entry), hops));
  ...
  set_entry_state(
      entry, hash1 == first ? OCCUPIED_PREFERRED : OCCUPIED_REHASHED);
...
static void __ci_ip4_netif_filter_remove(ci_netif_filter_table* tbl,
    ci_netif* ni, unsigned hash1, unsigned hash2, int hops,
    unsigned last_tbl_i)
{
  ci_netif_filter_table_entry_fast* entry;
  ci_netif_filter_table_entry_ext* entry_ext;
...
static void ci_ip4_netif_filter_remove(ci_netif_filter_table* tbl,
    ci_netif* netif, oo_sp sock_p, unsigned laddr, unsigned lport,
    unsigned raddr, unsigned rport, unsigned protocol)
{
  ci_netif_filter_table_entry_fast* entry;
  ...
  LOG_NV(ci_log("%s: [%d:%d] REMOVE %s %s:%u->%s:%u hash=%u:%u", __FUNCTION__,
      NI_ID(netif), OO_SP_FMT(sock_p), CI_IP_PROTOCOL_STR(protocol),
      ip_addr_str(laddr), (unsigned) CI_BSWAP_BE16(lport), ip_addr_str(raddr),
      (unsigned) CI_BSWAP_BE16(rport), hash1, hash2));
  ...
  while( 1 ) {
    ...
    if( tbl_i == first ) {
      LOG_E(ci_log(FN_FMT "ERROR: LOOP [%d] %s %s:%u->%s:%u",
          FN_PRI_ARGS(netif), OO_SP_FMT(sock_p), CI_IP_PROTOCOL_STR(protocol),
          ip_addr_str(laddr), (unsigned) CI_BSWAP_BE16(lport),
...
int ci_netif_filter_insert(ci_netif* netif, oo_sp tcp_id, int af_space,
    ...
    unsigned rport, unsigned protocol)
{
  ...
  if( IS_AF_SPACE_IP6(af_space) ) {
    ...
    rc = ci_ip6_netif_filter_insert(ip6_tbl, netif, tcp_id, laddr, lport,
        CI_IPX_ADDR_IS_ANY(raddr) ? addr_any : raddr, rport, protocol);
    if( rc < 0 )
      return rc;
  }
...
#endif
  {
    ...
    rc = ci_ip4_netif_filter_insert(
        ip4_tbl, netif, tcp_id, laddr.ip4, lport, raddr.ip4, rport, protocol);
...
void ci_netif_filter_remove(ci_netif* netif, oo_sp sock_p, int af_space,
    const ci_addr_t laddr, unsigned lport, const ci_addr_t raddr,
    unsigned rport, unsigned protocol)
{
  ci_netif_filter_table* ip4_tbl;
...
#endif
  {
    ...
    ci_ip4_netif_filter_remove(
        ip4_tbl, netif, sock_p, laddr.ip4, lport, raddr.ip4, rport, protocol);
  }
}
#endif
...
#if OO_DO_STACK_POLL
int __ci_ip4_netif_filter_lookup(ci_netif* netif, unsigned laddr,
    unsigned lport, unsigned raddr, unsigned rport, unsigned protocol)
{
  int rc;
  ...
  LOG_NV(log(LPF "FULL LOOKUP %s:%u->%s:%u rc=%d", ip_addr_str(laddr),
      (unsigned) CI_BSWAP_BE16(lport), ip_addr_str(raddr),
      (unsigned) CI_BSWAP_BE16(rport), rc));
  ...
  raddr = rport = 0;
  rc = ci_ip4_netif_filter_lookup(netif, laddr, lport, raddr, rport, protocol);
  LOG_NV(log(LPF "WILD LOOKUP %s:%u->%s:%u rc=%d", ip_addr_str(laddr),
      (unsigned) CI_BSWAP_BE16(lport), ip_addr_str(raddr),
      (unsigned) CI_BSWAP_BE16(rport), rc));
...
ci_sock_cmn* __ci_netif_filter_lookup(ci_netif* netif, int af_space,
...
#if CI_CFG_IPV6
  if( IS_AF_SPACE_IP6(af_space) ) {
    rc = __ci_ip6_netif_filter_lookup(
        netif, laddr, lport, raddr, rport, protocol);
    if( CI_LIKELY(rc >= 0) )
...
#endif
  {
    rc = __ci_ip4_netif_filter_lookup(
        netif, laddr.ip4, lport, raddr.ip4, rport, protocol);
    if( CI_LIKELY(rc >= 0) )
...
void ci_netif_filter_dump(ci_netif* ni)
{
  ...
  for( i = 0; i <= tbl->table_size_mask; ++i ) {
    ...
    if( CI_LIKELY(OCCUPIED(entry)) ) {
      ...
      unsigned hash1 = __onload_hash1(
          tbl->table_size_mask, laddr, lport, raddr, rport, protocol);
      unsigned hash2 = __onload_hash2(laddr, lport, raddr, rport, protocol);
      log("%010d state=%u id=%-10d rt_ct=%d %s " CI_IP_PRINTF_FORMAT
          ":%d " CI_IP_PRINTF_FORMAT ":%d %010d:%010d",
