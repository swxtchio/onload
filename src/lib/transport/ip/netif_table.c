...
oo_sp ci_netif_filter_lookup(ci_netif *netif, int af_space,
...
int ci_netif_filter_for_each_match(ci_netif *ni,
                                   ...
                                   void *callback_arg, ci_uint32 *hash_out)
{
  ...
  LOG_E(log("%s: %s %s:%u->%s:%u hash=%u:%u at=%u",
                           ...
                           unsigned protocol)
{
  ...
  LOG_E(ci_log("%s: %s:%u|%u->%s:%u|%u",
               __FUNCTION__,
               ip_addr_str(laddr.ip4), (unsigned)CI_BSWAP_BE16(lport), lport, ip_addr_str(raddr.ip4), (unsigned)CI_BSWAP_BE16(rport), rport));
...
#if OO_DO_STACK_POLL
int __ci_ip4_netif_filter_lookup(ci_netif *netif,
                                 unsigned laddr, unsigned lport,
                                 unsigned raddr, unsigned rport,
                                 unsigned protocol)
{
...
void ci_netif_filter_dump(ci_netif *ni)
{
  ...
  for (i = 0; i <= tbl->table_size_mask; ++i)
  {
    ...
    if (CI_LIKELY(OCCUPIED(entry)))
    {
      ...
      log("%010d state=%u id=%-10d rt_ct=%d %s " CI_IP_PRINTF_FORMAT ":%d " CI_IP_PRINTF_FORMAT ":%d %010d:%010d",
