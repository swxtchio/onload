...
const char *citp_waitable_type_str(citp_waitable *w)
{
  if (w->state & CI_TCP_STATE_TCP)
    ...
    return "UDP";
  else if (w->state == CI_TCP_STATE_FREE)
    return "FREE";
  else if (w->state == CI_TCP_STATE_PIPE)
    return "PIPE";
  else if (w->state == CI_TCP_STATE_AUXBUF)
    return "AUXBUFS";
  else if (w->state == CI_TCP_STATE_ACTIVE_WILD)
    return "ACTIVE_WILD";
  else
    return "<unknown-citp_waitable-type>";
}
