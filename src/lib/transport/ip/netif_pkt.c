...
ci_ip_pkt_fmt* __ci_netif_pkt(ci_netif* ni, unsigned id)
{
...
#endif
  {
    rc = oo_resource_mmap(ci_netif_get_driver_handle(ni), OO_MMAP_TYPE_NETIF,
        CI_NETIF_MMAP_ID_PKTSET(setid), CI_CFG_PKT_BUF_SIZE * PKTS_PER_SET,
        OO_MMAP_FLAG_POPULATE, &p);
    if( rc < 0 ) {
      ci_log("%s: oo_resource_mmap for pkt set %d failed (%d)", __FUNCTION__,
          setid, rc);
      goto out;
    }
  }
...
again:
  ...
  if( bufset_id != -1 ) {
    ci_netif_pkt_set_change(
        ni, bufset_id, ci_netif_pkt_set_is_underfilled(ni, bufset_id));
...
ci_inline void __ci_dbg_poison_header(ci_ip_pkt_fmt* pkt, ci_uint32 pattern)
{
  ...
  ci_uint32 len =
      (ETH_HLEN + ETH_VLAN_HLEN + 2) + sizeof(ci_ip4_hdr) + sizeof(ci_tcp_hdr);
  for( i = 0; i < len / 4; i++ )
    pkt_u32[i] = patn_u32;
}
  ...
  while( 1 ) {
    ...
    if( val & CI_EPLOCK_NETIF_MERGE_ATOMIC_COUNTERS )
      break;
                 ...
                 &ni->state->lock, CI_EPLOCK_NETIF_MERGE_ATOMIC_COUNTERS) ) {
      break;
    ...
    if( iter-- == 0 ) {
      ci_log(
          "%s: [%d] failed to set MERGE_ATOMIC_COUNTERS flag, "
...
void ci_netif_pkt_free(
    ci_netif* ni, ci_ip_pkt_fmt* pkt CI_KERNEL_ARG(int* p_netif_is_locked))
{
...
#ifdef __KERNEL__
    ci_netif_pkt_release_mnl(
        ni, PKT_CHK(ni, pkt->frag_next), p_netif_is_locked);
#else
...
#if defined(__KERNEL__) && OO_DO_STACK_POLL
  if( CI_UNLIKELY(
          (! *p_netif_is_locked) && (~pkt->flags & CI_PKT_FLAG_NONB_POOL)) ) {
  ...
  if( pkt->flags & CI_PKT_FLAG_NONB_POOL ) {
    ci_log("putting this on the nob list");
...
#if OO_DO_STACK_POLL
int ci_netif_pkt_try_to_free(
    ci_netif* ni, int desperation, int stop_once_freed_n)
{
  unsigned id;
  int freed = 0;
   ...
   */
  ci_assert(
      &ni->state->stats.pkt_scramble2 - &ni->state->stats.pkt_scramble0 ==
...
int ci_netif_pkt_alloc_block(ci_netif* ni, ci_sock_cmn* s, int* p_netif_locked,
    int can_block, ci_ip_pkt_fmt** p_pkt)
{
