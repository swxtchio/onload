...
{
  if( ci_is_multithreaded() )
    return oo_atomic_dec_and_test(a);
  else
    return --a->n == 0;
}
...
typedef struct {
  ...
  int (*accept)(citp_fdinfo*, struct sockaddr*, socklen_t*, int flags,
      citp_lib_context_t*);
  int (*connect)(
      citp_fdinfo*, const struct sockaddr*, socklen_t, citp_lib_context_t*);
  ...
  int (*recvmmsg)(
      citp_fdinfo*, struct mmsghdr*, unsigned, int, ci_recvmmsg_timespec*);
  ...
  int (*select)(citp_fdinfo*, int*, int, int, int, struct oo_ul_select_state*);
  ...
  int (*zc_recv_filter)(
      citp_fdinfo*, onload_zc_recv_filter_callback, void*, int);
  ...
  int (*tmpl_update)(citp_fdinfo*, struct oo_msg_template*,
      const struct onload_template_msg_update_iovec*, int, unsigned);
...
#endif
  enum onload_delegated_send_rc (*dsend_prepare)(citp_fdinfo*, int size,
      unsigned flags, struct onload_delegated_send* out);
  int (*dsend_complete)(
      citp_fdinfo*, const ci_iovec* iov, int iovlen, int flags);
...
extern void citp_protocol_manager_add(
    citp_protocol_impl*, int is_stream) CI_HF;

extern int citp_protocol_manager_create_socket(
    int dom, int type, int proto) CI_HF;

extern int dpdk_init(void);
extern int dpdk_cleanup(void);
 ...
 * lock IS held.
 */
ci_inline void citp_fdinfo_release_ref(citp_fdinfo* fdinfo, int fdt_locked)
{
  /* If we're releasing a reference from one of our "magic" fd's, then we
   * should never drop the ref-count below 1,000,000
   */
  ci_assert(
      ((fdinfo != &citp_the_closed_fd) && (fdinfo != &citp_the_reserved_fd)) ||
...
extern int             citp_timespec_compare(
                const struct timespec* a, const struct timespec* b) CI_HF;

extern int citp_oo_timespec_compare(
    const struct oo_timespec* a, const struct timespec* b) CI_HF;
...
#define fdip_to_waiter(fdip) \
  ((citp_fdtable_waiter*) (ci_uintptr_t) ((fdip) & ~(fdip_busy)))

#define waiter_to_fdip(w) ((citp_fdinfo_p) (ci_uintptr_t) (w) | fdip_busy)

extern void __citp_fdtable_busy_clear_slow(
    unsigned fd, citp_fdinfo_p, int fdt_locked) CI_HF;

ci_inline void citp_fdtable_busy_clear(
    unsigned fd, citp_fdinfo_p fdip, int fdt_locked)
{
...
#define __CITP_LOCK(l)          \
  do {                          \
    if( ci_is_multithreaded() ) \
      oo_rwlock_lock_write(l);  \
  } while( 0 )

#define __CITP_LOCK_RD(l)       \
  do {                          \
    if( ci_is_multithreaded() ) \
      oo_rwlock_lock_read(l);   \
  } while( 0 )

#define __CITP_UNLOCK(l)         \
  do {                           \
    if( ci_is_multithreaded() )  \
      oo_rwlock_unlock_write(l); \
  } while( 0 )

#define __CITP_UNLOCK_RD(l)     \
  do {                          \
    if( ci_is_multithreaded() ) \
...
ci_inline void _CITP_FDTABLE_ASSERT_LOCKED(
    int fdt_locked, char* file, int line)
{
  citp_signal_info* si = &oo_per_thread_get()->sig;
  if( ! fdt_locked )
    return;
 ...
 */
extern citp_fdinfo* citp_fdtable_lookup_fast(
    citp_lib_context_t*, unsigned fd) CI_HF;

extern citp_fdinfo* citp_fdtable_lookup_noprobe(
    unsigned fd, int fdt_locked) CI_HF;

extern int citp_reprobe_moved_common(citp_fdinfo* fdinfo, int from_fast_lookup,
    int fdip_is_already_busy, citp_fdinfo** fdinfo_out);

ci_inline citp_fdinfo* citp_reprobe_moved(
    citp_fdinfo* fdinfo, int from_fast_lookup, int fdip_is_already_busy)
{
  citp_fdinfo* new_fdinfo;
  citp_reprobe_moved_common(
      fdinfo, from_fast_lookup, fdip_is_already_busy, &new_fdinfo);
  return new_fdinfo;
}
...
extern citp_fdinfo_p citp_fdtable_new_fd_set(
    unsigned fd, citp_fdinfo_p, int fdt_locked) CI_HF;
extern void citp_fdtable_insert(
    citp_fdinfo*, unsigned fd, int fdt_locked) CI_HF;
...
extern int citp_passthrough_bind(
    citp_fdinfo* fdi, const struct sockaddr* sa, socklen_t sa_len);
extern int citp_passthrough_accept(citp_fdinfo* fdi, struct sockaddr* sa,
    socklen_t* p_sa_len, int flags, citp_lib_context_t* lib_context);
extern int citp_passthrough_connect(citp_fdinfo* fdi,
...
ci_inline int __citp_checked_enter_lib(
    citp_lib_context_t* lib_context CI_DEBUG_ARG(const char* fn)
        CI_DEBUG_ARG(int line))
{
  ...
  Log_LIB(log("  citp_checked_enter_lib(%p) [was_in=%d] %s (%d)",
      lib_context->thread, lib_context->thread->sig.c.inside_lib > 0, fn,
      line));
  ci_assert(
      ~lib_context->thread->sig.c.aflags & OO_SIGNAL_FLAG_FDTABLE_LOCKED);
  ++lib_context->thread->sig.c.inside_lib;
  return (lib_context->thread->sig.c.inside_lib == 1);
}


ci_inline void __citp_enter_lib(citp_lib_context_t* lib_context CI_DEBUG_ARG(
    const char* fn) CI_DEBUG_ARG(int line))
{
  lib_context->saved_errno = errno;
  lib_context->thread      = __oo_per_thread_get();
  Log_LIB(log("  citp_enter_lib(%p) inside_lib=%d %s (%d)",
      lib_context->thread, lib_context->thread->sig.c.inside_lib, fn, line));
...
ci_inline void __citp_reenter_lib(citp_lib_context_t* lib_context CI_DEBUG_ARG(
    const char* fn) CI_DEBUG_ARG(int line))
{
...
ci_inline void __citp_exit_lib(citp_lib_context_t* lib_context,
    int do_errno CI_DEBUG_ARG(const char* fn) CI_DEBUG_ARG(int line))
{
  Log_LIB(log("  citp_exit_lib(%p) inside_lib=%d %s (%d)", lib_context->thread,
      lib_context->thread->sig.c.inside_lib, fn, line));
  ...
  if( CI_UNLIKELY(
          ...
          (lib_context->thread->sig.c.aflags & OO_SIGNAL_FLAG_HAVE_PENDING)) )
...
#define citp_checked_enter_lib(c) \
  __citp_checked_enter_lib(       \
      (c) CI_DEBUG_ARG(__FUNCTION__) CI_DEBUG_ARG(__LINE__))
...
extern void   citp_environ_handle_args(
      char** argv, const char* arg, va_list args, char*** env_ptr) CI_HF;
extern char* const* citp_environ_check_preload(
    char* const* env, size_t* bytes_reqd) CI_HF;
extern void citp_environ_make_preload(
    char* const* env, char** new_env, size_t new_env_bytes) CI_HF;
...
extern int citp_sock_fcntl_os_sock(citp_sock_fdi* epi, int fd, int cmd,
    long arg, const char* cmd_str, int* fcntl_result) CI_HF;
...
int citp_ul_do_poll(struct pollfd* __restrict__ fds, nfds_t nfds,
    ci_uint64 timeout_ms, ci_uint64* used_ms, citp_lib_context_t* lib_context,
...
static inline int citp_ul_pwait_spin_pre(citp_lib_context_t* lib_context,
...
static inline void citp_ul_pwait_spin_done(
    citp_lib_context_t* lib_context, sigset_t* sigsaved, int* p_rc)
{
...
static inline int __citp_poll_if_needed(
    ci_netif* ni, ci_uint64 recent_frc, int is_spinning)
{
 ...
 */
static inline int citp_poll_if_needed(
    ci_netif* ni, ci_uint64 recent_frc, int is_spinning)
{
 ...
 * Utils
 */
ci_inline void ms2timespec(
    ci_uint64 timeout, ci_uint64 spent, struct timespec* tv)
{
  if( timeout > spent ) {
