...
static int citp_udp_socket(int domain, int type, int protocol)
{
  ...
  if (rc != 0)
  {
    Log_U(log(LPF "NON ZERO"));
    if (rc == CI_SOCKET_HANDOVER)
    {
      Log_U(log(LPF "ORDERLY_HANDOVER"));
  ...
  Log_U(log(LPF "socket(%d, %d, %d) = " EF_FMT, domain, type, protocol,
            EF_PRI_ARGS(epi, fd)));
  return fd;
...
static int citp_udp_shutdown(citp_fdinfo *fdinfo, int how)
{
  ...
  Log_U(ci_log("%s(" EF_FMT ", %d)", __FUNCTION__, EF_PRI_ARGS(epi, fdinfo->fd), how));
                                ...
                                struct sockaddr *sa, socklen_t *p_sa_len)
{
  ...
  Log_U(log("%s(" EF_FMT ")", __FUNCTION__, EF_PRI_ARGS(epi, fdinfo->fd)));
                               ...
                               int optname, void *optval, socklen_t *optlen)
{
  ...
  Log_U(log("%s(" EF_FMT ", %d, %d)", __FUNCTION__, EF_PRI_ARGS(epi, fdinfo->fd),
            level, optname));
                             ...
                             ci_recvmmsg_timespec *timeout)
{
  ...
  Log_U(log(LPF "recvmmsg(%d, msg, %u, %#x)", fdinfo->fd, vlen,
            (unsigned)flags));
...
static int citp_udp_recv(citp_fdinfo *fdinfo, struct msghdr *msg, int flags)
{
  ...
  Log_U(log(LPF "recv(%d, msg, %#x)", fdinfo->fd, (unsigned)flags));
                         ...
                         int flags)
{
  ...
  {
    Log_U(log(LPF "EFAULT (%d, msg, %#x)", fdinfo->fd, (unsigned)flags));
    rc = -1;
    errno = EFAULT;
  }
  return rc;
                             ...
                             unsigned vlen, int flags)
{
  ...
  Log_U(log(LPF "sendmmsg(%d, msg, %u, %#x)", fdinfo->fd, vlen,
            (unsigned)flags));
...
{
  Log_U(log(LPF "listen: not supported by dg protocol"));
  citp_fdinfo_release_ref(fdinfo, 0);
...
static int citp_udp_accept(citp_fdinfo *fdinfo,
                           ...
                           citp_lib_context_t *lib_context)
{
  Log_U(log(LPF "accept: not supported by dg protocol"));
  RET_WITH_ERRNO(EOPNOTSUPP);
}
