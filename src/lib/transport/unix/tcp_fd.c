                                      ...
                                      int domain, int type, int flags)
{
  ...
  if ((from_cache == 0) || (!has_fd || !pid_matches))
  {
    ...
    oo_sp sp = S_SP(ts);
    fd = (listener != NULL) ? ci_tcp_helper_tcp_accept_sock_attach(stack_fd, sp, flags) : ci_tcp_helper_sock_attach(stack_fd, sp, domain, type);
    if (fd < 0)
    {
      if (fdtable_strict())
        CITP_FDTABLE_UNLOCK();
...
tcp_rc_means_handover(int rc)
{
  ...
  return rc == CI_SOCKET_HANDOVER || ((rc < 0) && CITP_OPTS.no_fail &&
                                 ...
                                 int flags, citp_waitable *w)
{
  ...
  if (rc != 0)
  {
    ...
    ts->s.domain = AF_INET;
    tcp_laddr_be32(ts) = sock_laddr_be32(&listener->s) == INADDR_ANY ? INADDR_LOOPBACK : sock_laddr_be32(&listener->s);
                                  ...
                                  int active)
{
  ...
  ci_atomic32_or(&s->b.sb_aflags, CI_SB_AFLAG_NOT_READY | CI_SB_AFLAG_IN_CACHE | (active ? 0 : CI_SB_AFLAG_IN_PASSIVE_CACHE));
                           ...
                           struct oo_ul_select_state *__restrict__ ss)
{
  ...
  {
    ci_tcp_state *ts = SOCK_TO_TCP(s);
    if (rd && (ci_tcp_recv_not_blocked(ts) || ci_tcp_poll_timestamp_q_nonempty(ni, ts)))
    {
      FD_SET(fdi->fd, ss->rdu);
      ++*n;
    }
    if (wr && (ci_tcp_tx_advertise_space(ni, ts) || ci_tcp_poll_timestamp_q_nonempty(ni, ts)))
    {
