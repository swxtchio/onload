...
int onload_zc_alloc_buffers(int fd, struct onload_zc_iovec* iovecs,
    int iovecs_len, enum onload_zc_buffer_type_flags flags)
{
  int rc = 0, i;
  citp_lib_context_t lib_context;
  citp_fdinfo* fdi;
  ...
  Log_CALL(ci_log(
      "%s(%d, %p, %d, %x)", __FUNCTION__, fd, iovecs, iovecs_len, flags));
    ...
    for( i = 0; i < iovecs_len; ++i ) {
      ...
      if( flags & ONLOAD_ZC_BUFFER_HDR_TCP ) {
        if( ts != NULL ) {
          ...
          iovecs[i].iov_base =
              ((char*) oo_tx_ip_hdr(pkt)) + ts->outgoing_hdrs_len;
      ...
      iovecs[i].iov_len =
          CI_CFG_PKT_BUF_SIZE - ((char*) iovecs[i].iov_base - (char*) pkt);
...
int onload_zc_query_rx_memregs(
    int fd, struct onload_zc_iovec* iov, int* iovecs_len, int flags)
{
  int rc = 0;
  unsigned i;
  citp_lib_context_t lib_context;
  ...
  Log_CALL(
      ci_log("%s(%d, %p, %p, %d)", __FUNCTION__, fd, iov, iovecs_len, flags));
...
int onload_zc_recv(int fd, struct onload_zc_recv_args* args)
{
  ...
  Log_CALL(
      ci_log("%s(%d, %p(flags=%x))", __FUNCTION__, fd, args, args->flags));
...
int onload_set_recv_filter(
    int fd, onload_zc_recv_filter_callback filter, void* cb_arg, int flags)
{
  int rc;
  citp_lib_context_t lib_context;
  citp_fdinfo* fdi;
  ...
  Log_CALL(
      ci_log("%s(%d, %p, %p, %x)", __FUNCTION__, fd, filter, cb_arg, flags));
...
static bool have_unsupported_nic(ci_netif* ni)
{
  ...
  if( ci_netif_vi(ni, nic_i)->nic_type.arch == EF_VI_ARCH_AF_XDP ||
      ci_netif_vi(ni, nic_i)->nic_type.arch == EF_VI_ARCH_SWXTCH ||
...
int onload_zc_register_buffers(int fd, ef_addrspace addr_space,
    uint64_t base_ptr, uint64_t len, int flags, onload_zc_handle* handle)
{
  int rc;
  citp_lib_context_t lib_context;
  citp_fdinfo* fdi;
  ...
  if( flags )
    ...
    struct ci_zc_usermem* um =
        malloc(sizeof(struct ci_zc_usermem) +
               sizeof(um->hw_addrs[0]) * num_pages * oo_stack_intf_max(ni));
    if( ! um )
      rc = -ENOMEM;
    else if( addr_space != EF_ADDRSPACE_LOCAL &&
      ...
      if( addr_space == EF_ADDRSPACE_LOCAL )
        rc =
            ci_tcp_helper_zc_register_buffers(ni, (void*) (uintptr_t) base_ptr,
                num_pages, um->hw_addrs, &um->kernel_id);
