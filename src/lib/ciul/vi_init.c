...
int ef_vi_rx_ring_bytes(struct ef_vi* vi)
{
  ...
  return ef_vi_calc_rxq_descriptors_bytes(
      vi->nic_type.arch, vi->vi_rxq.mask + 1);
}
...
int ef_vi_init(struct ef_vi* vi, int arch, int variant, int revision,
    unsigned ef_vi_flags, unsigned char nic_flags, ef_vi_state* state)
{
  ...
  switch( arch ) {
    ...
    case EF_VI_ARCH_AF_XDP:
#ifndef __KERNEL__
      /* In the control plane we still need the xdp implementation for
       * consistency even though we disable it almost entirely*/
      efswxtch_vi_init(vi);
#else
      efxdp_vi_init(vi);
#endif
...
void ef_vi_init_txq(
    struct ef_vi* vi, int ring_size, void* descriptors, void* ids)
{
...
static char* ef_vi_sfc_init_qs(struct ef_vi* vi, char* q_mem, uint32_t* ids,
    int evq_size, int rxq_size, int rx_prefix_len, int txq_size)
{
  if( evq_size ) {
...
char* ef_vi_init_qs(struct ef_vi* vi, char* q_mem, uint32_t* ids, int evq_size,
    int rxq_size, int rx_prefix_len, int txq_size)
{
  if( vi->nic_type.arch == EF_VI_ARCH_AF_XDP ||
      vi->nic_type.arch == EF_VI_ARCH_SWXTCH )
    return ef_vi_xdp_init_qs(
        vi, q_mem, ids, rxq_size, rx_prefix_len, txq_size);
  else
    return ef_vi_sfc_init_qs(
        vi, q_mem, ids, evq_size, rxq_size, rx_prefix_len, txq_size);
}
...
void ef_vi_init_rx_timestamping(struct ef_vi* vi, int rx_ts_correction)
{
  ...
  if( vi->ts_format == TS_FORMAT_SECONDS_QTR_NANOSECONDS ) {
    ...
    if( vi->rx_ts_correction == 0 ) {
       ...
       */
      LOG(
          ef_log("%s: ERROR: NIC returned zero timestamp correction. "
...
void ef_vi_reset_evq(struct ef_vi* vi, int clear_ring)
{
  ...
  /* Set unsol_seq to default, but leave 1 credit-space in reserve for overflow
   * event. */
...
int ef_eventq_capacity(ef_vi* vi)
{
  ...
  return vi->evq_mask / EF_VI_EV_SIZE - 1u +
         vi->ep_state->evq.evq_clear_stride;
}
