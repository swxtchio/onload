...
#define EF_VI_BUILD_ASSERT(e)                                           \
  {                                                                     \
    __attribute__((__unused__)) typedef char __EF_VI_BUILD_ASSERT_NAME( \
        __LINE__)[(e) ? 1 : -1];                                        \
  }


#ifdef NDEBUG
#define EF_VI_ASSERT(x) \
  do {                  \
  } while( 0 )
#ifdef __KERNEL__
#define EF_VI_BUG_ON(x) WARN_ON(x)
#else
#define EF_VI_BUG_ON(x) \
  do {                  \
  } while( 0 )
#endif
#else
...
#define __EFVI_MASKFIELD32(LBN, WIDTH) \
  ((uint32_t) (__EFVI_MASK32(WIDTH) << (LBN)))

/* constructors for fields which span the first and second dwords */
#define __LW(LBN) (32 - LBN)
#define LOW(v, LBN, WIDTH) \
  ((uint32_t) (((v) &__EFVI_MASK64(__LW((LBN)))) << (LBN)))
#define HIGH(v, LBN, WIDTH) \
  ((uint32_t) (((v) >> __LW((LBN))) & __EFVI_MASK64((WIDTH - __LW((LBN))))))
...
#define LOW2(v, LBN, WIDTH) \
  ((uint32_t) (((v) &__EFVI_MASK64(__LW2((LBN)))) << ((LBN) -32)))
#define HIGH2(v, LBN, WIDTH) \
  ((uint32_t) (((v) >> __LW2((LBN))) & __EFVI_MASK64((WIDTH - __LW2((LBN))))))
...
#define LOW3(v, LBN, WIDTH) \
  ((uint32_t) (((v) &__EFVI_MASK64(__LW3((LBN)))) << ((LBN) -64)))
#define HIGH3(v, LBN, WIDTH) \
  ((unit32_t) (((v) >> __LW3((LBN))) & __EFVI_MASK64((WIDTH - __LW3((LBN))))))
...
#define DWCHCK(LBN, WIDTH) \
  EF_VI_BUG_ON(((LBN) < 0) || (((LBN) + (WIDTH)) > 32))
...
#define QWORD_GET_U(field, val)                  ((unsigned) CI_QWORD_FIELD(val, field))

#define QWORD_TEST_BIT(field, val)               (! ! CI_QWORD_FIELD(val, field))
...
extern void efswxtch_vi_init(ef_vi*) EF_VI_HF;
...
extern int      __ef_vi_capabilities_get(ef_driver_handle handle, int ifindex,
         int pd_id, ef_driver_handle pd_dh, enum ef_vi_capability cap,
         unsigned long* value);
extern int      ef_pd_capabilities_get(ef_driver_handle handle, ef_pd* pd,
         ef_driver_handle pd_dh, enum ef_vi_capability cap, unsigned long* value);
