...
#define ci_ss_assertfl2(ni, e, x, y, file, line)                                             \
  do                                                                                         \
  {                                                                                          \
    if (CI_UNLIKELY(!(e)))                                                                   \
    {                                                                                        \
      ...
      LOG_SSA(ci_log("ci_ss_assert(%s)\nwhere [%s=%" CI_PRIx64 "] "                          \
                     "[%s=%" CI_PRIx64 "]\nat %s:%d\nfrom %s:%d",                            \
                     #e, #x, (ci_uint64)(ci_uintptr_t)(x), #y, (ci_uint64)(ci_uintptr_t)(y), \
...
#if defined(__KERNEL__) && !defined(NDEBUG)
#define ci_ss_assertfl(ni, x, file, line)                          \
  do                                                               \
  {                                                                \
    if (CI_UNLIKELY(!(x)))                                         \
    {                                                              \
...
#if CI_CFG_IPV6
#define oo_tx_ipx_hdr(af, pkt) ((af == AF_INET6) ? (ci_ipx_hdr_t *)oo_tx_ip6_hdr(pkt) : (ci_ipx_hdr_t *)oo_tx_ip_hdr(pkt))
#define oo_tx_ipx_data(af, pkt) ((af == AF_INET6) ? oo_tx_ip6_data(pkt) : oo_tx_ip_data(pkt))
#else
ci_inline ci_ipx_hdr_t *oo_tx_ipx_hdr(int af, ci_ip_pkt_fmt *pkt)
{
  (void)af;
  return (ci_ipx_hdr_t *)oo_tx_ip_hdr(pkt);
}
ci_inline void *oo_tx_ipx_data(int af, ci_ip_pkt_fmt *pkt)
{
  (void)af;
  return oo_tx_ip_data(pkt);
}
#endif
