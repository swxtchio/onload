...
#define NI_PKT_SET(ni)                                                                      \
  ((ni)->packets->id < 0 ? 0 : (ni)->packets->id >= (ni)->pkt_sets_n ? (ni)->pkt_sets_n - 1 \
                                                                     : (ni)->packets->id)
#else
...
extern int ci_netif_rx_dpdk_init(ci_netif *netif) CI_HF;
#ifdef __KERNEL__
...
ci_inline void ci_pkt_init_from_ipcache_len(ci_ip_pkt_fmt *pkt,
...
ci_inline void ci_pkt_init_from_ipcache(ci_ip_pkt_fmt *pkt,
                                        const ci_ip_cached_hdrs *ipcache)
{
...
#define CI_TCP_TIME_WAIT (0x9000 | CI_TCP_STATE_TCP | CI_TCP_STATE_TCP_CONN | CI_TCP_STATE_SYNCHRONISED | CI_TCP_STATE_SLOW_PATH | CI_TCP_STATE_NOT_CONNECTED | CI_TCP_STATE_RECVD_FIN /* 2MSL timer doesn't count */ \
...
#if CI_CFG_IPV6
#define S_IPX_TCP_HDR(s) ((ipcache_is_ipv6(&(s)->pkt)) ? S_IP6_TCP_HDR(s) : S_TCP_HDR(s))
#define TS_IPX_TCP(ts) ((ipcache_is_ipv6(&(ts)->s.pkt)) ? TS_IP6_TCP(ts) : TS_TCP(ts))
#else
...
#if CI_CFG_IPV6
#define ipcache_ttl(ipcache) (*(ipcache_is_ipv6(ipcache) ? &(ipcache)->ipx.ip6.hop_limit : &(ipcache)->ipx.ip4.ip_ttl))
#else
...
#define ipcache_lport_be16(ipcache) \
  ((ipcache_is_ipv6(ipcache) ? ((ci_uint16 *)(&(ipcache)->ipx.ip6 + 1)) : ((ci_uint16 *)(&(ipcache)->ipx.ip4 + 1)))[0])
#define ipcache_rport_be16(ipcache) \
  ((ipcache_is_ipv6(ipcache) ? ((ci_uint16 *)(&(ipcache)->ipx.ip6 + 1)) : ((ci_uint16 *)(&(ipcache)->ipx.ip4 + 1)))[1])
#else
...
*/
extern int __ci_copy_iovec_to_pkt(ci_netif *, ci_ip_pkt_fmt *, ci_iovec_ptr *CI_KERNEL_ARG(ci_addr_spc_t)) CI_HF;
...
{
  return ((ni->packets->sets_max - ni->packets->sets_n) << CI_CFG_PKTS_PER_SET_S) +
         ni->packets->n_free;
}
...
{
  return ni->packets->n_pkts_allocated - ni->state->n_rx_pkts - ni->packets->n_free;
}
...
{
  int reserved_bufs = ts->s.b.state != CI_TCP_ESTABLISHED ? 0 : NI_OPTS(netif).endpoint_packet_reserve;
                                           ...
                                           ci_tcp_state *ts)
{
  ...
  if (!ci_ip_timer_pending(netif, &ts->delack_tid))
    ci_ip_timer_set(netif, &ts->delack_tid, ci_tcp_time_now(netif) + NI_CONF(netif).tconst_delack);
}
...
ci_inline
    ci_iptime_t
    ci_tcp_isn2tick(ci_netif *ni, ci_uint32 isn)
{
  ci_uint64 ticks = isn;
