...
#define CHECK_NI(ni) ci_netif_assert_valid(ni, __FILE__, __LINE__)
...
#define CHECK_TEP(ep) ci_tcp_ep_assert_valid(ep, __FILE__, __LINE__)

#define CHECK_US(ni, ts) \
  ci_udp_state_assert_valid((ni), (ts), __FILE__, __LINE__)

#define CHECK_UEP(ep) ci_udp_ep_assert_valid(ep, __FILE__, __LINE__)

#define CHECK_TIMERS(ni) \
  ci_ip_timer_state_assert_valid((ni), __FILE__, __LINE__)

#define CHECK_FREEPKTS(ni) ci_netif_verify_freepkts(ni, __FILE__, __LINE__);
...
extern void ci_pmtu_state_init(ci_netif* ni, ci_sock_cmn* s, oo_p pmtu_sp,
...
extern void ci_pmtu_update_fast(ci_netif* ni, ci_pmtu_state_t* pmtus,
    ci_ip_cached_hdrs* ipcache, unsigned mtu) CI_HF;
 ...
 */
ci_inline void ci_ip_cache_invalidate(ci_ip_cached_hdrs* ipcache)
{
...
static inline int oo_cp_ipcache_is_valid(
    ci_netif* ni, ci_ip_cached_hdrs* ipcache)
{
  int rc = oo_cp_verinfo_is_valid(
      ni->cplane, &ipcache->fwd_ver, ci_ni_fwd_table_id(ni));
  if( rc && ipcache->fwd_ver_init_net.id != CICP_MAC_ROWID_UNUSED ) {
    rc = ni->cplane_init_net != NULL &&
         oo_cp_verinfo_is_valid(ni->cplane_init_net,
             &ipcache->fwd_ver_init_net, ci_ni_fwd_table_id(ni));
  }
  return rc;
}
...
#define PKT_TCP_RX_BUF_ASSERT_VALID(ni, pkt) \
  OO_OFFBUF_ASSERT_VALID(                    \
      &(pkt)->buf, PKT_START(pkt), (pkt) + CI_CFG_PKT_BUF_SIZE)
...
static inline ci_udp_hdr* ci_tx_pkt_ipx_udp(
    int af, ci_ip_pkt_fmt* pkt, bool is_frag)
{
...
static inline ci_uint16 ci_tx_pkt_ipx_tcp_payload_len(
    int af, ci_ip_pkt_fmt* pkt)
{
  ci_uint16 len;
#if CI_CFG_IPV6
  if( af == AF_INET6 ) {
...
#define NI_PKT_SET(ni)                                               \
  ((ni)->packets->id < 0                      ? 0                    \
      : (ni)->packets->id >= (ni)->pkt_sets_n ? (ni)->pkt_sets_n - 1 \
                                              : (ni)->packets->id)
#else
...
#ifdef __KERNEL__
extern void ci_netif_state_init(
    ci_netif* ni, int cpu_khz, const char* name) CI_HF;
extern int ci_netif_ctor(
    ci_netif**, const ci_netif_config_opts*, unsigned flags) CI_HF;
#else
extern int ci_netif_ctor(
    ci_netif*, ef_driver_handle, const char* name, unsigned flags) CI_HF;
extern void ci_netif_cluster_prefault(ci_netif* ni) CI_HF;
#endif
extern int ci_netif_restore_id(
    ci_netif*, unsigned stack_id, bool is_service) CI_HF;
extern int citp_netif_by_id(
    ci_uint32 stack_id, ci_netif** out_ni, int locked) CI_HF;
extern int ci_netif_restore_name(ci_netif*, const char*) CI_HF;
extern int ci_netif_restore(
    ci_netif* ni, ci_fd_t fd, unsigned netif_mmap_bytes) CI_HF;
extern int ci_netif_dtor(ci_netif*) CI_HF;
extern unsigned ci_netif_build_future_intf_mask(ci_netif* ni) CI_HF;

extern void ci_netif_error_detected(
    ci_netif*, unsigned error_flag, const char* caller) CI_HF;

#if OO_DO_STACK_POLL
#ifndef __KERNEL__
extern int ci_netif_poll_intf_future(
    ci_netif*, int intf_i, ci_uint64 now_frc) CI_HF;
#endif
...
extern void ci_netif_tx_pkt_complete(
    ci_netif*, struct ci_netif_poll_state*, ci_ip_pkt_fmt*);
...
ci_inline void cicp_pkt_complete_fake(ci_netif* ni, ci_ip_pkt_fmt* pkt)
{
...
extern void ci_netif_dump_to_logger(
    ci_netif* ni, oo_dump_log_fn_t logger, void* log_arg) CI_HF;
extern void ci_netif_dump_vi_stats(ci_netif* ni) CI_HF;
extern void ci_netif_dump_vi_stats_to_logger(
    ci_netif* ni, oo_dump_log_fn_t logger, void* log_arg) CI_HF;
extern void ci_netif_dump_extra(ci_netif* ni) CI_HF;
extern void ci_netif_dump_extra_to_logger(
    ci_netif* ni, oo_dump_log_fn_t logger, void* log_arg) CI_HF;
extern void ci_netif_dump_sockets(ci_netif* ni) CI_HF;
extern void ci_netif_dump_sockets_to_logger(
    ci_netif* ni, oo_dump_log_fn_t logger, void* log_arg) CI_HF;
extern void ci_netif_netstat_sockets_to_logger(
    ci_netif* ni, oo_dump_log_fn_t logger, void* log_arg) CI_HF;
...
extern void ci_netif_config_opts_dump(
    ci_netif_config_opts* opts, oo_dump_log_fn_t logger, void* log_arg) CI_HF;
extern void ci_stack_time_dump(
    ci_netif* ni, oo_dump_log_fn_t logger, void* log_arg) CI_HF;
extern void ci_netif_pkt_dump_all(ci_netif* ni) CI_HF;
extern void ci_netif_pkt_queue_dump(
    ci_netif* ni, ci_ip_pkt_queue* q, int is_recv, int dump) CI_HF;
extern void ci_netif_pkt_list_dump(
    ci_netif* ni, oo_pkt_p head, int is_recv, int dump) CI_HF;
extern void ci_netif_pkt_dump(
    ci_netif* ni, ci_ip_pkt_fmt*, int is_recv, int dump) CI_HF;
...
extern void ci_tcp_listen_rx_checks(
    ci_netif*, ci_tcp_socket_listen*, ci_ip_pkt_fmt*) CI_HF;
...
#define ASSERT_VALID_NETIF_ADDR(ni, addr, size)                  \
  do {                                                           \
...
ci_inline int ci_netif_num_vis(ci_netif* ni)
{
  ...
  switch( NI_OPTS(ni).tcp_offload_plugin ) {
    case CITP_TCP_OFFLOAD_OFF:
      return 1;
    case CITP_TCP_OFFLOAD_NVME:
      return 1;
    case CITP_TCP_OFFLOAD_RAW_TCP:
      return 2;
    default:
      return 2 + CI_CFG_TCP_PLUGIN_EXTRA_VIS;
  }
#endif
  return 1;
}
...
extern void ci_netif_pkt_free(ci_netif* ni,
    ci_ip_pkt_fmt* pkt CI_KERNEL_ARG(int* p_netif_is_locked)) CI_HF;
...
extern int ci_netif_pkt_try_to_free(
    ci_netif* ni, int desperation, int stop_once_freed_n) CI_HF;
extern void ci_netif_try_to_reap(ci_netif* ni, int stop_once_freed_n) CI_HF;
extern void ci_netif_rxq_low_on_recv(
    ci_netif*, ci_sock_cmn*, int bytes_freed) CI_HF;
 ...
 */
extern int ci_netif_pkt_alloc_block(ci_netif*, ci_sock_cmn*, int* ni_locked,
    int can_block, ci_ip_pkt_fmt** p_pkt) CI_HF;
...
extern ci_ip_pkt_fmt* __ci_netif_pkt_rx_to_tx(
    ci_netif*, ci_ip_pkt_fmt*, const char*) CI_HF;
...
extern int ci_icmp_send(ci_netif* ni, ci_ip_pkt_fmt* tx_pkt,
    const ci_addr_t saddr, const ci_addr_t daddr,
    const ci_mac_addr_t* mac_dest, ci_uint8 type, ci_uint8 code,
    ci_uint16 data_len) CI_HF;
...
#define UDP_IS_SHUT_RDWR(us) \
  (((us)->s.rx_errno & (CI_SHUT_RD | CI_SHUT_WR)) == (CI_SHUT_RD | CI_SHUT_WR))
...
ci_inline void ci_sock_cmn_set_laddr(
    ci_sock_cmn* s, ci_addr_t addr, int lport_be16)
{
...
extern void ci_udp_state_assert_valid(
    ci_netif*, ci_udp_state* ts, const char* file, int line) CI_HF;

extern void ci_udp_ep_assert_valid(
    citp_socket* ep, const char* file, int line) CI_HF;


/*** udp_rx.c ***/
extern void ci_udp_handle_rx(
    ci_netif*, ci_ip_pkt_fmt* pkt, ci_udp_hdr*, int ip_paylen) CI_HF;


ci_inline void ci_pkt_init_from_ipcache_len(
    ci_ip_pkt_fmt* pkt, const ci_ip_cached_hdrs* ipcache, size_t header_len)
{
...
ci_inline void ci_pkt_init_from_ipcache(
    ci_ip_pkt_fmt* pkt, const ci_ip_cached_hdrs* ipcache)
{
  ci_pkt_init_from_ipcache_len(
      pkt, ipcache, CI_IPX_HDR_SIZE(ipcache_af(ipcache)) + sizeof(ci_tcp_hdr));
}
...
extern ci_fd_t ci_udp_ep_ctor(
    citp_socket* ep, ci_netif* sh, int domain, int type) CI_HF;
extern int ci_udp_bind_start(citp_socket* ep, ci_fd_t fd,
    const struct sockaddr* addr, socklen_t addrlen, ci_uint16* lport) CI_HF;
...
extern void ci_udp_handle_force_reuseport(ci_fd_t fd, citp_socket* ep,
    const struct sockaddr* sa, socklen_t sa_len) CI_HF;
extern int ci_udp_reuseport_bind(citp_socket* ep, ci_fd_t fd,
    const struct sockaddr* sa, socklen_t sa_len, ci_uint16 lport) CI_HF;
#endif
extern int ci_udp_connect(
    citp_socket*, ci_fd_t fd, const struct sockaddr*, socklen_t addrlen) CI_HF;
extern int ci_udp_connect_conclude(citp_socket* ep, ci_fd_t fd,
    const struct sockaddr* serv_addr, socklen_t addrlen,
    ci_fd_t os_sock) CI_HF;
...
extern int ci_udp_getpeername(
    citp_socket*, struct sockaddr*, socklen_t*) CI_HF;
...
extern int ci_udp_sendmsg(ci_udp_iomsg_args* a, const ci_msghdr*, int) CI_HF;
extern int ci_udp_recvmsg(ci_udp_iomsg_args* a, ci_msghdr*, int flags) CI_HF;
...
extern int ci_udp_recvmmsg(ci_udp_iomsg_args* a, struct mmsghdr* mmsg,
    ...
    const struct timespec* timeout CI_KERNEL_ARG(
        ci_addr_spc_t addr_spc)) CI_HF;

struct onload_zc_mmsg;
extern int ci_tcp_zc_send(
    ci_netif* ni, ci_tcp_state* ts, struct onload_zc_mmsg* msgs, int flags);
 ...
 * doing zero-copy
 */
extern int ci_udp_recvmsg_kernel(
    int fd, ci_netif* ni, ci_udp_state* us, struct msghdr* msg, int flags);

extern enum onload_delegated_send_rc ci_tcp_ds_fill_headers(ci_netif* ni,
    ci_tcp_state* ts, unsigned flags, void* headers, int* headers_len_inout,
    int* ip_tcp_hdr_len_out, int* tcp_seq_offset_out, int* ip_len_offset_out);
extern int ci_tcp_ds_done(ci_netif* ni, ci_tcp_state* ts, const ci_iovec* iov,
    int iovlen, int flags);

extern int ci_netif_raw_send(
    ci_netif* ni, int intf_i, const ci_iovec* iov, int iovlen);
#endif

extern void ci_ip_cmsg_recv(ci_netif*, ci_udp_state*, const ci_ip_pkt_fmt*,
    struct msghdr*, int netif_locked, int* p_msg_flags) CI_HF;
#if OO_DO_STACK_POLL
...
extern void ci_udp_perform_deferred_socket_work(
    ci_netif*, ci_udp_state*) CI_HF;
extern int ci_udp_try_to_free_pkts(
    ci_netif*, ci_udp_state*, int desperation) CI_HF;
...
typedef int (*ci_pipe_zc_read_cb)(
    void* context, struct iovec* iovec, int iov_num, int flags);

extern int ci_pipe_read(
    ci_netif*, struct oo_pipe*, const struct iovec*, size_t iovlen) CI_HF;
extern int oo_pipe_write_block(
    ci_netif* ni, struct oo_pipe* p, int flags) CI_HF;
extern int ci_pipe_write(
    ci_netif*, struct oo_pipe*, const struct iovec*, size_t iovlen) CI_HF;
extern int ci_pipe_zc_read(ci_netif* ni, struct oo_pipe* p, int len, int flags,
    ci_pipe_zc_read_cb cb, void* ctx) CI_HF;
...
extern int ci_pipe_zc_write(ci_netif* ni, struct oo_pipe* p,
    struct ci_pipe_pkt_list* pkts, int len, int flags) CI_HF;
extern int ci_pipe_zc_alloc_buffers(ci_netif* ni, struct oo_pipe* p, int flags,
    int count, struct ci_pipe_pkt_list* pkts_out) CI_HF;
extern int ci_pipe_zc_release_buffers(
    ci_netif* ni, struct oo_pipe* p, struct ci_pipe_pkt_list* pkts) CI_HF;
extern int ci_pipe_set_size(
    ci_netif* ni, struct oo_pipe* p, size_t size) CI_HF;
...
extern int ci_pipe_list_to_iovec(ci_netif* ni, struct oo_pipe* p,
    struct iovec* iov, int* iov_num, struct ci_pipe_pkt_list* pkts, int len);
...
#define CI_TCP_CLOSED                                   \
  (0x0000 | CI_TCP_STATE_TCP | CI_TCP_STATE_SLOW_PATH | \
      CI_TCP_STATE_NOT_CONNECTED | CI_TCP_STATE_NO_TIMERS)
#define CI_TCP_LISTEN                                   \
  (0x1000 | CI_TCP_STATE_TCP | CI_TCP_STATE_SLOW_PATH | \
      CI_TCP_STATE_NOT_CONNECTED | CI_TCP_STATE_NO_TIMERS)
#define CI_TCP_INVALID (CI_TCP_LISTEN | CI_TCP_STATE_INVALID)
#define CI_TCP_SYN_SENT                                     \
  (0x2000 | CI_TCP_STATE_TCP | CI_TCP_STATE_TCP_CONN |      \
      CI_TCP_STATE_SLOW_PATH | CI_TCP_STATE_NOT_CONNECTED | \
      CI_TCP_STATE_TXQ_ACTIVE)
#define CI_TCP_ESTABLISHED                                   \
  (0x3000 | CI_TCP_STATE_TCP | CI_TCP_STATE_TCP_CONN |       \
      CI_TCP_STATE_SYNCHRONISED | CI_TCP_STATE_ACCEPT_DATA | \
      CI_TCP_STATE_TXQ_ACTIVE)
#define CI_TCP_CLOSE_WAIT                                  \
  (0x4000 | CI_TCP_STATE_TCP | CI_TCP_STATE_TCP_CONN |     \
      CI_TCP_STATE_SYNCHRONISED | CI_TCP_STATE_SLOW_PATH | \
      CI_TCP_STATE_RECVD_FIN | CI_TCP_STATE_TXQ_ACTIVE)
#define CI_TCP_LAST_ACK                                     \
  (0x5000 | CI_TCP_STATE_TCP | CI_TCP_STATE_TCP_CONN |      \
      CI_TCP_STATE_SYNCHRONISED | CI_TCP_STATE_SLOW_PATH |  \
      CI_TCP_STATE_NOT_CONNECTED | CI_TCP_STATE_RECVD_FIN | \
      CI_TCP_STATE_TXQ_ACTIVE | CI_TCP_STATE_TIMEOUT_ORPHAN)
#define CI_TCP_FIN_WAIT1                                   \
  (0x6000 | CI_TCP_STATE_TCP | CI_TCP_STATE_TCP_CONN |     \
      CI_TCP_STATE_SLOW_PATH | CI_TCP_STATE_SYNCHRONISED | \
      CI_TCP_STATE_ACCEPT_DATA | CI_TCP_STATE_TXQ_ACTIVE | \
      CI_TCP_STATE_TIMEOUT_ORPHAN)
#define CI_TCP_FIN_WAIT2                                   \
  (0x7000 | CI_TCP_STATE_TCP | CI_TCP_STATE_TCP_CONN |     \
      CI_TCP_STATE_SLOW_PATH | CI_TCP_STATE_SYNCHRONISED | \
      CI_TCP_STATE_ACCEPT_DATA | CI_TCP_STATE_TIMEOUT_ORPHAN)
#define CI_TCP_CLOSING                                      \
  (0x8000 | CI_TCP_STATE_TCP | CI_TCP_STATE_TCP_CONN |      \
      CI_TCP_STATE_SYNCHRONISED | CI_TCP_STATE_SLOW_PATH |  \
      CI_TCP_STATE_NOT_CONNECTED | CI_TCP_STATE_RECVD_FIN | \
      CI_TCP_STATE_TXQ_ACTIVE | CI_TCP_STATE_TIMEOUT_ORPHAN)
#define CI_TCP_TIME_WAIT                                    \
  (0x9000 | CI_TCP_STATE_TCP | CI_TCP_STATE_TCP_CONN |      \
      CI_TCP_STATE_SYNCHRONISED | CI_TCP_STATE_SLOW_PATH |  \
      CI_TCP_STATE_NOT_CONNECTED |                          \
      CI_TCP_STATE_RECVD_FIN /* 2MSL timer doesn't count */ \
...
#define CI_TCP_OOB_FLAGS                                                   \
  (CI_TCP_FLAG_SYN | CI_TCP_FLAG_FIN | CI_TCP_FLAG_RST | CI_TCP_FLAG_ECE | \
      CI_TCP_FLAG_CWR)

/* Flags to check for a socket */
#define CI_TCP_STATE_IS_SOCKET(s) \
  ((s) == CI_TCP_STATE_UDP || (s) &CI_TCP_STATE_TCP)
...
#define tcp_urg_data_invalidate(ts) \
  ((ts)->urg_data &=                \
...
#if CI_CFG_IPV6
#define S_IPX_TCP_HDR(s) \
  ((ipcache_is_ipv6(&(s)->pkt)) ? S_IP6_TCP_HDR(s) : S_TCP_HDR(s))
#define TS_IPX_TCP(ts) \
  ((ipcache_is_ipv6(&(ts)->s.pkt)) ? TS_IP6_TCP(ts) : TS_TCP(ts))
#else
...
#define TS_QUEUE_RX(ts) \
  ((ci_ip_pkt_queue*) ((ci_uintptr_t) (ts) + (ts)->recv_off))

/** Offset of TS within netif state. */
#define TS_OFF(ni, ts) oo_sockp_to_statep((ni), S_SP(ts))
#define TS_MEMBER_OFF(ni, ts, member) \
  ((ci_uint32) ((char*) &(member) - (char*) (ts) + TS_OFF((ni), (ts))))
#define TS_FMT         "%d(%u)"
...
#if CI_CFG_IPV6
#define ipcache_ttl(ipcache)                                  \
  (*(ipcache_is_ipv6(ipcache) ? &(ipcache)->ipx.ip6.hop_limit \
                              : &(ipcache)->ipx.ip4.ip_ttl))
#else
#define ipcache_ttl(ipcache) ((ipcache)->ipx.ip4.ip_ttl)
#endif

#if CI_CFG_IPV6
#define ipcache_protocol(ipcache)                            \
  (*(ipcache_is_ipv6(ipcache) ? &(ipcache)->ipx.ip6.next_hdr \
                              : &(ipcache)->ipx.ip4.ip_protocol))
#else
...
#define ipcache_lport_be16(ipcache)                                     \
  ((ipcache_is_ipv6(ipcache) ? ((ci_uint16*) (&(ipcache)->ipx.ip6 + 1)) \
...
#define ci_tcp_can_use_fast_path(ts)                                  \
  ((~ts->s.b.state & CI_TCP_STATE_SLOW_PATH) &&                       \
      ci_ip_queue_is_empty(&(ts)->rob) && tcp_rx_urg_fast_path(ts) && \
...
static inline bool ci_tcp_is_pluginized(ci_tcp_state* ts)
{
...
extern void ci_tcp_state_init(
    ci_netif* netif, ci_tcp_state* ts, int from_cache) CI_HF;
extern void ci_tcp_state_tcb_reinit_minimal(
    ci_netif* netif, ci_tcp_state* ts) CI_HF;
...
extern int ci_tcp_try_to_free_pkts(
    ci_netif* ni, ci_tcp_state* ts, int desperation) CI_HF;
...
#if OO_DO_STACK_POLL
extern void ci_tcp_listen_all_fds_gone(
    ci_netif*, ci_tcp_socket_listen*, int do_free) CI_HF;
extern void ci_tcp_all_fds_gone(
    ci_netif* netif, ci_tcp_state*, int do_free) CI_HF;
#endif
...
extern void ci_tcp_rx_reap_rxq_last_buf(
    ci_netif* netif, ci_tcp_state* ts) CI_HF;

static inline void ci_tcp_rx_reap_rxq_bufs_socklocked(
...
extern void ci_tcp_pkt_dump(
    ci_netif* ni, ci_ip_pkt_fmt* pkt, int is_recv, int dump) CI_HF;
extern void ci_tcp_socket_listen_dump(ci_netif*, ci_tcp_socket_listen*,
    const char* pf, oo_dump_log_fn_t logger, void* log_arg) CI_HF;
...
extern void ci_tcp_rcvbuf_unabuse(
    ci_netif* ni, ci_tcp_state* ts, int sock_already_locked) CI_HF;

extern void ci_tcp_syncookie_syn(
    ci_netif* netif, ci_tcp_socket_listen* tls, ci_tcp_state_synrecv* tsr);
extern void ci_tcp_syncookie_ack(ci_netif* netif, ci_tcp_socket_listen* tls,
    ciip_tcp_rx_pkt* rxp, ci_tcp_state_synrecv** tsr_p);
...
extern int ci_tcp_use_mac_filter_listen(
    ci_netif* ni, ci_sock_cmn* s, ci_ifid_t ifindex);

#ifndef __KERNEL__
extern int ci_tcp_can_set_filter_in_ul(ci_netif* ni, ci_sock_cmn* s);
#endif

extern int ci_tcp_sock_set_stack_filter(ci_netif* ni, ci_sock_cmn* s);

extern void ci_tcp_sock_clear_stack_filter(ci_netif* ni, ci_tcp_state* ts);

#if CI_CFG_FD_CACHING
extern int /*bool*/ ci_tcp_is_cacheable_active_wild_sharer(ci_sock_cmn*);
#endif

extern int ci_tcp_offload_get_stream_id(
    ci_netif* ni, ci_tcp_state* ts, int intf_i);
...
#if OO_DO_STACK_POLL
extern void ci_pipe_all_fds_gone(
    ci_netif* netif, struct oo_pipe* p, int do_free);
#endif
...
ci_inline void ci_addr_simple_hash(ci_addr_t addr, ci_uint32 entries,
...
extern void ci_active_wild_all_fds_gone(
    ci_netif* ni, ci_active_wild* aw, int do_free);
...
extern void citp_waitable_cleanup(
    ci_netif* ni, citp_waitable_obj* wo, int do_free);
#endif
...
extern void citp_waitable_dump_to_logger(ci_netif* ni, citp_waitable* w,
    const char* pf, oo_dump_log_fn_t logger, void* log_arg) CI_HF;
extern void citp_waitable_print_to_logger(
    ci_netif*, citp_waitable*, oo_dump_log_fn_t logger, void* log_arg) CI_HF;
extern void ci_tcp_listenq_print_to_logger(ci_netif* ni,
    ci_tcp_socket_listen* tls, oo_dump_log_fn_t logger, void* log_arg);


/*********************************************************************
*********************************************************************/

extern void ci_tcp_listenq_insert(
    ci_netif*, ci_tcp_socket_listen*, ci_tcp_state_synrecv*) CI_HF;
extern void ci_tcp_listenq_remove(
    ci_netif*, ci_tcp_socket_listen*, ci_tcp_state_synrecv*) CI_HF;
extern void ci_tcp_listenq_drop(
    ci_netif*, ci_tcp_socket_listen*, ci_tcp_state_synrecv*) CI_HF;
extern ci_tcp_state_synrecv* ci_tcp_listenq_lookup(
    ci_netif* netif, ci_tcp_socket_listen* tls, ciip_tcp_rx_pkt*) CI_HF;
...
extern int ci_tcp_listenq_try_promote(ci_netif*, ci_tcp_socket_listen*,
    ci_tcp_state_synrecv*, ci_ip_cached_hdrs*, ci_ip_pkt_fmt*,
    ci_tcp_state**) CI_HF;
...
extern void ci_tcp_rx_deliver2(
    ci_tcp_state*, ci_netif*, ciip_tcp_rx_pkt*) CI_HF;
extern void ci_tcp_rx_plugin_meta(
    ci_netif*, struct ci_netif_poll_state*, ci_ip_pkt_fmt* pkt) CI_HF;

extern void ci_tcp_tx_change_mss(ci_netif*, ci_tcp_state*) CI_HF;
extern void ci_tcp_enqueue_no_data(
    ci_tcp_state* ts, ci_netif* netif, ci_ip_pkt_fmt* pkt) CI_HF;
...
extern void ci_tcp_retrans_init_ptrs(
    ci_netif* ni, ci_tcp_state* ts, unsigned* recover_seq_out) CI_HF;
extern void ci_tcp_get_fack(ci_netif* ni, ci_tcp_state* ts, unsigned* fack_out,
    int* retrans_data_out) CI_HF;


extern void ci_tcp_retrans_coalesce_block(
    ci_netif* ni, ci_tcp_state* ts, ci_ip_pkt_fmt* pkt) CI_HF;
extern int ci_tcp_tx_coalesce(ci_netif* ni, ci_tcp_state* ts,
    ci_ip_pkt_queue* q, ci_ip_pkt_fmt* pkt, ci_boolean_t is_sendq) CI_HF;
extern void ci_tcp_tx_insert_option_space(ci_netif* ni, ci_tcp_state* ts,
    ci_ip_pkt_fmt* pkt, int hdrlen, int extra_opts) CI_HF;
...
extern void ci_tcp_send_rst_with_flags(
    ci_netif*, ci_tcp_state*, ci_uint8 extra_flags) CI_HF;
extern void ci_tcp_send_rst(ci_netif* netif, ci_tcp_state* ts) CI_HF;
extern void ci_tcp_reply_with_rst(ci_netif* netif,
    const struct oo_sock_cplane* sock_cp, ciip_tcp_rx_pkt* rxp) CI_HF;
...
extern int ci_tcp_parse_options(
    ci_netif*, ciip_tcp_rx_pkt*, ci_tcp_options*) CI_HF;

extern void ci_ipx_hdr_init_fixed(
    ci_ipx_hdr_t* ip, int af, int protocol, int ttl, unsigned tos) CI_HF;

extern void ci_tcp_send_ack_rx(ci_netif*, ci_tcp_state*, ci_ip_pkt_fmt*,
    int sock_locked, int update_wnd) CI_HF;
ci_inline void ci_tcp_send_ack(
    ci_netif* netif, ci_tcp_state* ts, ci_ip_pkt_fmt* pkt, int sock_locked)
{
  ci_tcp_send_ack_rx(netif, ts, pkt, sock_locked, 1);
}
extern int ci_tcp_send_challenge_ack(
    ci_netif*, ci_tcp_state*, ci_ip_pkt_fmt*) CI_HF;
extern int /*bool*/
...
extern int ci_tcp_send_wnd_update(
    ci_netif*, ci_tcp_state*, int sock_locked) CI_HF;
...
#if CI_CFG_IPV6
void ci_ip6_netif_filter_init(
    ci_ip6_netif_filter_table* tbl, int size_lg2) CI_HF;
#endif

extern ci_sock_cmn* __ci_netif_filter_lookup(ci_netif* netif, int af_space,
    ci_addr_t daddr, unsigned dport, ci_addr_t saddr, unsigned sport,
    unsigned prot) CI_HF;

#if CI_CFG_IPV6
extern int ci_ip6_netif_filter_lookup(ci_netif* netif, ci_addr_t laddr,
    unsigned lport, ci_addr_t raddr, unsigned rport, unsigned prot) CI_HF;
extern int __ci_ip6_netif_filter_lookup(ci_netif* netif, ci_addr_t laddr,
    unsigned lport, ci_addr_t raddr, unsigned rport, unsigned prot) CI_HF;
#endif
extern oo_sp ci_netif_filter_lookup(ci_netif* netif, int af_space,
    ci_addr_t laddr, unsigned lport, ci_addr_t raddr, unsigned rport,
    unsigned protocol);

/* Returns socket index, or OO_SP_NULL if lookup failed. */
extern oo_sp ci_netif_listener_lookup(
    ci_netif* netif, int af_space, ci_addr_t laddr, unsigned lport) CI_HF;
 ...
 */
extern int ci_netif_filter_for_each_match(ci_netif*, unsigned laddr,
    unsigned lport, unsigned raddr, unsigned rport, unsigned protocol,
    int intf_i, int vlan, int (*callback)(ci_sock_cmn*, void*),
    void* callback_arg, ci_uint32* hash_out) CI_HF;

#if CI_CFG_IPV6
extern int ci_netif_filter_for_each_match_ip6(ci_netif* ni,
    const ci_addr_t* laddr, unsigned lport, const ci_addr_t* raddr,
    unsigned rport, unsigned protocol, int intf_i, int vlan,
    int (*callback)(ci_sock_cmn*, void*), void* callback_arg,
    ci_uint32* hash_out) CI_HF;
#endif

extern ci_uint32 ci_netif_filter_hash(ci_netif* ni, ci_addr_t laddr,
    unsigned lport, ci_addr_t raddr, unsigned rport, unsigned protocol) CI_HF;

extern int ci_netif_filter_insert(ci_netif* netif, oo_sp sock_id, int af_space,
    const ci_addr_t laddr, unsigned lport, const ci_addr_t raddr,
    unsigned rport, unsigned protocol) CI_HF;

extern void ci_netif_filter_remove(ci_netif* netif, oo_sp tcp_id, int af_space,
    const ci_addr_t laddr, unsigned lport, const ci_addr_t raddr,
    unsigned rport, unsigned protocol) CI_HF;

#if CI_CFG_UL_INTERRUPT_HELPER || defined(__KERNEL__)
ci_inline void oo_sw_filter_apply(ci_netif* ni, struct oo_sw_filter_op* op)
{
  if( op->op == OO_SW_FILTER_OP_ADD ) {
    ci_netif_filter_insert(ni, op->sock_id, op->af_space, op->laddr, op->lport,
        op->raddr, op->rport, op->protocol);
  } else {
    ci_netif_filter_remove(ni, op->sock_id, op->af_space, op->laddr, op->lport,
        op->raddr, op->rport, op->protocol);
  }
}
#endif
...
extern oo_sp ci_netif_active_wild_get(ci_netif* ni, ci_addr_t laddr,
    ci_addr_t raddr, unsigned lport, ci_uint16* port_out,
...
extern int ci_netif_active_wild_nic_hash(ci_netif* ni, ci_addr_t laddr,
    ci_uint16 lport, ci_addr_t raddr, ci_uint16 rport);

extern struct oo_p_dllink_state ci_netif_get_active_wild_list(
    ci_netif* ni, int aw_pool, ci_addr_t laddr);
#endif
...
extern int __ci_tcp_shutdown(ci_netif*, ci_tcp_state*, int how) CI_HF;
extern void __ci_tcp_listen_shutdown(ci_netif*, ci_tcp_socket_listen*) CI_HF;
extern void ci_tcp_listen_shutdown_queues(
    ci_netif* netif, ci_tcp_socket_listen* tls) CI_HF;
#if CI_CFG_FD_CACHING
extern void ci_tcp_listen_uncache_fds(
    ci_netif* netif, ci_tcp_socket_listen* tls) CI_HF;
extern void ci_tcp_epcache_drop_cache(ci_netif* ni) CI_HF;
extern void ci_tcp_listen_update_cached(
    ci_netif* netif, ci_tcp_socket_listen* tls) CI_HF;
...
extern unsigned int ci_tcp_wscl_by_buff(
    ci_netif* netif, ci_int32 rcv_buff) CI_HF;
...
extern void ci_tcp_timeout_listen(
    ci_netif* netif, ci_tcp_socket_listen* tls) CI_HF;
...
extern void ci_tcp_send_corked_packets(
    ci_netif* netif, ci_tcp_state* ts) CI_HF;

extern void ci_tcp_tx_pkt_assert_valid(ci_netif* ni, ci_tcp_state* ts,
    ci_ip_pkt_fmt*, const char* f, int l) CI_HF;
extern void ci_tcp_state_assert_valid(
    ci_netif*, ci_tcp_state* ts, const char* file, int line) CI_HF;
extern void ci_tcp_state_listen_assert_valid(
    ci_netif*, ci_tcp_socket_listen*, const char* file, int line) CI_HF;
...
extern int ci_opt_is_setting_reuseport(
    int level, int optname, const void* optval, socklen_t optlen) CI_HF;
extern int ci_setsockopt_os_fail_ignore(ci_netif* ni, ci_sock_cmn* s, int err,
    int level, int optname, const void* optval, socklen_t optlen) CI_HF;
...
#endif
extern void ci_tcp_get_peer_addr(
    ci_tcp_state* ts, struct sockaddr* name, socklen_t* namelen) CI_HF;
extern int ci_tcp_getpeername(
    citp_socket*, struct sockaddr*, socklen_t*) CI_HF;
extern int ci_tcp_getsockname(
    citp_socket*, ci_fd_t, struct sockaddr*, socklen_t*) CI_HF;

extern int ci_tcp_getsockopt(citp_socket* ep, ci_fd_t fd, int level,
    int optname, void* optval, socklen_t* optlen) CI_HF;
extern int ci_tcp_setsockopt(citp_socket* ep, ci_fd_t fd, int level,
    int optname, const void* optval, socklen_t optlen) CI_HF;
extern int ci_tcp_ioctl(
    citp_socket* ep, ci_fd_t fd, int request, void* arg) CI_HF;

struct oo_msg_template;
struct onload_template_msg_update_iovec;

extern int ci_tcp_tmpl_alloc(ci_netif* ni, ci_tcp_state* ts,
    struct oo_msg_template** omt_pp, const struct iovec* initial_msg, int mlen,
...
extern int ci_tcp_tmpl_update(ci_netif* ni, ci_tcp_state* ts,
    ...
    unsigned flags) CI_HF;
extern int ci_tcp_tmpl_abort(
    ci_netif* ni, ci_tcp_state* ts, struct oo_msg_template* omt) CI_HF;
...
extern int ci_tcp_sync_sockopts_to_os_sock(
    ci_netif* ni, oo_sp sock_id, struct socket* sock) CI_HF;
#endif
...
struct onload_zc_recv_args;
extern int ci_tcp_zc_recvmsg(
    const ci_tcp_recvmsg_args*, struct onload_zc_recv_args* args) CI_HF;
extern int ci_tcp_sendmsg(ci_netif* ni, ci_tcp_state* ts, const ci_iovec* iov,
    unsigned long iovlen,
    int flags CI_KERNEL_ARG(ci_addr_spc_t addr_spc)) CI_HF;
extern void ci_tcp_sendmsg_enqueue_prequeue_deferred(
    ci_netif*, ci_tcp_state*) CI_HF;
extern void ci_tcp_sendmsg_enqueue_prequeue(
    ci_netif* ni, ci_tcp_state* ts, int /*bool*/ shutdown) CI_HF;
extern void ci_tcp_perform_deferred_socket_work(
    ci_netif*, ci_tcp_state*) CI_HF;
...
extern oo_sp ci_tcp_connect_find_local_peer(
    ci_netif* ni, int locked, ci_addr_t dst_addr, int dport_be16) CI_HF;

#ifdef __KERNEL__
extern int ci_tcp_connect_lo_samestack(
    ci_netif* ni, ci_tcp_state* ts, oo_sp tls_id, int* stack_locked) CI_HF;
...
extern ci_uint16 ci_tcp_limit_mss(
    ci_uint16 mss, ci_netif* ni, const char* caller) CI_HF;
#endif
extern unsigned ci_tcp_amss(ci_netif* ni, const ci_tcp_socket_cmn* c,
    ci_ip_cached_hdrs* ipcache, const char* caller) CI_HF;
...
extern int ci_can_handle_addr(ci_netif* netif, ci_uint32 ip_be32,
    unsigned int proto, ci_uint32* src_ip_be32_out, unsigned* nic_i_out,
    unsigned* mtu_out) CI_HF;
...
extern int ci_sock_sleep(ci_netif* ni, citp_waitable* w, ci_bits why,
    unsigned lock_flags, ci_uint64 sleep_seq, ci_uint32* timeout_ms_p) CI_HF;
#endif
...
#define ci_rel_os_sock_fd(fd)          \
  do {                                 \
    if( CI_IS_VALID_SOCKET(fd) )       \
      ci_tcp_helper_rel_sock_fd((fd)); \
  } while( 0 )
...
*/
extern int __ci_copy_iovec_to_pkt(ci_netif*, ci_ip_pkt_fmt*,
    ci_iovec_ptr* CI_KERNEL_ARG(ci_addr_spc_t)) CI_HF;
...
extern ssize_t __ci_ip_copy_pkt_to_user(
    ci_netif*, ci_iovec*, ci_ip_pkt_fmt*, int peek_off) CI_HF;

#if defined(__KERNEL__)
#define ci_ip_copy_pkt_from_piov __ci_ip_copy_pkt_from_piov
extern size_t __ci_ip_copy_pkt_from_piov(
    ci_netif*, ci_ip_pkt_fmt*, ci_iovec_ptr*, ci_addr_spc_t) CI_HF;
#else /* __KERNEL__ */

#define ci_ip_copy_pkt_from_piov(ni, pkt, iov, aspc) \
  __ci_ip_copy_pkt_from_piov((ni), (pkt), (iov))
extern size_t __ci_ip_copy_pkt_from_piov(
    ci_netif*, ci_ip_pkt_fmt*, ci_iovec_ptr*) CI_HF;
#endif
...
#define OO_FOR_EACH_BIT(init_mask, mask, bitno)                     \
  for( (mask) = (init_mask), (bitno) = __builtin_ctz(mask); (mask); \
...
extern ci_uint32 ci_ipcache_make_flowlabel(
    ci_netif* ni, ci_ip_cached_hdrs* ipcache) CI_HF;
...
extern void ci_netif_send_plugin_app_ctrl(ci_netif* ni, int nic_index,
    ci_ip_pkt_fmt* pkt, const void* payload, size_t paylen);

extern void __ci_netif_ring_plugin_app_doorbell(
    ci_netif* netif, int nic_index);

ci_inline void ci_netif_ring_ceph_doorbell(
    ci_netif* netif, int nic_index, int n)
{
#if CI_CFG_TCP_OFFLOAD_RECYCLER
...
#else
ci_inline ci_tcp_state* SOCK_TO_TCP_DEBUG(
    ci_sock_cmn* s, const char* file, int line)
{
  _ci_assert(s->b.state & CI_TCP_STATE_TCP, file, line);
  _ci_assert(
      s->b.state == CI_TCP_CLOSED || (s->b.state & CI_TCP_STATE_TCP_CONN),
      file, line);
 ...
 * If we add support for detecting subsequent cache lines, or if packet
 * prefixes are enabled, there will be the possibility of deciding falsely that
 * a packet is still poisonous when in fact it is not, but there is very little
 * that we can do about that. It would not cause a functional problem in any
 * case.
 *
 ...
 */
ci_inline const volatile uint32_t* ci_netif_intf_rx_future(
    ci_netif* ni, int intf_i, const uint32_t* poison)
{
  ci_ip_pkt_fmt* pkt;
  ci_uint8* p;
  ef_vi* vi;
...
{
  return ci_netif_not_primed(ni) && ci_netif_need_poll_spinning(ni, frc_now);
}
...
ci_inline int ci_netif_need_poll_maybe_spinning(
    ci_netif* ni, ci_uint64 frc_now, int spinning)
{
  if( spinning )
...
{
  return NI_OPTS(ni).tcp_shared_local_ports > 0 &&
         NI_OPTS(ni).scalable_filter_enable !=
...
ci_inline struct ci_pkt_zc_payload* oo_tx_zc_payload_next(
    ci_netif* ni, struct ci_pkt_zc_payload* zcp)
{
  if( zcp->is_remote )
    return (void*) ((char*) zcp + oo_tx_zc_payload_size(ni));
  return (
      void*) (zcp->local + CI_ALIGN_FWD(zcp->len, CI_PKT_ZC_PAYLOAD_ALIGN));
}

#define OO_TX_FOR_EACH_ZC_PAYLOAD(ni, zch, zcp)                         \
  for( (zcp) = (zch)->data; (char*) (zcp) - (char*) (zch) < (zch)->end; \
 ...
 */
ci_inline void ci_nvme_plugin_crc_id_init(
    struct nvme_crc_plugin_idp_t* idp, unsigned base, unsigned limit)
{
  int i = 0;
...
ci_inline int ci_nvme_plugin_crc_id_alloc(
    struct nvme_crc_plugin_idp_t* idp, unsigned* id)
{
...
ci_inline void ci_nvme_plugin_crc_id_release(
    struct nvme_crc_plugin_idp_t* idp, unsigned id)
{
...
ci_inline bool ci_nvme_plugin_crc_last_byte_sent(
    const struct ci_pkt_zc_payload* zcp)
{
...
ci_inline void ci_nvme_plugin_crc_free_acked_ids(
    ci_netif* ni, ci_ip_pkt_fmt* pkt)
{
  ...
  OO_TX_FOR_EACH_ZC_PAYLOAD(ni, zch, zcp)
  {
    if( zcp->zcp_flags & ZC_PAYLOAD_FLAG_INSERT_CRC &&
      ...
      ci_nvme_plugin_crc_id_release(
          &ni->state->nvme_crc_plugin_idp[intf_i], zcp->crc_id);
...
ci_inline void ci_nvme_plugin_crc_packet_cleanup(
    ci_netif* ni, ci_tcp_state* ts, ci_ip_pkt_fmt* pkt)
{
#if CI_CFG_TX_CRC_OFFLOAD
  struct ci_pkt_zc_header* zch = oo_tx_zc_header(pkt);
  struct ci_pkt_zc_payload* zcp;
 ...
 */
ci_inline void ci_nvme_plugin_idp_dropped_queue_cleanup(
    ci_netif* ni, ci_tcp_state* ts, ci_ip_pkt_queue* qu)
{
#if CI_CFG_TX_CRC_OFFLOAD
  oo_pkt_p pp = qu->head;
  ci_ip_pkt_fmt* p;
  ...
  if( ts->current_crc_id != ZC_NVME_CRC_ID_INVALID ) {
    ci_nvme_plugin_crc_id_release(
        &ni->state->nvme_crc_plugin_idp[intf_i], ts->current_crc_id);
 ...
 */
ci_inline void ci_netif_pkt_set_change(
    ci_netif* ni, int bufset_id, int /*bool*/ is_underfilled)
{
  ni->packets->id = bufset_id;
  ci_assert_equal(bufset_id, NI_PKT_SET(ni));

  if( ni->packets->sets_n < ni->packets->sets_max && is_underfilled )
    ef_eplock_holder_set_flag(&ni->state->lock, CI_EPLOCK_NETIF_NEED_PKT_SET);
...
{
  return ((ni->packets->sets_max - ni->packets->sets_n)
 ...
 */
ci_inline ci_ip_pkt_fmt* ci_netif_pkt_tx_tcp_alloc(
    ci_netif* ni, ci_tcp_state* ts)
{
  ...
  if( CI_LIKELY(ci_netif_pkt_tx_may_alloc(ni) &&
    ...
    LOG_S(ci_log(
        "Packets Allocated: %d, Packets free: %d, Set free: %d, Num sets: %d, "
        "Pkts per set: %d, Bufset Id: %d",
        ni->packets->n_pkts_allocated, ni->packets->n_free,
        ni->packets->set[bufset_id].n_free, ni->packets->sets_n, PKTS_PER_SET,
        bufset_id));
    return ci_netif_pkt_get(ni, bufset_id);
  } else {
    if( (! ci_netif_pkt_tx_may_alloc(ni)) &&
        (NI_OPTS(ni).tcp_sndbuf_mode == 2) && (ts != NULL) )
     ...
     */
    return ci_netif_pkt_alloc_slow(
        ni, CI_PKT_ALLOC_FOR_TCP_TX | CI_PKT_ALLOC_USE_NONB);
  }
}
...
again:
  ...
  if( id != 0xffffffff ) {
    ...
    pkt = PKT(ni, pp);
    new_link =
        ((unsigned) OO_PP_ID(pkt->next)) | (link & 0xffffffff00000000llu);
...
ci_inline void ci_netif_pkt_free_nonb_list(
    ci_netif* ni, oo_pkt_p pkt_list, ci_ip_pkt_fmt* pkt_list_tail)
{
  volatile ci_uint64* nonb_pkt_pool_ptr;
...
#ifdef __KERNEL__
ci_inline void ci_netif_pkt_release_mnl(
    ci_netif* ni, ci_ip_pkt_fmt* pkt, int* p_netif_is_locked)
{
  ci_assert_gt(pkt->refcount, 0);
  if( --pkt->refcount == 0 )
...
ci_inline unsigned __ci_tcp_rx_buf_count(ci_netif* netif, ci_tcp_state* ts)
{
  return ts->recv1.num + ts->recv2.num + ts->rob.num;
}

ci_inline unsigned __ci_tcp_rx_reserved_bufs(
    ci_netif* netif, ci_tcp_state* ts, int allocated_pkts)
{
  int reserved_bufs = ts->s.b.state != CI_TCP_ESTABLISHED
                          ? 0
...
ci_inline unsigned ci_tcp_rx_reserved_bufs(ci_netif* netif, ci_tcp_state* ts)
{
  return __ci_tcp_rx_reserved_bufs(
      netif, ts, __ci_tcp_rx_buf_count(netif, ts));
}

/* adjusts per-nic count of reserved buffers
 * needs to be called BEFORE any of tcp recv queues gets to be modified */
ci_inline void ci_tcp_rx_buf_adjust(
    ci_netif* netif, ci_tcp_state* ts, ci_ip_pkt_queue* q, int delta)
{
  int m, n;
  ...
  n = __ci_tcp_rx_reserved_bufs(
      netif, ts, __ci_tcp_rx_buf_count(netif, ts) + delta);
...
ci_inline void ci_tcp_rx_queue_drop(
    ci_netif* ni, ci_tcp_state* ts, ci_ip_pkt_queue* q)
{
  ci_tcp_rx_buf_adjust(ni, ts, q, -q->num);
  ci_ip_queue_drop(ni, q);
}

ci_inline void ci_tcp_rx_buf_account_begin(ci_netif* netif, ci_tcp_state* ts)
{
  ci_assert(ci_netif_is_locked(netif));
  ci_assert_nflags(netif->state->flags, CI_NETIF_FLAG_PKT_ACCOUNT_PENDING);
...
ci_inline void ci_tcp_rx_buf_account_end(ci_netif* netif, ci_tcp_state* ts)
{
  ci_assert(ci_netif_is_locked(netif));
...
ci_inline int ci_ip_queue_is_valid_long(
    ci_netif* netif, ci_ip_pkt_queue* qu, const char* name)
{
  ...
  if( ! ci_ip_queue_is_valid(netif, qu) )
    return 0;
  for( id = qu->head; OO_PP_NOT_NULL(id); id = PKT(netif, id)->next ) {
  ...
  if( i != qu->num ) {
    ci_log("validation of %s queue failed: %d real members, %d declared", name,
        i, qu->num);
    return 0;
  }
  if( i && ! found_tail ) {
    ci_log("validation of %s queue failed: tail %d not in queue", name,
        OO_PP_FMT(qu->tail));
    return 0;
  }
  return 1;
...
ci_inline void __ci_ip_queue_enqueue(
    ci_netif* netif, ci_ip_pkt_queue* qu, ci_ip_pkt_fmt* pkt)
{
...
ci_inline void __ci_tcp_rx_queue_enqueue(
    ci_netif* netif, ci_tcp_state* ts, ci_ip_pkt_queue* qu, ci_ip_pkt_fmt* pkt)
{
...
ci_inline void ci_ip_queue_enqueue(
    ci_netif* netif, ci_ip_pkt_queue* qu, ci_ip_pkt_fmt* pkt)
{
  pkt->next = OO_PP_NULL;
  __ci_ip_queue_enqueue(netif, qu, pkt);
...
ci_inline void ci_ip_queue_dequeue(
    ci_netif* netif, ci_ip_pkt_queue* qu, ci_ip_pkt_fmt* head)
{
...
ci_inline void ci_ip_queue_move(ci_netif* netif, ci_ip_pkt_queue* from,
    ci_ip_pkt_queue* to, ci_ip_pkt_fmt* last, int num)
{
 ...
 * be initialised. */
ci_inline void ci_ip_queue_move_all(
    ci_netif* netif, ci_ip_pkt_queue* from, ci_ip_pkt_queue* to)
{
  ci_assert(netif);
  ci_assert(from);
  ci_assert(to);
...
#define NEXT_IP_ID(ni) \
  (NI_IPID(ni)->base | (NI_IPID(ni)->next++ & CI_IPID_BLOCK_MASK))

#if CI_CFG_IPV6
#define NEXT_IP6_ID(ni) \
  (NI_IPID(ni)->ip6_base | (NI_IPID(ni)->ip6_next++ & CI_IP6ID_BLOCK_MASK))
#endif
...
ci_inline ci_ipx_id_t ci_next_ipx_id_be(int af, ci_netif* ni)
{
  ci_ipx_id_t ipx_id;
#if CI_CFG_IPV6
  if( IS_AF_INET6(af) )
 ...
 */
extern int ci_ip_options_parse(
    ci_netif* netif, ci_ip4_hdr* ip, const int hdr_size);

/**********************************************************************
**************************** citp_waitable ****************************
**********************************************************************/

ci_inline int citp_waitable_lock_or_set_flag(citp_waitable* w, ci_uint32 flag)
{
  ci_uint32 l, new_l;
  int rc;
...
ci_inline void citp_waitable_lock_set_flag(citp_waitable* w, ci_uint32 flag)
{
  ci_uint32 l;
  do {
    l = w->lock.wl_val;
...
ci_inline void citp_waitable_lock_clear_flag(citp_waitable* w, ci_uint32 flag)
{
  ci_uint32 l;
  do {
    l = w->lock.wl_val;
...
#define ci_icmp_send_error(ni, rx_pkt, type, code)                  \
  __ci_icmp_send_error((ni), oo_pkt_af(rx_pkt), oo_ipx_hdr(rx_pkt), \
      oo_ether_hdr(rx_pkt), (type), (code))

ci_inline int ci_icmp_send_port_unreach(ci_netif* ni, ci_ip_pkt_fmt* rx_pkt)
{
  ci_uint8 type, code;
...
extern void ci_udp_recvq_dump(ci_netif* ni, ci_udp_recv_q* q, const char* pf1,
    const char* pf2, oo_dump_log_fn_t logger, void* log_arg) CI_HF;

#if CI_CFG_TIMESTAMPING
extern int ci_udp_timestamp_q_enqueue(
    ci_netif* ni, ci_udp_state* us, ci_ip_pkt_fmt* pkt);
#endif

/* Put a packet into recv_q but don't mark it as visible to the consumer yet.
 * Stack should be locked. */
ci_inline void ci_udp_recv_q_put_pending(
    ci_netif* ni, ci_udp_recv_q* q, ci_ip_pkt_fmt* pkt)
{
  ci_assert(ci_netif_is_locked(ni));
...
ci_inline void ci_udp_recv_q_put(
    ci_netif* ni, ci_udp_recv_q* q, ci_ip_pkt_fmt* pkt)
{
...
ci_inline ci_ip_pkt_fmt* ci_udp_recv_q_get(ci_netif* ni, ci_udp_recv_q* q)
{
  ci_ip_pkt_fmt* pkt;
...
ci_inline void ci_udp_recv_q_deliver(
    ci_netif* ni, ci_udp_recv_q* q, ci_ip_pkt_fmt* pkt)
{
...
ci_inline ci_ip_pkt_fmt* ci_udp_recv_q_next(ci_netif* ni, ci_ip_pkt_fmt* pkt)
{
...
ci_inline void ci_tcp_rto_set_with_timeout(
    ci_netif* netif, ci_tcp_state* ts, ci_iptime_t timeout)
{
  /* shouldn't set an RTO if retrans queue is empty */
  ci_assert(! ci_tcp_retransq_is_empty(ts));
  /* shouldn't set an RTO timer in a state that doesn't allow them */
...
#define ci_tcp_rto_set(ni, ts) \
  ci_tcp_rto_set_with_timeout((ni), (ts), (ts)->rto)
...
/* delayed ack timers */
ci_inline void ci_tcp_delack_check_and_set(ci_netif* netif, ci_tcp_state* ts)
{
...
ci_inline int ci_tcp_taildrop_probe_enabled(
    const ci_netif* ni, const ci_tcp_state* ts)
{
  return NI_OPTS(ni).tail_drop_probe && (ts->tcpflags & CI_TCPT_FLAG_SACK) &&
...
ci_inline unsigned ci_tcp_taildrop_timeout(
    const ci_netif* netif, const ci_tcp_state* ts)
{
  unsigned offset;
 ...
 */
ci_inline void ci_tcp_kalive_restart(
    ci_netif* netif, ci_tcp_state* ts, ci_iptime_t t)
{
  /*
  ...
  ci_assert(ts->s.b.state != CI_TCP_CLOSED && ts->s.b.state != CI_TCP_LISTEN);
 ...
 * @param netif netif
 */
ci_inline ci_iptime_t ci_tcp_kalive_intvl_get(
    ci_netif* netif, ci_tcp_state* ts)
{
...
ci_inline void ci_tcp_recycle_reset(ci_netif* netif, ci_tcp_state* ts)
{
  ...
  struct oo_p_dllink_state link =
      oo_p_dllink_sb(netif, &ts->s.b, &ts->recycle_link);
  ci_assert(ci_ip_queue_not_empty(&ts->rob));
  if( ! oo_p_dllink_is_empty(netif, link) )
    return;
  oo_p_dllink_add(
      netif, oo_p_dllink_ptr(netif, &netif->state->recycle_retry_q), link);
  if( ! ci_ip_timer_pending(netif, &netif->state->recycle_tid) ) {
    ...
    ci_ip_timer_set(
        netif, &netif->state->recycle_tid, ci_tcp_time_now(netif) + 1);
  }
#endif
}
...
ci_inline ci_uint16 tcp_eff_mss(const ci_tcp_state* ts)
{
  if( ts->s.b.state != CI_TCP_CLOSED ) {
    ...
    ci_assert_ge(
        ts->eff_mss, CI_CFG_TCP_MINIMUM_MSS - tcp_outgoing_opts_len(ts));
  }
  return ts->eff_mss;
}
...
ci_inline ci_iptime_t ci_tcp_isn2tick(ci_netif* ni, ci_uint32 isn)
{
  ci_uint64 ticks = isn;
 ...
 * acceptable.
 */
ci_inline unsigned ci_tcp_future_isn(ci_netif* ni, ci_addr_t laddr,
    ci_uint16 lport_be, ci_addr_t raddr, ci_uint16 rport_be,
    ci_uint64 future_delta_ticks)
{
  ci_uint64 frc;
  ci_uint32 hash = onload_hash3(laddr, lport_be, raddr, rport_be, IPPROTO_TCP);
  ci_frc64(&frc);
...
ci_inline unsigned ci_tcp_initial_seqno(ci_netif* ni, ci_addr_t laddr,
    ci_uint16 lport_be, ci_addr_t raddr, ci_uint16 rport_be)
{
...
ci_inline ci_uint32 ci_tcp_rcvbuf2window(
    ci_uint32 so_rcvbuf, ci_uint32 amss, ci_uint8 rcv_wscl)
{
  ci_assert(amss);
...
ci_inline ci_uint16 ci_tcp_calc_rcv_wnd_syn(
    ci_uint32 so_rcvbuf, ci_uint16 amss, ci_uint8 rcv_wscl)
{
...
{
  ts->rcv_window_max =
      ci_tcp_rcvbuf2window(ts->s.so.rcvbuf, ts->amss, ts->rcv_wscl);
...
#define ci_tcp_reduce_faststart(ts, n) \
  do {                                 \
  } while( 0 )
...
ci_inline int ci_tcp_can_stripe(
    ci_netif* ni, unsigned laddr_be32, unsigned raddr_be32)
{
#if CI_CFG_PORT_STRIPING
...
ci_inline void ci_tcp_set_rtt_timing(
    ci_netif* netif, ci_tcp_state* ts, int seq)
{
  ts->timed_seq = seq;
...
{
  uint8_t* end =
      (uint8_t*) oo_tx_l3_hdr(pkt) + ts->outgoing_hdrs_len + tcp_eff_mss(ts);
...
ci_inline void ci_tcp_tx_pkt_set_zc_header_pos(
    ci_tcp_state* ts, ci_ip_pkt_fmt* pkt)
{
  char* end = CI_PTR_ALIGN_FWD(
      (char*) oo_tx_l3_hdr(pkt) + sizeof(ci_tcp_hdr) + CI_TCP_MAX_OPTS_LEN,
...
ci_inline unsigned ci_ipx_tcp_checksum(
    int af, const ci_ipx_hdr_t* ipx, const ci_tcp_hdr* tcp, void* payload)
{
#if CI_CFG_IPV6
  if( af == AF_INET6 ) {
...
ci_inline unsigned ci_ipx_udp_checksum(
    int af, const ci_ipx_hdr_t* ipx, const ci_udp_hdr* udp, void* payload)
{
 ...
 */
ci_inline int ci_sock_lock(
    ci_netif*, citp_waitable*) OO_MUST_CHECK_RET_IN_KERNEL;
...
ci_inline void ci_tcp_acceptq_put(
    ci_netif* ni, ci_tcp_socket_listen* tls, citp_waitable* w)
{
  ci_assert(OO_SP_IS_NULL(w->wt_next));
  ci_assert(ci_netif_is_locked(ni));
  do
    w->wt_next = OO_SP_FROM_INT(ni, tls->acceptq_put);
  while( ci_cas32_fail(&tls->acceptq_put, OO_SP_TO_INT(w->wt_next), W_ID(w)) );
  ++tls->acceptq_n_in;
}


ci_inline void ci_tcp_acceptq_put_back_tail(
    ci_netif* ni, ci_tcp_socket_listen* tls, citp_waitable* w)
{
  ci_assert(OO_SP_IS_NULL(w->wt_next));
 ...
 * ci_tcp_acceptq_peek(). */
ci_inline void ci_tcp_acceptq_get_swizzle(
    ci_netif* ni, ci_tcp_socket_listen* tls)
{
  ci_int32 from;
  oo_sp from_sp;
  ci_tcp_state* ts;
...
ci_inline citp_waitable* ci_tcp_acceptq_get(
    ci_netif* ni, ci_tcp_socket_listen* tls)
{
  citp_waitable* w;
  ci_assert(ci_sock_is_locked(ni, &tls->s.b) ||
...
/* Only call this if ci_tcp_acceptq_not_empty() is true. */
ci_inline ci_tcp_state* ci_tcp_acceptq_peek(
    ci_netif* ni, ci_tcp_socket_listen* tls)
{
  ci_assert(ci_sock_is_locked(ni, &tls->s.b));
  if( OO_SP_IS_NULL(tls->acceptq_get) )
...
ci_inline void ci_tcp_acceptq_put_back(
    ci_netif* ni, ci_tcp_socket_listen* tls, citp_waitable* w)
{
  ci_assert(ci_sock_is_locked(ni, &tls->s.b));
  ci_assert(w->sb_aflags & CI_SB_AFLAG_TCP_IN_ACCEPTQ);
...
{
  switch( type ) {
    case CI_TCP_AUX_TYPE_SYNRECV:
      return "syn-recv state";
    case CI_TCP_AUX_TYPE_BUCKET:
      return "syn-recv bucket";
    case CI_TCP_AUX_TYPE_EPOLL:
      return "epoll3 state";
    default:
      return "unknown";
  }
...
ci_inline citp_waitable* ci_ni_aux2container_w(ci_ni_aux_mem* aux)
{
...
ci_inline struct oo_p_dllink_state ci_ni_aux2link(
    ci_netif* ni, ci_ni_aux_mem* aux)
{
...
ci_inline struct oo_p_dllink_state ci_tcp_synrecv2link(
    ci_netif* ni, ci_tcp_state_synrecv* tsr)
{
...
ci_inline struct oo_p_dllink_state ci_sb_epoll_ready_link(
    ci_netif* ni, ci_sb_epoll_state* epoll, int i)
{
  return oo_p_dllink_sb(ni,
      ci_ni_aux2container_w(CI_CONTAINER(ci_ni_aux_mem, u.epoll, epoll)),
...
ci_inline void ci_netif_put_on_post_poll_epoll(ci_netif* ni, citp_waitable* sb)
{
#if CI_CFG_EPOLL3
  ...
  CI_READY_LIST_EACH(sb->ready_lists_in_use, tmp, i)
  {
    ...
    oo_p_dllink_del(ni, link);
    oo_p_dllink_add_tail(
        ni, oo_p_dllink_ptr(ni, &ni->state->ready_lists[i]), link);
  }
#endif
}

ci_inline void citp_waitable_remove_from_epoll(
    ci_netif* ni, citp_waitable* w, int do_free)
{
...
ci_inline void ci_netif_put_on_post_poll(ci_netif* ni, citp_waitable* sb)
{
  ...
  oo_p_dllink_del(ni, link);
  oo_p_dllink_add_tail(
      ni, oo_p_dllink_ptr(ni, &ni->state->post_poll_list), link);
...
ci_inline void ci_netif_poll_free_pkts(
    ci_netif* ni, struct ci_netif_poll_state* ps)
{
  ci_ip_pkt_fmt* tail =
      CI_CONTAINER(ci_ip_pkt_fmt, next, ps->tx_pkt_free_list_insert);
...
ci_inline void ci_pmtu_discover_timer(
    ci_netif* ni, ci_pmtu_state_t* pmtus, ci_iptime_t timeout)
{
...
{
  if( s->domain == PF_INET6 &&
      (! ci_tcp_ipv6_is_ipv4(addr) || ci_tcp_ipv6_is_addr_any(addr)) )
    return 1;
  return 0;
}
#endif
...
ci_inline void ci_make_sockaddr_from_ip4(
    struct sockaddr_in* sin, ci_uint16 port_be16, ci_uint32 addr_be32)
{
 ...
 * into sockaddr_in6. */
ci_inline void ci_make_sockaddr_in6_from_ip4(
    struct sockaddr_in6* sin, ci_uint16 port_be16, ci_uint32 addr_be32)
{
 ...
 * into sockaddr_in6. */
ci_inline void ci_make_sockaddr_in6_from_ip6(struct sockaddr_in6* sin,
...
ci_inline struct sockaddr_storage ci_make_sockaddr_storage_from_addr(
    ci_uint16 port_be16, ci_addr_t addr)
{
  union {
...
ci_inline void ci_addr_to_user(struct sockaddr* sa, socklen_t* sa_len,
    sa_family_t domain_in, sa_family_t domain_out, ci_uint16 port_be16,
    const ci_uint32* addr_be32_p, ci_ifid_t scope_id)
{
...
ci_inline int ci_sock_set_ip6_scope_id(ci_netif* ni, ci_sock_cmn* s,
    const struct sockaddr* addr, socklen_t addrlen, int /*bool*/ at_connect)
{
  ...
  if( addrlen >= sizeof(struct sockaddr_in6) && sin6->sin6_scope_id ) {
    ...
    if( at_connect && s->cp.so_bindtodevice &&
      ...
      return -1;
    rc =
        oo_cp_find_llap(ni->cplane, ifindex, NULL, NULL, &hwports, NULL, NULL);
    if( rc != 0 || hwports == 0 )
      return -1;
    s->cp.so_bindtodevice = ifindex;
  }
...
ci_inline void ci_tcp_recvmsg_args_init(ci_tcp_recvmsg_args* a, ci_netif* ni,
    ci_tcp_state* ts, ci_msghdr* msg, int flags)
{
  a->ni = ni;
  a->ts = ts;
  a->msg = msg;
...
ci_inline int oo_tcpdump_check_no_match(
    ci_netif* ni, ci_ip_pkt_fmt* pkt, int intf_i)
{
...
ci_inline void oo_os_sock_status_bit_set(ci_sock_cmn* s, ci_int32 bits)
{
  ci_uint32 tmp;
  do {
    tmp = s->os_sock_status;
  } while( ci_cas32u_fail(
      &s->os_sock_status, tmp, (tmp + (1 << OO_OS_STATUS_SEQ_SHIFT)) | bits) );
}
...
static inline ef_addr zc_usermem_dma_addr(
    struct ci_zc_usermem* um, uint64_t user_ptr, int intf_i)
{
  if( um->addr_space == EF_ADDRSPACE_LOCAL ) {
    uint64_t offset = user_ptr - um->base;
    uint64_t* hw_addrs =
        um->hw_addrs + ((intf_i * um->size) >> EF_VI_NIC_PAGE_SHIFT);
